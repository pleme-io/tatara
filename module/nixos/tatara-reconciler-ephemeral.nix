# NixOS module — system-wide tatara-reconciler ephemeral defaults.
#
# Writes the typed YAML at /etc/tatara-reconciler/ephemeral.yaml; the
# binary's systemd service unit (services.tatara-reconciler) sets
# Environment="TATARA_RECONCILER_EPHEMERAL_CONFIG=/etc/tatara-reconciler/ephemeral.yaml"
# so shikumi finds it without an XDG search.
...
in
{
  options.services.tatara-reconciler.ephemeral = {
    enable = lib.mkEnableOption "tatara-reconciler ephemeral defaults (system-wide)";
  ...
  config = lib.mkIf cfg.enable {
    ...
    # Hint for systemd unit configs (consumers that ship a unit can read
    # this assertion and avoid divergence between the file location and
    # the binary's CLI flag).
    assertions = [
      {
        assertion = builtins.pathExists "/etc/tatara-reconciler";
        message =
          "tatara-reconciler ephemeral defaults expected /etc/tatara-reconciler to exist (the controller chart writes it on install)";
      }
    ];
  };
}
