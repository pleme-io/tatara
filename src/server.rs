...
use crate::api::graphql::{self, TataraSchema};
use crate::api::rest;
use crate::client::executor::Executor;
use crate::client::log_collector::LogCollector;
use crate::cluster::gossip::GossipCluster;
use crate::cluster::membership::MembershipReconciler;
use crate::cluster::raft_node::RaftCluster;
use crate::cluster::raft_sm::TypeConfig;
use crate::cluster::roles;
...
use crate::kindling_bridge::identity;
use crate::p2p::cache::DataCache;
...
pub async fn run(config: ServerConfig) -> Result<()> {
    ...
    let kindling_client =
        crate::kindling_bridge::client::probe_kindling(&config.kindling.daemon_addr).await;
    ...
    let node_meta = match &kindling_id {
        ...
        None => {
            ...
            NodeMeta {
                ...
                total_resources: crate::domain::job::Resources {
                    cpu_mhz,
                    memory_mb: 0,
                },
                available_resources: crate::domain::job::Resources {
                    cpu_mhz,
                    memory_mb: 0,
                },
    ...
    if config.cluster.mdns_discovery {
        ...
        match crate::cluster::discovery::discover_peers(
    ...
    if config.kindling.publish_reports {
        ...
        if let Ok(Some(report)) = crate::kindling_bridge::report::load_report(report_path) {
            if let Err(e) =
                crate::kindling_bridge::report::publish_report(&transfer, &report, &hostname).await
            {
    ...
    let _mdns_announcer = if config.cluster.mdns_discovery {
        ...
        match crate::cluster::discovery::MdnsAnnouncer::new(
...
async fn raft_vote(
    State(state): State<RaftEndpointState>,
    Json(rpc): Json<openraft::raft::VoteRequest<crate::cluster::types::NodeId>>,
) -> impl IntoResponse {
