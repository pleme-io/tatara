...
) -> Result<kube::api::GroupVersionKind, KubeError> {
...
pub async fn server_side_apply(
    ...
    let (ar, caps) = discovery::pinned_kind(client, &gvk).await.map_err(|e| {
        KubeError::DiscoveryFailed {
    ...
    let obj: DynamicObject =
        serde_json::from_value(resource.manifest.clone()).map_err(|e| {
            KubeError::ApplyFailed {
                kind: resource.identity.kind.clone(),
