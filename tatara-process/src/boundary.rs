...
impl Boundary {
    ...
    /// (precondition, postcondition, condition-union) at-least-one
    /// halfspace triad on [`Boundary`] on the closed-set-inversion
    /// axis, byte-for-byte peer of the at-least-one halfspace triad
    /// [`Self::has_any_missing_condition_kind`] on the closed-set-
    /// complement axis.
    ///
    /// # Composed body
    ///
    /// `ConditionKind::ALL.iter().copied().any(|k|
    /// self.has_condition_kind(k))` — a SHORT-CIRCUITING closed-set
    /// walk under the two-slice union primitive
    /// [`Self::has_condition_kind`]. The walk returns `true` at the
    /// FIRST kind present in EITHER slice WITHOUT materializing
    /// [`Self::distinct_condition_kinds`]'s `Vec` and WITHOUT walking
    /// every kind to build [`Self::distinct_condition_kind_count`]'s
    /// scalar. Strictly cheaper than either widened primitive on every
    /// non-empty arm because the walk short-circuits at the first
    /// populated kind rather than paying for the Vec allocation or the
    /// full cardinality count.
    ///
    /// # Peer on the ephemeral surface — [`crate::ephemeral::EphemeralSpec::has_any_distinct_condition_kind`]
    ///
    /// Byte-identical signature `(&Self) -> bool`, byte-identical
    /// `ConditionKind::ALL.iter().copied().any(|k|
    /// self.has_condition_kind(k))` body, on the sugar-surface type
    /// whose pre/post condition vectors live directly on the struct.
    /// Both methods compose against the SAME slice-level substrate
    /// primitive [`ConditionSliceExt::has_any_distinct_kind`] via the
    /// two-slice union through [`Self::has_condition_kind`] — a
    /// regression at the per-slice `any` short-circuit fails at that
    /// primitive's tests rather than as silent drift at either struct-
    /// level at-least-one halfspace caller.
    ///
    /// # Compounding
    ///
    /// A `has-any-distinct-condition-kind` require-tag classifier arm
    /// — byte-for-byte peer of the tagged-union `has-any-populated-
    /// kind` classifier one struct-layer up + the at-least-one
    /// halfspace triad's closed-set-inversion peer — reaches this
    /// primitive at ONE call site rather than restating
    /// `boundary.distinct_condition_kind_count() > 0` (which walks
    /// every kind to count) or
    ...
    /// and the slice-level substrate primitive under short-circuit
    /// semantics). THEORY.md §VI.1 (generation over composition — a
    /// new [`ConditionKind`] variant reaches both surfaces' at-least-
    /// one halfspace triads mechanically through the delegated union
    /// primitive).
    #[must_use]
    ...
    /// [`ConditionKind::ALL`] variant — the precondition-side arm of
    /// the (precondition, postcondition, condition-union) at-least-one
    /// halfspace triad on [`Boundary`] on the closed-set-inversion
    /// axis. Thin typed delegate to
    /// [`ConditionSliceExt::has_any_distinct_kind`] over
    /// [`Self::preconditions`].
    ///
    /// Peer of [`Self::has_any_distinct_postcondition_kind`] on the
    /// (precondition, postcondition) partition of the boundary's two
    /// condition-vector slots; both peers compose against the SAME
    /// slice-level substrate primitive so a regression at the per-
    ...
    /// [`Self::postconditions`].
    ///
    /// Peer of [`Self::has_any_distinct_precondition_kind`]. See that
    /// method for the full rationale — the two methods share ONE lift
    /// motivation, ONE fail-before-pass-after composition-law pin, and
    /// ONE two-surface parity contract with the ephemeral sugar type
    /// via
    /// [`crate::ephemeral::EphemeralSpec::has_any_distinct_postcondition_kind`].
    #[must_use]
    ...
    /// The SOLE [`ConditionKind::ALL`] variant covered by
    /// `preconditions ∪ postconditions`, or `None` when the union
    /// covers 0 or ≥ 2 kinds — the union arm of the (precondition,
    /// postcondition, condition-union) exactly-one-hit witnessing
    /// triad on [`Boundary`] closing the singleton-coverage witness on
    /// the closed-set-inversion axis on the union of the two condition
    /// slots. Witnessing scalar peer of
    /// [`Self::has_unique_distinct_condition_kind`] (=1 predicate) on
    /// the union axis: where the Boolean predicate answers "does the
    /// union cover EXACTLY one kind?", `unique_distinct_condition_kind`
    /// returns `Some(k)` naming that SOLE kind (or `None` on the 0 or
    /// ≥ 2 arms).
    ///
    /// # Composed body
    ///
    /// A two-step-short-circuit walk over [`ConditionKind::ALL`] under
    /// the [`Self::has_condition_kind`] union primitive — pull the
    /// first hit; return `Some(first)` iff the second hit is [`None`],
    /// else `None`. Byte-for-byte peer of
    /// [`ConditionSliceExt::unique_distinct_kind`] one slice-layer
    /// down, lifted to compose against
    /// [`Self::has_condition_kind`]'s pre-OR-post union rather than
    /// against a single slice's `has_kind`. A regression at the union
    /// primitive fails at the slice-level substrate tests + the union
    /// composition-law tests rather than as silent drift here.
    ///
    /// # Peer on the ephemeral surface — [`crate::ephemeral::EphemeralSpec::unique_distinct_condition_kind`]
    ///
    /// Byte-identical signature `(&Self) -> Option<ConditionKind>`,
    /// byte-identical two-step short-circuit body composed against the
    /// ephemeral surface's own union primitive. Both methods compose
    /// against the SAME slice-level substrate primitive
    /// [`ConditionSliceExt::unique_distinct_kind`] via the two-slice
    /// union — a regression at the per-slice singleton-coverage
    /// witnessing walk fails at that primitive's tests rather than as
    /// silent drift at either struct-level singleton-coverage caller.
    ///
    /// # Sibling to [`Self::unique_missing_condition_kind`]
    ///
    /// Closed-set-inversion peer of the exactly-one-hit witness on the
    /// closed-set-complement axis at the boundary union struct layer —
    /// where `unique_missing_condition_kind` names the SOLE kind AWAY
    /// from covered, `unique_distinct_condition_kind` names the SOLE
    /// kind covered. Both compose against a two-step-short-circuit walk
    /// over [`ConditionKind::ALL`] under [`Self::has_condition_kind`]
    /// (this primitive) vs `!has_condition_kind` (the missing peer).
    ///
    /// Theory anchor: THEORY.md §II.1 invariant 5 (composition
    /// preserves proofs — the exactly-one-hit witnessing projection on
    /// the closed-set-inversion axis composes the SAME two-step short-
    /// circuit walk under a two-slice union on both this boundary
    /// surface and the ephemeral surface). THEORY.md §VI.1 (generation
    /// over composition — a new [`ConditionKind`] variant reaches both
    /// surfaces' unique-distinct-kind triads mechanically through the
    /// delegated union primitive).
    #[must_use]
    pub fn unique_distinct_condition_kind(&self) -> Option<ConditionKind> {
        let mut it = ConditionKind::ALL
            .iter()
            .copied()
            .filter(|k| self.has_condition_kind(*k));
    ...
    /// The SOLE [`ConditionKind::ALL`] variant covered by
    /// [`Self::preconditions`], or `None` when preconditions cover 0
    /// or ≥ 2 kinds — the precondition-side arm of the (precondition,
    /// postcondition, condition-union) exactly-one-hit witnessing
    /// triad on [`Boundary`] on the closed-set-inversion axis. Thin
    /// typed delegate to [`ConditionSliceExt::unique_distinct_kind`]
    /// over [`Self::preconditions`].
    ///
    /// Peer of [`Self::unique_distinct_postcondition_kind`] on the
    /// (precondition, postcondition) partition of the boundary's two
    /// condition-vector slots; both peers compose against the SAME
    /// slice-level substrate primitive so a regression at the per-
    /// slice two-step short-circuit witnessing walk fails at that
    /// primitive's tests rather than as silent drift at either
    /// struct-level arm.
    #[must_use]
    pub fn unique_distinct_precondition_kind(&self) -> Option<ConditionKind> {
        self.preconditions.unique_distinct_kind()
    }

    /// The SOLE [`ConditionKind::ALL`] variant covered by
    /// [`Self::postconditions`], or `None` when postconditions cover 0
    /// or ≥ 2 kinds — the postcondition-side arm of the (precondition,
    /// postcondition, condition-union) exactly-one-hit witnessing
    /// triad on [`Boundary`] on the closed-set-inversion axis. Thin
    /// typed delegate to [`ConditionSliceExt::unique_distinct_kind`]
    /// over [`Self::postconditions`].
    ///
    /// Peer of [`Self::unique_distinct_precondition_kind`]. See that
    /// method for the full rationale — the two methods share ONE lift
    /// motivation, ONE fail-before-pass-after composition-law pin, and
    /// ONE two-surface parity contract with the ephemeral sugar type
    /// via
    /// [`crate::ephemeral::EphemeralSpec::unique_distinct_postcondition_kind`].
    #[must_use]
    pub fn unique_distinct_postcondition_kind(&self) -> Option<ConditionKind> {
        self.postconditions.unique_distinct_kind()
    }
    ...
    /// many-arm triad on [`Boundary`] closing the "≥ 2 kinds covered"
    /// arm on the union of the two condition slots. The many-arm
    /// Boolean fast-path peer of
    /// [`Self::has_unique_distinct_condition_kind`] (=1 arm) and
    /// [`Self::has_any_distinct_condition_kind`] (≥1 halfspace) on the
    /// union axis, closing the {0, 1, ≥2} trichotomy on the distinct
    /// axis at the union struct layer.
    ///
    /// Composed body: constructs a two-step-short-circuit walk over
    /// [`ConditionKind::ALL`] under the [`Self::has_condition_kind`]
    /// union primitive — pulls up to two hits off the filtered
    ...
    /// [`ConditionSliceExt::has_multiple_distinct_kinds`] one slice-
    /// layer down, lifted to compose against
    /// [`Self::has_condition_kind`]'s pre-OR-post union rather than
    /// against a single slice's `has_kind`. A regression at the union
    /// primitive fails at the slice-level substrate tests + the union
    /// composition-law tests rather than as silent drift here.
    ///
    /// # Peer on the ephemeral surface — [`crate::ephemeral::EphemeralSpec::has_multiple_distinct_condition_kind`]
    ///
    /// Byte-identical signature `(&Self) -> bool`, byte-identical
    /// two-step short-circuit body composed against the ephemeral
    /// surface's own union primitive. Both methods compose against
    /// the SAME slice-level substrate primitive
    /// [`ConditionSliceExt::has_multiple_distinct_kinds`] via the
    /// two-slice union — a regression at the per-slice many-arm walk
    /// fails at that primitive's tests rather than as silent drift at
    /// either struct-level many-distinct caller.
    ///
    /// # Sibling to [`Self::has_multiple_missing_condition_kind`]
    ///
    /// Closed-set-inversion peer of the cardinality-many-arm on the
    /// closed-set-complement axis at the boundary union struct layer
    /// — where `has_multiple_missing_condition_kind` returns `true`
    /// iff the union has ≥ 2 kinds ABSENT, `has_multiple_distinct_condition_kind`
    /// returns `true` iff the union covers ≥ 2 kinds. Both compose
    /// against a two-step-short-circuit walk over [`ConditionKind::ALL`]
    /// under [`Self::has_condition_kind`] (this primitive) vs
    /// `!has_condition_kind` (the missing peer).
    ///
    /// # Compounding
    ///
    /// A future operator-facing "≥ 2 dependencies covered" fast-path
    /// diagnostic reads `boundary.has_multiple_distinct_condition_kind()`
    /// at ONE call site rather than restating
    /// `boundary.distinct_condition_kind_count() >= 2` (which walks
    /// every slot to count) or
    /// `boundary.distinct_condition_kinds().len() >= 2` (which
    /// allocates the Vec). A `has-multiple-distinct-condition-kind`
    /// require-tag classifier arm reaches this primitive at ONE
    /// substrate call — byte-for-byte peer of the tagged-union
    /// `has-multiple-populated-kinds` classifier one struct-layer up.
    ///
    /// Theory anchor: THEORY.md §II.1 invariant 5 (composition
    /// preserves proofs — the cardinality-many-arm projection on the
    /// closed-set-inversion axis composes the SAME two-step short-
    /// circuit walk under a two-slice union on both this boundary
    ...
    /// many-arm triad on [`Boundary`] on the closed-set-inversion axis.
    /// Thin typed delegate to
    /// [`ConditionSliceExt::has_multiple_distinct_kinds`] over
    /// [`Self::preconditions`].
    ///
    /// Peer of [`Self::has_multiple_distinct_postcondition_kind`] on
    ...
    /// [`Self::postconditions`].
    ///
    /// Peer of [`Self::has_multiple_distinct_precondition_kind`]. See
    /// that method for the full rationale — the two methods share ONE
    /// lift motivation, ONE fail-before-pass-after composition-law
    /// pin, and ONE two-surface parity contract with the ephemeral
    /// sugar type via
    /// [`crate::ephemeral::EphemeralSpec::has_multiple_distinct_postcondition_kind`].
    #[must_use]
    ...
    /// [`ConditionKind::ALL`] variant — the union arm of the
    /// (precondition, postcondition, condition-union) cardinality
    /// "≤ 1" triad on [`Boundary`] closing the "at most one kind
    ...
    /// EMPTY-OR-SINGLETON on the union (zero or exactly one kind
    /// present across the union of the two slices).
    ///
    ...
    /// many-arm walk stops with fewer than two distinct hits, WITHOUT
    ...
    /// peer of [`ConditionSliceExt::has_at_most_one_distinct_kind`]
    ...
    /// composition-law tests rather than as silent drift here.
    ///
    /// # Peer on the ephemeral surface — [`crate::ephemeral::EphemeralSpec::has_at_most_one_distinct_condition_kind`]
    ///
    /// Byte-identical signature `(&Self) -> bool`, byte-identical
    /// definitional-negation body composed against the ephemeral
    /// surface's own many-arm union primitive. Both methods compose
    /// against the SAME slice-level substrate primitive
    /// [`ConditionSliceExt::has_at_most_one_distinct_kind`] via the
    /// two-slice union — a regression at the per-slice "≤ 1" negation
    /// fails at that primitive's tests rather than as silent drift at
    /// either struct-level empty-or-singleton caller.
    ///
    /// # Peer to [`Self::has_at_most_one_missing_condition_kind`]
    ///
    /// Axis-parity mirror of the closed-set-complement "≤ 1" negation
    /// peer on the union struct layer — where
    /// `has_at_most_one_missing_condition_kind` tests "at most one
    /// kind MISSING across the union" (near-saturation-or-saturated),
    /// this primitive tests "at most one kind PRESENT across the
    /// union" (empty-or-singleton). Both compose the same
    /// definitional negation shape (`!has_multiple_*_condition_kind()`)
    /// at the union struct layer — the two primitives close the "≤ 1"
    /// arm on both the closed-set-complement and closed-set-inversion
    /// axes at the union struct layer under the SAME shape.
    ///
    /// # Compounding
    ///
    /// A future operator-facing "at most one dependency currently
    /// covered" coverage-analysis diagnostic reads
    /// `boundary.has_at_most_one_distinct_condition_kind()` at ONE
    /// call site rather than restating
    /// `boundary.distinct_condition_kind_count() <= 1` (which walks
    /// every slot to count) or
    /// `boundary.distinct_condition_kinds().len() <= 1` (which
    /// allocates the Vec). A `has-at-most-one-distinct-condition-kind`
    /// require-tag classifier arm reaches this primitive at ONE
    /// substrate call — byte-for-byte peer of the tagged-union
    /// `has-at-most-one-populated-kind` classifier one struct-layer
    /// up, closing the {0, 1, ≥ 2, ≤ 1} cardinality-Boolean grid on
    /// the distinct axis at the Boundary struct layer alongside its
    /// sibling `has-multiple-distinct-condition-kind` under the
    /// Boolean negation axis.
    ///
    /// Theory anchor: THEORY.md §II.1 invariant 5 (composition
    ...
    /// [`ConditionKind::ALL`] variant — the precondition-side arm of
    /// the (precondition, postcondition, condition-union) cardinality
    /// "≤ 1" triad on [`Boundary`] on the closed-set-inversion axis.
    /// Thin typed delegate to
    /// [`ConditionSliceExt::has_at_most_one_distinct_kind`] over
    /// [`Self::preconditions`].
    ///
    /// Peer of [`Self::has_at_most_one_distinct_postcondition_kind`]
    /// on the (precondition, postcondition) partition of the boundary's
    /// two condition-vector slots; both peers compose against the SAME
    /// slice-level substrate primitive so a regression at the per-
    ...
    /// [`Self::postconditions`].
    ///
    /// Peer of [`Self::has_at_most_one_distinct_precondition_kind`].
    ...
    /// [`crate::ephemeral::EphemeralSpec::has_at_most_one_distinct_postcondition_kind`].
    #[must_use]
    ...
    /// The SOLE [`ConditionKind::ALL`] variant ABSENT from
    /// `preconditions ∪ postconditions`, or `None` when the union is
    /// missing 0 or ≥ 2 kinds — the union arm of the (precondition,
    /// postcondition, condition-union) exactly-one-missing witnessing
    /// triad on [`Boundary`] closing the near-saturation witness on
    /// the closed-set-complement axis on the union of the two
    /// condition slots. Witnessing scalar peer of
    /// [`Self::has_unique_missing_condition_kind`] (=1 predicate) on
    /// the union axis: where the Boolean predicate answers "is the
    /// union one kind AWAY from covered?", `unique_missing_condition_kind`
    /// returns `Some(k)` naming that SOLE remaining hole (or `None` on
    /// the 0 or ≥ 2 arms).
    ///
    /// # Composed body
    ///
    /// A two-step-short-circuit walk over [`ConditionKind::ALL`] under
    /// a NEGATED [`Self::has_condition_kind`] union primitive — pull
    /// the first hit; return `Some(first)` iff the second hit is
    /// [`None`], else `None`. Byte-for-byte peer of
    /// [`ConditionSliceExt::unique_missing_kind`] one slice-layer
    /// down, lifted to compose against the two-slice union rather than
    /// against a single slice's `has_kind`. A regression at the union
    /// primitive fails at the slice-level substrate tests + the union
    /// composition-law tests rather than as silent drift here.
    ///
    /// # Peer on the ephemeral surface — [`crate::ephemeral::EphemeralSpec::unique_missing_condition_kind`]
    ///
    /// Byte-identical signature `(&Self) -> Option<ConditionKind>`,
    /// byte-identical two-step short-circuit body composed against the
    /// ephemeral surface's own union primitive. Both methods compose
    /// against the SAME slice-level substrate primitive
    /// [`ConditionSliceExt::unique_missing_kind`] via the two-slice
    /// union — a regression at the per-slice near-saturation
    /// witnessing walk fails at that primitive's tests rather than as
    /// silent drift at either struct-level near-saturation caller.
    ///
    /// # Sibling to [`Self::unique_distinct_condition_kind`]
    ///
    /// Closed-set-complement peer of the exactly-one-hit witness on
    /// the closed-set-inversion axis at the boundary union struct
    /// layer — where `unique_distinct_condition_kind` names the SOLE
    /// kind COVERED, `unique_missing_condition_kind` names the SOLE
    /// kind ABSENT. Both compose against a two-step-short-circuit walk
    /// over [`ConditionKind::ALL`] under [`Self::has_condition_kind`]
    /// vs `!has_condition_kind` (this primitive).
    ///
    /// # Compounding
    ///
    /// A future gap-analysis diagnostic that names "one remaining
    /// [`ConditionKind`] not covered by this Boundary" reads
    /// `boundary.unique_missing_condition_kind()` at ONE call site —
    /// the WITNESS + the exactly-one predicate composed at ONE short-
    /// circuit walk, rather than pairing the Boolean
    /// [`Self::has_unique_missing_condition_kind`] with
    /// [`Self::first_missing_condition_kind`] at TWO independent walks
    /// whose agreement is a coincidence.
    ///
    /// Theory anchor: THEORY.md §II.1 invariant 5 (composition
    /// preserves proofs — the complement-exactly-one-hit witnessing
    /// projection composes the SAME two-step short-circuit walk under
    /// a two-slice union negation on both this boundary surface and
    /// the ephemeral surface). THEORY.md §VI.1 (generation over
    /// composition — a new [`ConditionKind`] variant reaches both
    /// surfaces' unique-missing-kind triads mechanically through the
    /// delegated union primitive).
    #[must_use]
    pub fn unique_missing_condition_kind(&self) -> Option<ConditionKind> {
        let mut it = ConditionKind::ALL
            .iter()
            .copied()
            .filter(|k| !self.has_condition_kind(*k));
    ...
    /// The SOLE [`ConditionKind::ALL`] variant ABSENT from
    /// [`Self::preconditions`], or `None` when preconditions are
    /// missing 0 or ≥ 2 kinds — the precondition-side arm of the
    /// (precondition, postcondition, condition-union) exactly-one-
    /// missing witnessing triad on [`Boundary`]. Thin typed delegate
    /// to [`ConditionSliceExt::unique_missing_kind`] over
    /// [`Self::preconditions`].
    ///
    /// Peer of [`Self::unique_missing_postcondition_kind`] on the
    /// (precondition, postcondition) partition of the boundary's two
    /// condition-vector slots; both peers compose against the SAME
    /// slice-level substrate primitive so a regression at the per-
    /// slice two-step short-circuit witnessing walk under negation
    /// fails at that primitive's tests rather than as silent drift at
    /// either struct-level arm.
    #[must_use]
    pub fn unique_missing_precondition_kind(&self) -> Option<ConditionKind> {
        self.preconditions.unique_missing_kind()
    }

    /// The SOLE [`ConditionKind::ALL`] variant ABSENT from
    /// [`Self::postconditions`], or `None` when postconditions are
    /// missing 0 or ≥ 2 kinds — the postcondition-side arm of the
    /// (precondition, postcondition, condition-union) exactly-one-
    /// missing witnessing triad on [`Boundary`]. Thin typed delegate
    /// to [`ConditionSliceExt::unique_missing_kind`] over
    /// [`Self::postconditions`].
    ///
    /// Peer of [`Self::unique_missing_precondition_kind`]. See that
    /// method for the full rationale — the two methods share ONE lift
    /// motivation, ONE fail-before-pass-after composition-law pin, and
    /// ONE two-surface parity contract with the ephemeral sugar type
    /// via
    /// [`crate::ephemeral::EphemeralSpec::unique_missing_postcondition_kind`].
    #[must_use]
    pub fn unique_missing_postcondition_kind(&self) -> Option<ConditionKind> {
        self.postconditions.unique_missing_kind()
    }
    ...
    /// `true` iff `preconditions ∪ postconditions` carries NO
    /// [`Condition`] with the given [`ConditionKind`] AND carries at
    /// least one [`Condition`] for every OTHER [`ConditionKind`] — the
    /// union arm of the (precondition, postcondition, condition-union)
    /// kind-scoped strict-refinement triad on [`Boundary`] specialized
    /// to the MISSING axis, byte-for-byte peer of the populated-axis
    /// [`Self::has_only_condition_kind`] under closed-set complement.
    ///
    /// # Composed body
    ///
    /// A FUSED short-circuit closed-set walk over
    ...
    /// widened composition
    /// `boundary.missing_condition_kinds() == vec![kind]` (which
    /// allocates the missing-kind Vec before the equality test) or
    /// the (pre AND post) AND-of-strict-refinement
    /// `boundary.preconditions.lacks_only_kind(kind)
    ///     && boundary.postconditions.lacks_only_kind(kind)` (which is
    /// TOO STRICT — a single-slice-populated arrangement whose empty
    /// side returns `false` fails this AND but IS well-formed on the
    /// union).
    ///
    /// # Peer on the ephemeral surface — [`crate::ephemeral::EphemeralSpec::lacks_only_condition_kind`]
    ///
    /// Byte-identical signature `(&Self, ConditionKind) -> bool`,
    /// byte-identical fused-closed-set-walk body under complement, on
    /// the ephemeral sugar surface whose pre/post condition vectors
    /// live inline. Both methods compose against the SAME slice-level
    /// substrate primitive [`ConditionSliceExt::lacks_only_kind`] via
    /// the two-slice union composed through
    /// [`Self::has_condition_kind`] — a regression at the per-slice
    ...
    /// closed-loop probe Process is missing ONLY the `ClosedLoopAuth`
    /// postcondition" reads
    /// `boundary.lacks_only_condition_kind(ConditionKind::ClosedLoopAuth)`
    /// at ONE call site rather than restating either widened
    /// composition. A `lacks-only-<kind>` require-tag classifier arm
    /// reaches this primitive at ONE substrate call — byte-for-byte
    /// peer of the tagged-union `lacks-only-<kind>` classifier one
    /// struct-layer up, CLOSING the kind-scoped strict-refinement 2x2
    /// grid on the Boundary struct layer alongside its populated-axis
    /// peer [`Self::has_only_condition_kind`].
    ///
    /// Theory anchor: THEORY.md §II.1 invariant 5 (composition
    /// preserves proofs — the kind-scoped strict-refinement projection
    ...
    /// boundary surface and the ephemeral surface). THEORY.md §VI.1
    /// (generation over composition — a new [`ConditionKind`] variant
    /// reaches both surfaces' kind-scoped strict-refinement-on-missing
    /// triads mechanically through the delegated union primitive).
    #[must_use]
    ...
    /// `true` iff [`Self::preconditions`] carries NO [`Condition`]
    /// with the given [`ConditionKind`] AND carries at least one
    /// [`Condition`] for every OTHER [`ConditionKind`] — the
    /// precondition-side arm of the (precondition, postcondition,
    /// condition-union) kind-scoped strict-refinement-on-missing triad
    /// on [`Boundary`]. Thin typed delegate to
    /// [`ConditionSliceExt::lacks_only_kind`] over
    /// [`Self::preconditions`].
    ///
    /// Peer of [`Self::lacks_only_postcondition_kind`] on the
    /// (precondition, postcondition) partition of the boundary's two
    /// condition-vector slots; both peers compose against the SAME
    /// slice-level substrate primitive so a regression at the per-
    ...
    /// [`Self::postconditions`].
    ///
    /// Peer of [`Self::lacks_only_precondition_kind`]. See that method
    /// for the full rationale — the two methods share ONE lift
    /// motivation, ONE fail-before-pass-after composition-law pin,
    /// and ONE two-surface parity contract with the ephemeral sugar
    /// type via
    /// [`crate::ephemeral::EphemeralSpec::lacks_only_postcondition_kind`].
    #[must_use]
    ...
    /// Returns the slice index of the FIRST [`Condition`] carrying
    /// the given [`ConditionKind`] on the boundary — the union arm
    /// of the (precondition, postcondition, condition-union)
    /// index-domain peer triad on [`Boundary`], sibling to
    /// [`Self::find_condition_kind`] one refinement across on the
    /// (`&Condition`, `usize`) witness axis.
    ///
    /// # Semantics — precondition takes precedence
    ///
    /// Composed body:
    ...
    /// `Some`, otherwise [`Self::postconditions`]). Callers that need
    /// the disambiguated per-side index reach for
    /// [`Self::position_of_precondition_kind`] or
    /// [`Self::position_of_postcondition_kind`] directly; callers that
    /// need the `&Condition` value at the same first-match position
    /// reach for [`Self::find_condition_kind`], which uses the SAME
    /// `or_else` composition on the value witness.
    ///
    /// # Compounding
    ///
    /// A future diagnostic pointer (a YAML path emission, an
    /// operator-facing "your first `ClosedLoopAuth` postcondition
    /// lives at index N of the boundary's post-slot") reaches this
    /// ONE primitive rather than restating either
    /// `find_condition_kind(k)` + a slice-scan for the reference, or
    /// two `.iter().position(|c| c.kind == K)` closure bodies at the
    /// callsite. Peer of [`Self::find_condition_kind`] on the value-
    /// vs-index witness axis; both compose against the SAME slice-
    /// level substrate primitive per half-arm.
    #[must_use]
    ...
    /// Returns the slice index of the FIRST [`Condition`] in
    /// [`Self::preconditions`] carrying the given [`ConditionKind`],
    /// or `None` — the precondition-side arm of the (precondition,
    /// postcondition, condition-union) index-domain peer triad on
    /// [`Boundary`]. Thin typed delegate to
    /// [`ConditionSliceExt::position_of_kind`] over
    /// [`Self::preconditions`].
    ///
    /// Peer of [`Self::position_of_postcondition_kind`] on the
    /// (precondition, postcondition) partition; both compose against
    /// the SAME slice-level substrate primitive and their `or_else`
    /// composition is [`Self::position_of_condition_kind`]. Byte-
    /// identical semantics to [`Self::find_precondition_kind`] with
    /// the `usize` index witness rather than the `&Condition` value
    /// witness.
    #[must_use]
    ...
    /// Returns the slice index of the FIRST [`Condition`] in
    /// [`Self::postconditions`] carrying the given [`ConditionKind`],
    /// or `None` — the postcondition-side arm of the (precondition,
    /// postcondition, condition-union) index-domain peer triad on
    /// [`Boundary`]. Thin typed delegate to
    /// [`ConditionSliceExt::position_of_kind`] over
    /// [`Self::postconditions`].
    ///
    /// Peer of [`Self::position_of_precondition_kind`]. See that
    /// method for the full rationale — the two methods share ONE
    /// lift motivation, ONE fail-before-pass-after composition-law
    /// pin, and ONE two-surface parity contract with the ephemeral
    /// sugar type via
    /// [`crate::ephemeral::EphemeralSpec::position_of_postcondition_kind`].
    #[must_use]
...
pub trait ConditionSliceExt {
    ...
    /// [`ConditionKind::ALL`] variant appears at least once in this slice
    /// (equivalently, [`Self::distinct_kinds`] is non-empty,
    /// [`Self::distinct_kind_count`] `> 0`, and
    /// [`Self::first_distinct_kind`] is `Some`).
    ///
    /// Default body: `ConditionKind::ALL.iter().copied().any(|k|
    /// self.has_kind(k))` — a SHORT-CIRCUITING closed-set walk that
    /// returns `true` at the FIRST populated kind WITHOUT materializing
    /// [`Self::distinct_kinds`]'s `Vec`, WITHOUT walking every slot to
    /// build [`Self::distinct_kind_count`]'s scalar, and WITHOUT
    /// allocating the closed-set-inversion scan. Strictly cheaper than
    /// either widened primitive on every non-empty arm because the walk
    /// short-circuits at the first `has_kind` hit rather than paying
    /// for the Vec allocation or the full cardinality count.
    ///
    ...
    /// answers "is ANY slot on the tagged-union parent occupied?",
    /// `has_any_distinct_kind` answers "does ANY kind appear in AT
    /// LEAST ONE condition of the slice?". Both compose against a
    /// SHORT-CIRCUITING closed-set walk under the SAME `has` /
    /// `has_kind` predicate at two adjacent typescape sites — the two
    /// primitives close the at-least-one halfspace on the closed-set-
    /// inversion axis at both struct layers under the SAME shape.
    ///
    /// # Sibling to [`Self::has_any_missing_kind`]
    ///
    /// Closed-set-inversion peer of the at-least-one halfspace on the
    /// closed-set-complement axis — where `has_any_missing_kind`
    /// returns `true` iff at least one kind is ABSENT,
    /// `has_any_distinct_kind` returns `true` iff at least one kind is
    /// PRESENT. Together with their zero-arm endpoints
    /// ([`Self::is_kind_saturated`] on the missing axis and the empty-
    /// slice endpoint on the distinct axis), the two Booleans partition
    /// the (distinct, missing) product: a slice is EMPTY iff neither
    /// `has_any_distinct_kind()` nor `is_kind_saturated()` returns
    /// `true`; a slice is SATURATED iff both `has_any_distinct_kind()`
    /// returns `true` and `has_any_missing_kind()` returns `false`; a
    /// slice is PARTIALLY POPULATED iff both `has_any_distinct_kind()`
    /// and `has_any_missing_kind()` return `true`.
    ///
    /// # Sibling to [`Self::distinct_kinds`] / [`Self::distinct_kind_count`]
    ///
    /// Boolean at-least-one halfspace peer of the widened + scalar
    /// closed-set-inversion primitives — where `distinct_kinds` returns
    /// the FULL distinct SET and `distinct_kind_count` returns its
    /// cardinality, `has_any_distinct_kind` collapses either the
    /// widened primitive to its non-emptiness Boolean or the scalar to
    /// its `>= 1` halfspace Boolean. The composition laws
    /// `has_any_distinct_kind() == !distinct_kinds().is_empty()` and
    /// `has_any_distinct_kind() == (distinct_kind_count() > 0)` bind
    /// this Boolean projection to the widened + scalar primitives at
    /// the trait's default body — strictly cheaper than either widened
    /// primitive on every non-empty arm because the walk short-circuits
    /// at the first populated kind on the has-side walk rather than
    /// allocating the closed-set-inversion scan or walking every slot
    /// to build the scalar cardinality.
    ///
    /// # Semantics
    ///
    /// An empty slice returns `false` — the SOLE arm on which
    /// `has_any_distinct_kind` returns `false`. A slice carrying any
    /// [`ConditionKind`] at least once returns `true` (a single-
    /// populated slice, a partially-populated slice, and a saturated
    /// slice all return `true`).
    ///
    /// # Compounding future consumers
    ///
    /// - A fleet-wide "any coverage at all" fast-path that discriminates
    ///   "the slice carries at least one closed-set kind" from "the
    ///   slice is empty" reads
    ///   `boundary.postconditions.has_any_distinct_kind()` at ONE call
    ///   site rather than restating `distinct_kind_count() > 0` (which
    ///   walks every slot to count) or `!distinct_kinds().is_empty()`
    ///   (which allocates the Vec before the negated emptiness check).
    /// - A `has-any-distinct-kind` require-tag classifier arm reaches
    ///   this primitive with no allocation, byte-for-byte peer of the
    ///   tagged-union `has-any-populated-kind` classifier one struct-
    ///   layer up under the SAME `any(has)` short-circuit shape.
    /// - A coherence check that flags "any process boundary whose
    ///   postcondition slice covers at least one [`ConditionKind`]"
    ///   reads `boundary.postconditions.has_any_distinct_kind()` at
    ///   ONE substrate primitive per test rather than restating the
    ///   `.iter().copied().any(|k| slice.has_kind(k))` body at every
    ///   callsite.
    ///
    /// # Theory grounding
    ///
    /// - THEORY.md §II.1 invariant 5 — composition preserves proofs.
    ///   The at-least-one halfspace projection on the closed-set-
    ///   inversion axis lives at ONE substrate site as a typed short-
    ///   circuiting closed-set walk `ConditionKind::ALL.iter().any(
    ///   has_kind)`. Every downstream consumer whose semantic reading
    ///   is "at least one kind is present" reads through this
    ///   primitive rather than paying for the widened primitive's Vec
    ///   allocation.
    /// - THEORY.md §VI.1 — generation over composition. A new
    ///   [`ConditionKind`] variant added to `ALL` reaches this
    ///   primitive mechanically through the `any` short-circuit — an
    ///   empty slice (returning `false` here) that later picks up the
    ...
    /// Witnessing `Option<ConditionKind>` peer of
    /// [`Self::has_unique_distinct_kind`] on the closed-set-inversion
    /// axis — `Some(k)` iff `k` is the SOLE [`ConditionKind::ALL`]
    /// variant appearing at least once in this slice, else `None`.
    ///
    /// Default body: a two-step-short-circuit fold through
    /// [`Self::iter_distinct_kinds`] — pull the first hit; return
    /// `Some(first)` iff the second hit is [`None`], else `None`.
    /// Byte-for-byte peer of
    /// [`crate::tagged_union::TaggedUnion::unique_populated_kind`]
    /// one struct-layer up under the SAME `iter_populated_kinds`
    /// two-step short-circuit shape, and the WITNESSING scalar peer of
    /// the Boolean [`Self::has_unique_distinct_kind`] predicate at the
    /// SAME two-step short-circuit shape.
    ///
    /// # Sibling to [`Self::first_distinct_kind`] / [`Self::last_distinct_kind`]
    ///
    /// FIFTH refinement on the closed-set-inversion axis under exactly-
    /// one-hit semantics, `Option<ConditionKind>`-valued: together with
    /// [`Self::first_distinct_kind`] and [`Self::last_distinct_kind`]
    /// the three primitives project [`Self::distinct_kinds`] onto its
    /// cardinality-conditioned scalar identity on the present side. The
    /// composition laws
    /// `unique_distinct_kind().is_some() == (distinct_kind_count() == 1)`
    /// and (on the `Some` arm) `unique_distinct_kind() ==
    /// first_distinct_kind() == last_distinct_kind()` bind the exactly-
    /// one scalar identity to the widened primitives at the trait's
    /// default body.
    ///
    /// # Peer to [`Self::unique_missing_kind`]
    ///
    /// Closed-set-inversion peer of the exactly-one-hit scalar on the
    /// closed-set-complement axis — where `unique_missing_kind` names
    /// the SOLE ABSENT kind, `unique_distinct_kind` names the SOLE
    /// PRESENT kind. The two primitives close the (present, absent) x
    /// (endpoint, exactly-one) 2x3 scalar-Option grid on the slice
    /// level under the SAME two-step short-circuit shape via the
    /// load-bearing iterator peers.
    ///
    /// # Semantics
    ///
    /// An empty slice returns `None` (0 distinct, not 1). A slice
    /// carrying a single [`ConditionKind`] (with any multiplicity)
    /// returns `Some(k)` — the SOLE arm where `unique_distinct_kind`
    /// returns `Some` on any `N ≥ 2` closed set. A slice carrying `K ≥ 2`
    /// distinct kinds returns `None`. A saturated slice returns `None`
    /// on `N ≥ 2` closed sets.
    ///
    /// # Compounding future consumers
    ///
    /// - A future singleton-coverage diagnostic that prints "the SOLE
    ///   [`ConditionKind`] covered by this Boundary is X" reads
    ///   `boundary.postconditions.unique_distinct_kind()` at ONE call
    ///   site — the WITNESS + the exactly-one predicate composed at
    ///   ONE short-circuit walk, rather than pairing the Boolean
    ///   [`Self::has_unique_distinct_kind`] with [`Self::first_distinct_kind`]
    ///   at TWO independent walks whose agreement is a coincidence.
    /// - A `unique-distinct-<kind>` require-tag classifier arm reads
    ///   this primitive with no allocation, byte-for-byte symmetrical
    ///   with `slice.unique_missing_kind()`.
    /// - A fast-path branch that discriminates "exactly one kind
    ///   covered" from "0 or ≥ 2 covered" reads
    ///   `slice.unique_distinct_kind().is_some()` at ONE call site.
    ///
    /// # Theory grounding
    ///
    /// - THEORY.md §II.1 invariant 5 — composition preserves proofs.
    ///   The exactly-one-hit witnessing projection on the closed-set-
    ///   inversion axis lives at ONE substrate site as a typed two-
    ///   step-short-circuit fold through the load-bearing
    ///   [`Self::iter_distinct_kinds`] iterator — byte-for-byte peer of
    ///   the tagged-union [`crate::tagged_union::TaggedUnion::unique_populated_kind`]
    ///   under the SAME iterator shape.
    /// - THEORY.md §VI.1 — generation over composition. A new
    ///   [`ConditionKind`] variant added to `ALL` reaches this
    ///   primitive mechanically through the short-circuit walk — a
    ///   slice previously at the singleton-coverage arm (returned
    ///   `Some(k)` here) that also picks up the new variant now has
    ///   TWO distinct kinds and returns `None`.
    ...
    /// appear at least once in this slice (equivalently,
    /// [`Self::distinct_kind_count`] `>= 2` and
    /// [`Self::distinct_kinds`]`.len() >= 2`).
    ///
    /// Default body: a two-step-short-circuit closed-set walk over
    /// [`Self::iter_distinct_kinds`] — pulls up to two hits off the
    /// load-bearing distinct iterator; the primitive returns `true`
    /// iff BOTH the first and the second are [`Some`], WITHOUT
    /// materializing [`Self::distinct_kinds`]'s `Vec` and WITHOUT
    /// walking every slot to build [`Self::distinct_kind_count`]'s
    /// scalar. Short-circuits at the second distinct kind — strictly
    /// cheaper than either widened primitive on every arm with `≥ 2`
    /// distinct kinds. Byte-for-byte peer of
    /// [`crate::tagged_union::TaggedUnion::has_multiple_populated_kinds`]
    /// under the (populated, missing) inversion axis one struct-layer
    /// up.
    ///
    ...
    /// occupied?", `has_multiple_distinct_kinds` answers "do AT LEAST
    /// TWO kinds appear in AT LEAST ONE condition of the slice?". Both
    /// compose against a two-step-short-circuit closed-set walk under
    /// a presence predicate (`has(kind)` / `has_kind(kind)`) at two
    /// adjacent typescape sites — the two primitives close the at-
    /// least-two arm on the closed-set-inversion axis at both struct
    ...
    /// on the distinct axis at the slice level, closing the natural
    /// partition alongside the zero-arm (the empty-distinct endpoint,
    /// reached via `!has_any_distinct_kind()`) and
    /// [`Self::has_unique_distinct_kind`] (one-arm). Every slice
    /// satisfies EXACTLY ONE of the three Boolean projections — the
    /// three primitives partition `0..=ConditionKind::ALL.len()` at 0,
    /// 1, and ≥ 2 respectively. The composition law
    ...
    /// [`assert_slice_refinement_composition_laws`] as its
    /// distinct-cardinality-many-arm arm.
    ///
    /// # Semantics
    ///
    /// An empty slice returns `false` (0 distinct, not ≥ 2). A slice
    /// carrying a single [`ConditionKind`] (with any multiplicity)
    /// returns `false` (1 distinct, not ≥ 2). A slice carrying `K ≥ 2`
    /// distinct kinds returns `true`. A saturated slice returns `true`
    /// on any `N ≥ 2` closed set (every kind present, ≥ 2 ≥ 2).
    ///
    /// # Compounding future consumers
    ///
    /// - An operator-facing "≥ 2 dependencies covered" fast-path
    ///   discriminator on the many-distinct arm reads
    ///   `boundary.postconditions.has_multiple_distinct_kinds()` at
    ///   ONE call site — one two-step short-circuit walk, no
    ///   allocation, no scalar comparison against `>= 2`, byte-for-
    ///   byte peer of the tagged-union `has-multiple-populated-kinds`
    ///   classifier one struct-layer up under the SAME two-step
    ///   short-circuit shape.
    /// - A `has-multiple-distinct-kinds` require-tag classifier arm
    ///   reaches this primitive with no allocation, byte-for-byte
    ///   peer of the tagged-union `has-multiple-populated-kinds`
    ///   classifier one struct-layer up.
    /// - A future multi-coverage diagnostic that prints "≥ 2 distinct
    ///   ConditionKinds covered by this Boundary" reads
    ///   `has_multiple_distinct_kinds()` at ONE call site without
    ///   allocating [`Self::distinct_kinds`]'s `Vec`.
    ///
    /// # Theory grounding
    ///
    /// - THEORY.md §II.1 invariant 5 — composition preserves proofs.
    ///   The cardinality-many-arm projection on the distinct axis
    ///   lives at ONE substrate site as a typed two-step-short-circuit
    ///   fold through the load-bearing [`Self::iter_distinct_kinds`]
    ///   iterator — byte-for-byte peer of `distinct_kind_count()`
    ///   composed against `>= 2`, but with a second-distinct-slot
    ///   short-circuit that the scalar counter primitive does not
    ///   offer.
    /// - THEORY.md §VI.1 — generation over composition. A new
    ///   [`ConditionKind`] variant added to `ALL` reaches this
    ///   primitive mechanically through the short-circuit walk — a
    ///   slice previously at the singleton-coverage arm (returned
    ///   `false` here) that also picks up the new variant now has TWO
    ///   distinct kinds and flips to `true`.
    ...
    /// Boolean cardinality "≤ 1" negation peer of
    /// [`Self::has_multiple_distinct_kinds`] on the closed-set-inversion
    /// axis — `true` iff AT MOST ONE [`ConditionKind::ALL`] variant
    /// appears at least once in this slice (equivalently,
    /// [`Self::distinct_kind_count`] `<= 1` and
    /// [`Self::distinct_kinds`]`.len() <= 1`). Closes the {0, 1, ≥2, ≤1}
    /// Boolean-negation grid on the distinct axis at the slice level
    /// alongside its sibling `has_multiple_distinct_kinds` (≥ 2 many-
    /// arm) under the definitional negation `!(≥ 2) == (≤ 1)`, and
    /// alongside `!has_any_distinct_kind` (=0 empty-endpoint) OR
    /// `has_unique_distinct_kind` (=1 mid-endpoint) as the trichotomy-
    /// union arm. Names the arrangement space where the slice is
    /// EMPTY-OR-SINGLETON (zero or exactly one distinct kind).
    ///
    /// Default body: `!self.has_multiple_distinct_kinds()` — a
    /// definitional Boolean negation of the many-arm primitive. Short-
    /// circuits transitively through
    /// [`Self::has_multiple_distinct_kinds`]'s two-step short-circuit
    /// walk over [`Self::iter_distinct_kinds`]: returns `true` as soon
    /// as the many-arm walk stops with fewer than two distinct hits,
    /// WITHOUT materializing [`Self::distinct_kinds`]'s `Vec` and
    /// WITHOUT walking every slot to build
    /// [`Self::distinct_kind_count`]'s scalar. Byte-for-byte peer of
    ...
    /// [`crate::tagged_union::TaggedUnion::has_at_most_one_populated_kind`]
    /// answers "does the tagged-union parent have AT MOST ONE occupied
    /// slot?", `has_at_most_one_distinct_kind` answers "do AT MOST ONE
    /// kind appear in AT LEAST ONE condition of the slice?". Both
    /// compose as the definitional Boolean negation of their many-arm
    /// primitive (`!has_multiple_populated_kinds()` /
    /// `!has_multiple_distinct_kinds()`) at two adjacent typescape
    /// sites — the two primitives close the "≤ 1" arm on the closed-
    /// set-inversion axis at both struct layers under the SAME shape.
    ///
    /// # Peer to [`Self::has_at_most_one_missing_kind`]
    ///
    /// Axis-parity mirror of the closed-set-complement "≤ 1" negation
    /// peer at the slice level — where `has_at_most_one_missing_kind`
    /// tests "at most one kind MISSING" (near-saturation-or-saturated),
    /// `has_at_most_one_distinct_kind` tests "at most one kind
    /// PRESENT" (empty-or-singleton). Both compose the same
    /// definitional negation shape (`!has_multiple_*_kinds()`) at the
    /// slice-level trait's default body — the two primitives close the
    /// "≤ 1" arm on both the closed-set-complement and closed-set-
    /// inversion axes at the SAME struct layer under the SAME shape.
    ///
    /// # Sibling to the Boolean distinct-cardinality tetrachotomy
    ///
    /// Fourth arm of the `{0, ≥1, 1, ≥2, ≤1}` Boolean-cardinality
    /// closure on the distinct axis at the slice level, closing the
    /// Boolean-negation grid alongside `!has_any_distinct_kind` (=0
    /// zero-arm reached via [`Self::has_any_distinct_kind`]),
    /// [`Self::has_unique_distinct_kind`] (=1 mid-endpoint),
    /// [`Self::has_any_distinct_kind`] (≥1 halfspace), and
    /// [`Self::has_multiple_distinct_kinds`] (≥2 many-arm). The
    /// {≤1, ≥2} pair sit on the Boolean-negation axis:
    /// `has_at_most_one_distinct_kind == !has_multiple_distinct_kinds`
    /// on every arm. The {0, 1} union arm sits on the trichotomy-union
    /// axis: `has_at_most_one_distinct_kind == !has_any_distinct_kind
    /// || has_unique_distinct_kind` on every arm. Both composition laws
    /// bind the "≤ 1" Boolean projection to the sibling primitives at
    /// the trait's default body — swept substrate-wide by
    /// [`assert_slice_refinement_composition_laws`] as its distinct-
    /// axis "≤ 1" arm.
    ///
    /// # Semantics
    ///
    /// An empty slice returns `true` (0 distinct, `≤ 1`) — the dual of
    /// the empty-slice arm on [`Self::has_at_most_one_missing_kind`]
    /// which returns `false` on `N ≥ 2` closed sets (empty means every
    /// kind missing, `N ≥ 2` missing, not `≤ 1`). A slice carrying a
    /// single [`ConditionKind`] (with any multiplicity) returns `true`
    /// (1 distinct, `≤ 1`) — the singleton arm on the distinct axis.
    /// A slice carrying `K ≥ 2` distinct kinds returns `false`. A
    /// saturated slice returns `false` on any `N ≥ 2` closed set
    /// (every kind present, `N ≥ 2` distinct, not `≤ 1`) — the union of
    /// the two "≤ 1" arms (`=0` and `=1`) is exactly the arrangement
    /// space where the primitive returns `true`.
    ///
    /// # Compounding future consumers
    ///
    /// - An operator-facing "at most one dependency currently covered"
    ///   fast-path discriminator on the empty / singleton-coverage arms
    ///   reads `boundary.postconditions.has_at_most_one_distinct_kind()`
    ...
    ///   `has-at-most-one-populated-kind` classifier one struct-layer
    ///   up under the SAME `!has_multiple_populated_kinds` definitional
    ///   negation shape.
    /// - A `has-at-most-one-distinct-kind` require-tag classifier arm
    ///   reaches this primitive with no allocation, closing the
    ///   {0, 1, ≥ 2, ≤ 1} cardinality-Boolean grid on the distinct axis
    ///   at the slice level alongside its sibling
    ///   `has-multiple-distinct-kinds` under the Boolean negation axis
    ///   and the missing-axis peer `has-at-most-one-missing-kind` under
    ///   the closed-set-inversion axis.
    /// - A future under-coverage diagnostic that says "at most one
    ///   ConditionKind covered by this Boundary" reads
    ///   `has_at_most_one_distinct_kind()` at ONE call site without
    ///   allocating [`Self::distinct_kinds`]'s `Vec`.
    ///
    /// # Theory grounding
    ///
    /// - THEORY.md §II.1 invariant 5 — composition preserves proofs.
    ///   The cardinality "≤ 1" projection on the distinct axis lives
    ///   at ONE substrate site as the definitional Boolean negation
    ///   of [`Self::has_multiple_distinct_kinds`]; the three composition
    ///   forms (`!has_multiple_distinct_kinds()`,
    ///   `distinct_kind_count() <= 1`, and `!has_any_distinct_kind() ||
    ///   has_unique_distinct_kind()`) compose through the SAME two-
    ///   step-short-circuit walk shape one negation up, byte-for-byte
    ///   identical on every arm.
    /// - THEORY.md §VI.1 — generation over composition. A new
    ///   [`ConditionKind`] variant added to `ALL` reaches this
    ///   primitive mechanically through the delegated
    ///   [`Self::has_multiple_distinct_kinds`] — a slice previously at
    ///   the singleton arm (returned `true` here) that also picks up
    ///   the new variant now has TWO distinct kinds and flips to
    ///   `false`.
    fn has_at_most_one_distinct_kind(&self) -> bool {
        !self.has_multiple_distinct_kinds()
    }
    ...
    /// axis — `Some(k)` iff `k` is the SOLE [`ConditionKind::ALL`]
    /// variant ABSENT from this slice, else `None`.
    ///
    /// Default body: a two-step-short-circuit fold through
    /// [`Self::iter_missing_kinds`] — pull the first hit; return
    /// `Some(first)` iff the second hit is [`None`], else `None`.
    /// Byte-for-byte peer of
    /// [`crate::tagged_union::TaggedUnion::unique_missing_kind`] one
    /// struct-layer up under the SAME `iter_missing_kinds` two-step
    /// short-circuit shape, and the WITNESSING scalar peer of the
    /// Boolean [`Self::has_unique_missing_kind`] predicate at the SAME
    /// two-step short-circuit shape.
    ///
    /// # Sibling to [`Self::first_missing_kind`] / [`Self::last_missing_kind`]
    ///
    /// FIFTH refinement on the closed-set-complement axis under
    /// exactly-one-hit semantics, `Option<ConditionKind>`-valued:
    /// together with [`Self::first_missing_kind`] and
    /// [`Self::last_missing_kind`] the three primitives project
    /// [`Self::missing_kinds`] onto its cardinality-conditioned scalar
    /// identity on the absent side. The composition laws
    ...
    ///
    /// Closed-set-complement peer of the exactly-one-hit scalar on the
    /// closed-set-inversion axis — where `unique_distinct_kind` names
    /// the SOLE PRESENT kind, `unique_missing_kind` names the SOLE
    /// ABSENT kind. The two primitives close the (present, absent) x
    /// (endpoint, exactly-one) 2x3 scalar-Option grid on the slice
    /// level under the SAME two-step short-circuit shape via the
    /// load-bearing iterator peers on the two opposite sides.
    ///
    /// # Semantics
    ///
    /// An empty slice returns `None` on any `N ≥ 2` closed set (every
    /// kind is missing — the fully-missing endpoint, `N` missing not
    /// `1`). A slice at the near-saturation arm (carrying every kind
    /// except exactly one) returns `Some(the-lone-empty)` — the SOLE
    /// arm where `unique_missing_kind` returns `Some` on any `N ≥ 3`
    /// closed set. A saturated slice returns `None` (zero missing).
    ///
    /// # Compounding future consumers
    ///
    /// - An operator-facing "one dependency still unfulfilled: X"
    ///   diagnostic on the near-saturation arm reads
    ///   `boundary.postconditions.unique_missing_kind()` at ONE call
    ///   site — the WITNESS + the exactly-one predicate composed at
    ///   ONE short-circuit walk, rather than pairing the Boolean
    ///   [`Self::has_unique_missing_kind`] with [`Self::first_missing_kind`]
    ///   at TWO independent walks whose agreement is a coincidence.
    /// - A `unique-missing-<kind>` require-tag classifier arm reads
    ///   this primitive with no allocation, byte-for-byte symmetrical
    ///   with `slice.unique_distinct_kind()`.
    /// - A fast-path branch that discriminates "exactly one kind
    ///   still missing" from "0 or ≥ 2 still missing" reads
    ///   `slice.unique_missing_kind().is_some()` at ONE call site.
    ///
    /// # Theory grounding
    ///
    /// - THEORY.md §II.1 invariant 5 — composition preserves proofs.
    ///   The complement-exactly-one-hit witnessing projection lives at
    ///   ONE substrate site as a typed two-step-short-circuit fold
    ///   through the load-bearing [`Self::iter_missing_kinds`] iterator
    ///   — byte-for-byte peer of the tagged-union
    ///   [`crate::tagged_union::TaggedUnion::unique_missing_kind`]
    ///   under the SAME iterator shape.
    /// - THEORY.md §VI.1 — generation over composition. A new
    ///   [`ConditionKind`] variant added to `ALL` reaches this
    ///   primitive mechanically on the missing side.
    ...
    /// Kind-scoped strict refinement of [`Self::lacks_kind`] — `true` iff
    /// the given `kind` does NOT appear in the slice AND every OTHER
    /// [`ConditionKind`] DOES appear at least once. The "exactly this
    /// one variant is the sole hole" predicate at the slice level.
    ///
    /// Default body: a FUSED short-circuit closed-set walk over
    /// [`ConditionKind::ALL`] under [`Self::has_kind`] that skips every
    /// populated slot, returns `false` at the EARLIEST missing slot
    /// whose kind is NOT `kind`, and returns `true` iff the sweep
    /// completes with `kind` seen as the sole missing slot. Byte-for-
    /// byte cheaper than either widened composition
    /// `self.missing_kinds() == vec![kind]` (which allocates the
    /// missing-kind Vec before the equality test) or
    /// `self.lacks_kind(kind) && self.missing_kind_count() == 1` (which
    /// walks the closed-set-complement scan twice) on every arm where
    /// the slice carries a missing kind that isn't `kind`.
    ///
    /// # Peer to [`crate::tagged_union::TaggedUnion::lacks_only`]
    ///
    /// Slice-level peer of the tagged-union parent-level kind-scoped
    /// strict-refinement predicate on the missing axis one struct-layer
    /// up: where
    /// [`crate::tagged_union::TaggedUnion::lacks_only`] answers "is THIS
    /// kind's slot on the tagged-union parent the sole empty slot?",
    /// `lacks_only_kind` answers "is THIS kind the sole missing kind
    /// from the slice's distinct set?". Both primitives compose the
    /// SAME fused short-circuit closed-set walk under a per-kind
    /// [`Self::has_kind`] / `TaggedUnion::has` predicate at two adjacent
    /// typescape sites — the two primitives close the kind-scoped
    /// strict-refinement invariant on the near-saturation-diagonal
    /// (`N-1`-of-N populated with the sole hole at `kind`) arm at both
    /// struct layers under the SAME shape.
    ///
    /// # Sibling to [`Self::has_only_kind`]
    ///
    /// Closed-set-complement mirror of the well-formed-diagonal
    /// strict-refinement primitive on the populated axis — where
    /// `has_only_kind(k)` returns `true` iff `k` is the sole distinct
    /// populated kind, `lacks_only_kind(k)` returns `true` iff `k` is
    /// the sole missing kind. Together the two peers CLOSE the
    /// (populated, missing) × (subset, equal) 2x2 kind-scoped
    /// strict-refinement grid at the slice level alongside `has_kind`
    /// (populated subset) and `lacks_kind` (missing subset).
    ///
    ...
    /// and a fixed argument `kind`:
    ///
    /// - Empty slice (0 conditions, distinct-kind set empty,
    ///   missing-kind set == ALL): `false` on any `N ≥ 2` — every kind
    ///   is missing, so `kind` is NOT the sole missing kind.
    /// - Single-populated slice with populated kind `p` (1 condition,
    ///   missing-kind set == `ALL \ {p}`): `false` on any `N ≥ 3`
    ///   (`N - 1 ≥ 2` missing kinds, no sole missing kind); on `N == 2`
    ///   the missing set is `{q}` where `q ≠ p`, so
    ///   `lacks_only_kind(kind) == (kind == q)`.
    /// - Near-saturation slice with populated kinds `ALL \ {q}` (each
    ///   kind except `q` populated, missing set `{q}`): the SOLE `true`
    ///   arm — `lacks_only_kind(kind) == (kind == q)`.
    /// - Saturated slice (every kind appears): `false` on every kind —
    ///   no kind is missing, so no kind is the sole missing kind.
    /// - Multiplicity is ignored on the populated side: a slice
    ///   carrying `k` at every position still has an empty missing set,
    ///   or a missing set `{k'}` where `k' ≠ k`, byte-for-byte with
    ///   the single-populated arrangement.
    ///
    /// # Kind-domain exhaustivity
    ///
    /// A slice satisfies `lacks_only_kind(k)` for AT MOST one `k`,
    /// since two distinct kinds cannot both be the sole missing kind.
    /// On the near-saturation arm the count is exactly 1 (the sole
    /// missing kind); on every other arm the count is 0. This
    /// kind-domain exhaustivity law binds the argument-scoped
    /// projection to the parent-scoped cardinality primitive
    /// `missing_kind_count() == 1` at the composition-law surface.
    ///
    /// # Compounding future consumers
    ///
    /// - A `lacks-only-<kind>` require-tag classifier arm reaches this
    ///   primitive with no allocation, byte-for-byte peer of the
    ///   tagged-union `lacks-only-<kind>` classifier one struct-layer
    ///   up under the SAME fused short-circuit walk shape.
    /// - A "one dependency short: <kind>" diagnostic on the aggregate
    ///   boundary check reads
    ///   `slice.lacks_only_kind(k)` at ONE call site — one fused
    ///   short-circuit walk, no allocation, strictly cheaper than
    ///   `slice.first_missing_kind() == Some(k) && slice.missing_kind_count() == 1`
    ///   which walks the closed-set-complement scan twice.
    /// - A coherence check that verifies "the near-saturation slice
    ///   from an `all_but_one_kind_of(k)` factory is unambiguously
    ///   missing kind `k`" reads `slice.lacks_only_kind(k)` at ONE
    ///   site — the strongest structural pin on the missing-side
    ///   well-formed diagonal.
    ///
    /// # Theory grounding
    ///
    /// - THEORY.md §II.1 invariant 5 — composition preserves proofs.
    ...
    ///   walk over [`ConditionKind::ALL`] under [`Self::has_kind`]
    ///   with early exit on the first missing slot whose kind is not
    ///   `kind` — byte-for-byte peer of [`Self::has_only_kind`]'s
    ///   fused walk under complement, semantically identical to
    ///   `first_missing_kind() == Some(kind) && missing_kind_count() == 1`
    ///   on every arm.
    /// - THEORY.md §VI.1 — generation over composition. A new
    ///   [`ConditionKind`] variant added to `ALL` reaches this
    ///   primitive mechanically through the delegated `has_kind` walk
    ///   — every downstream `lacks-only-<kind>` classifier arm sees
    ///   the wider kind set without further per-caller edit.
    ...
    /// Returns the slice index of the FIRST [`Condition`] carrying
    /// the given [`ConditionKind`], or `None` if none matches — the
    /// index-domain peer of [`Self::find_kind`] on the per-kind axis.
    /// Default body: `self.iter().position(|c| c.kind == kind)` —
    /// a byte-identical delegate to [`std::iter::Iterator::position`]
    /// on the wrapped slice iterator.
    ///
    /// # Sibling to [`Self::find_kind`]
    ///
    /// Same axis, ORTHOGONAL return: `find_kind(k)` returns the
    /// matching `&Condition` (the *value* witness), `position_of_kind`
    /// returns the matching slice `usize` (the *index* witness). The
    /// two peers close the (value-witness, index-witness) refinement
    /// grid on the per-kind first-match axis — a caller that needs
    /// the parameter map ([`Condition::params`]) reaches `find_kind`,
    /// a caller that needs the slice index (for context messages, for
    /// pinning a positional invariant, for slicing before / after the
    /// hit without re-walking) reaches this method. Pinned by the
    /// composition law `slice.position_of_kind(k).map(|i| &slice[i])
    /// == slice.find_kind(k)` at the substrate testkit — the two
    /// methods walk the SAME slice in the SAME order, so a
    /// regression that drifted one from the other becomes
    /// structurally impossible past the trait boundary. Byte-for-byte
    /// equivalent to `self.iter().position(|c| c.kind == kind)`.
    ///
    /// # Semantics
    ///
    /// Returns `Some(i)` where `i` is the slice position of the
    /// earliest match, or `None` if no [`Condition`] carries `kind`.
    /// A slice with multiple matches yields the earliest position
    /// (the composition law
    /// `position_of_kind(k) == iter_kind(k).next().map(|c| slice.iter()
    /// .position(|x| std::ptr::eq(x, c)).unwrap())` binds the index
    /// witness to the first-match value witness on every arm).
    ///
    /// # Compounding
    ///
    /// A future diagnostic that surfaces "the first `ClosedLoopAuth`
    /// postcondition lives at position N" (an operator-facing YAML
    /// pointer, a JSON-schema-path emission, an editor-line marker
    /// for a fleet-wide audit hit) reaches this ONE primitive
    /// through `slice.position_of_kind(k)` rather than restating
    /// `slice.iter().position(|c| c.kind == k)` at the callsite.
    /// A future coherence check that pins "the first `ProcessPhase`
    /// precondition MUST precede the first `PromQL` precondition"
    /// reads two `slice.position_of_kind(_)` calls with an `<`
    /// composition — no re-walk, no `.iter().position()` restatement,
    /// no cross-position invariant hidden inside an ad-hoc closure.
    /// The per-kind first-match axis now carries two refinements
    /// (`&Condition` via [`Self::find_kind`], `usize` via this
    /// method) at ONE typed algebra surface, so a future normalization
    /// of either witness (a widened return carrying a `Condition &
    /// index` pair, a debug-build assertion on duplicate positions,
    /// a fleet-wide warn on out-of-order kinds) lands at ONE site
    /// per witness and every downstream caller picks it up
    /// mechanically.
    ///
    /// # Theory grounding
    ///
    /// - THEORY.md §II.1 invariant 5 — composition preserves proofs.
    ///   The index witness lives at ONE substrate site as
    ///   `self.iter().position(|c| c.kind == kind)`; the composition
    ///   law `position_of_kind(k).map(|i| &slice[i]) == find_kind(k)`
    ///   binds it to the value-witness peer at the trait's default
    ///   body.
    /// - THEORY.md §VI.1 — generation over composition. A new
    ///   [`ConditionKind`] variant reaches this primitive mechanically
    ///   through the delegated `.iter().position` walk — every
    ///   downstream `first-position-of-<kind>` callsite sees the
    ///   wider closed set with no per-caller edit.
    fn position_of_kind(&self, kind: ConditionKind) -> Option<usize>;
}
...
impl ConditionSliceExt for [Condition] {
    ...
    fn position_of_kind(&self, kind: ConditionKind) -> Option<usize> {
        self.iter().position(|c| c.kind == kind)
    }
}
    ...
    S: ConditionSliceExt + ?Sized,
{
    ...
    // has_any_distinct_kind ↔ (distinct_kind_count > 0) — the Boolean
    // at-least-one halfspace projection of the closed-set-inversion
    // scalar cardinality. Peer of `has_any_missing_kind ↔
    // !is_kind_saturated` on the axis-parity axis: where the at-least-
    // one halfspace peer on the closed-set-complement axis tests the
    // ≥ 1 arm on the missing scalar, this at-least-one halfspace peer
    // on the closed-set-inversion axis tests the ≥ 1 arm on the
    // distinct scalar. A regression that overrode `has_any_distinct_kind`
    // to drop the short-circuit, skip a kind, or drift the walk from
    // `ConditionKind::ALL` surfaces HERE at the substrate boundary, not
    // as silent drift at every downstream `has-any-distinct-kind`
    // require-tag classifier or fleet-wide coverage-analysis dashboard
    // callsite. Byte-for-byte peer of
    // `crate::tagged_union::TaggedUnion::has_any_populated_kind` one
    // struct-layer up under the SAME `any(has)` short-circuit shape.
    // Also pins the widened composition law `has_any_distinct_kind() ==
    // !distinct_kinds().is_empty()` at every slice — binds the at-
    // least-one halfspace Boolean projection to the widened primitive
    // without paying for the Vec allocation.
    assert_eq!(
        slice.has_any_distinct_kind(),
        slice.distinct_kind_count() > 0,
        "has_any_distinct_kind() drifted from (distinct_kind_count() > 0)",
    );
    assert_eq!(
        slice.has_any_distinct_kind(),
        !distinct.is_empty(),
        "has_any_distinct_kind() drifted from !distinct_kinds().is_empty()",
    );
    assert_eq!(
        slice.has_any_distinct_kind(),
        slice.first_distinct_kind().is_some(),
    ...
    // has_multiple_distinct_kinds ↔ (distinct_kind_count >= 2) — the
    // Boolean cardinality many-arm projection of the closed-set-
    // inversion scalar cardinality. Peer of `has_any_distinct_kind ↔
    // !distinct_kinds.is_empty()` (≥ 1 halfspace) and
    // `has_unique_distinct_kind ↔ (distinct_kind_count == 1)` (= 1
    // mid-endpoint) on the Boolean-projection axis: where those peers
    // test the ≥ 1 and = 1 arms on the distinct scalar, this many-arm
    // peer tests the ≥ 2 arm. Together with the empty-endpoint
    // (`!has_any_distinct_kind`) and `has_unique_distinct_kind`, the
    // three Booleans partition the distinct-cardinality closed set at
    // 0, 1, and ≥ 2 respectively — every slice satisfies EXACTLY ONE
    // of the three projections. A regression that overrode
    // `has_multiple_distinct_kinds` to drop the second-slot short-
    // circuit (returning `true` on any ≥ 1-distinct arm), skip a
    // kind, drift the walk from `ConditionKind::ALL`, or conflate
    // with `has_any_distinct_kind` (the ≥ 1 halfspace) surfaces HERE
    // at the substrate boundary, not as silent drift at every
    // downstream `has-multiple-distinct-kinds` require-tag classifier
    // or multi-coverage diagnostic callsite. Byte-for-byte peer of
    // `crate::tagged_union::TaggedUnion::has_multiple_populated_kinds`
    // one struct-layer up under the SAME two-step short-circuit walk
    // shape. Also pins the widened composition law
    // `has_multiple_distinct_kinds() == (distinct_kinds().len() >= 2)`
    // at every slice — binds the cardinality-many-arm Boolean
    // projection to the widened + scalar closed-set-inversion
    // primitives without paying for the Vec allocation on the ≥ 2-
    // distinct arms (where the short-circuit fires) or the full-slot
    // walk on the scalar counter.
    assert_eq!(
        slice.has_multiple_distinct_kinds(),
        slice.distinct_kind_count() >= 2,
        "has_multiple_distinct_kinds() drifted from (distinct_kind_count() >= 2)",
    );
    assert_eq!(
        slice.has_multiple_distinct_kinds(),
        distinct.len() >= 2,
        "has_multiple_distinct_kinds() drifted from (distinct_kinds().len() >= 2)",
    );

    // has_at_most_one_distinct_kind ↔ !has_multiple_distinct_kinds —
    // the Boolean cardinality "≤ 1" negation projection of the many-
    // arm primitive on the closed-set-inversion axis. Peer of
    // `has_multiple_distinct_kinds ↔ (distinct_kind_count >= 2)` (≥ 2
    // many-arm) under the definitional Boolean negation
    // `!(≥ 2) == (≤ 1)`. Together with `!has_any_distinct_kind` (=0
    // empty-endpoint reached via `has_any_distinct_kind`) and
    // `has_unique_distinct_kind` (=1 mid-endpoint), the "≤ 1"
    // primitive collapses to the trichotomy-union
    // `!has_any_distinct_kind() || has_unique_distinct_kind()` — a
    // regression that overrode `has_at_most_one_distinct_kind` to drop
    // the definitional negation (returning `has_multiple_distinct_kinds`
    ...
    // `has-at-most-one-distinct-kind` require-tag classifier or
    // empty-or-singleton coverage-analysis diagnostic callsite. Byte-
    // for-byte peer of
    // `crate::tagged_union::TaggedUnion::has_at_most_one_populated_kind`
    // one struct-layer up under the SAME `!has_multiple_populated_kinds`
    // definitional negation shape. Axis-parity mirror of the closed-
    // set-complement "≤ 1" negation peer
    // `has_at_most_one_missing_kind ↔ !has_multiple_missing_kinds` at
    // the same slice-level trait. Also pins the widened composition
    // laws
    // `has_at_most_one_distinct_kind() == (distinct_kind_count() <= 1)`
    // and `has_at_most_one_distinct_kind() == (distinct_kinds().len() <= 1)`
    // at every slice — binds the "≤ 1" Boolean projection to the
    // widened + scalar closed-set-inversion primitives without paying
    // for the Vec allocation on the ≤ 1-distinct arms (where the
    ...
    // `has_at_most_one_distinct_kind() == !has_any_distinct_kind() ||
    // has_unique_distinct_kind()` at every slice — surfaces any
    ...
    assert_eq!(
        slice.has_at_most_one_distinct_kind(),
        !slice.has_multiple_distinct_kinds(),
        "has_at_most_one_distinct_kind() drifted from !has_multiple_distinct_kinds()",
    );
    assert_eq!(
        slice.has_at_most_one_distinct_kind(),
        slice.distinct_kind_count() <= 1,
        "has_at_most_one_distinct_kind() drifted from (distinct_kind_count() <= 1)",
    );
    assert_eq!(
        slice.has_at_most_one_distinct_kind(),
        distinct.len() <= 1,
        "has_at_most_one_distinct_kind() drifted from (distinct_kinds().len() <= 1)",
    );
    assert_eq!(
        slice.has_at_most_one_distinct_kind(),
        !slice.has_any_distinct_kind() || slice.has_unique_distinct_kind(),
        "has_at_most_one_distinct_kind() drifted from (!has_any_distinct_kind() || has_unique_distinct_kind())",
    );
    ...
    // lacks_only_kind(k) ↔ (missing_kinds() == vec![k]) — the kind-
    // scoped strict-refinement projection on the closed-set-complement
    // axis. Byte-for-byte peer of `has_only_kind` under complement:
    // where `has_only_kind` refines `has_kind` under a well-formed-
    // diagonal strengthening on the populated axis, `lacks_only_kind`
    // refines `lacks_kind` under the same strengthening on the missing
    // axis — the closed-set-complement mirror closes the (populated,
    // missing) × (subset, equal) 2x2 kind-scoped strict-refinement grid
    // at the slice level alongside `has_kind` / `lacks_kind` /
    // `has_only_kind`. A regression that overrode `lacks_only_kind` to
    // drop the fused-walk short-circuit (returning `lacks_kind` — TOO
    // LOOSE, admits multi-missing-kind slices) or to drop the
    // `saw_kind` arm (returning `missing_kind_count() <= 1` — TOO
    // LOOSE, admits the saturated slice as well-formed on the missing
    // axis) surfaces HERE at the substrate boundary, not as silent
    // drift at every downstream `lacks-only-<kind>` require-tag
    // classifier or near-saturation-diagonal coherence check callsite.
    // Byte-for-byte peer of `crate::tagged_union::TaggedUnion::lacks_only`
    // one struct-layer up under the SAME fused short-circuit closed-set
    // walk shape. Also pins the widened composition laws
    // `lacks_only_kind(k) == (missing_kinds() == vec![k])`,
    // `lacks_only_kind(k) == (lacks_kind(k) && missing_kind_count() == 1)`,
    // and the kind-domain exhaustivity law "AT MOST ONE `k` satisfies
    // `lacks_only_kind(k)` on any slice".
    let mut lacks_only_hits = 0usize;
    for kind in ConditionKind::ALL {
        let expected_widened = missing == vec![kind];
        assert_eq!(
            slice.lacks_only_kind(kind),
            expected_widened,
            "lacks_only_kind({kind:?}) drifted from (missing_kinds() == vec![{kind:?}])",
        );
        assert_eq!(
            slice.lacks_only_kind(kind),
            slice.lacks_kind(kind) && slice.missing_kind_count() == 1,
            "lacks_only_kind({kind:?}) drifted from (lacks_kind({kind:?}) && missing_kind_count() == 1)",
        );
        // Strict-refinement of `lacks_kind`: lacks_only_kind(k) ⟹ lacks_kind(k).
        if slice.lacks_only_kind(kind) {
            assert!(
                slice.lacks_kind(kind),
                "lacks_only_kind({kind:?}) implies lacks_kind({kind:?})",
            );
            lacks_only_hits += 1;
        }
    }
    ...
    assert!(
        lacks_only_hits <= 1,
        "lacks_only_kind(k) satisfied by more than one kind (count={lacks_only_hits}) — kind-domain exhaustivity violated",
    );
    // lacks_only_kind(k) for SOME k ⟺ missing_kind_count() == 1 — the
    // kind-domain-exhaustivity ⟺ near-saturation-diagonal pin.
    assert_eq!(
        lacks_only_hits == 1,
        slice.missing_kind_count() == 1,
        "lacks_only_kind holds for some kind iff missing_kind_count() == 1",
    );
    // lacks_only_kind(k) ⟺ has_unique_missing_kind && first_missing_kind() == Some(k)
    // — kind-domain agreement with the arg-less unique-missing predicate.
    for kind in ConditionKind::ALL {
        assert_eq!(
            slice.lacks_only_kind(kind),
            slice.has_unique_missing_kind() && slice.first_missing_kind() == Some(kind),
...
mod tests {
    ...
    // ── ConditionSliceExt::has_any_distinct_kind — at-least-one halfspace pins ──
    //
    // Boolean at-least-one halfspace peer of `has_any_missing_kind` on
    // the closed-set-inversion axis: `has_any_distinct_kind()` returns
    // `true` iff AT LEAST ONE `ConditionKind::ALL` variant appears at
    // least once in the slice, via a SHORT-CIRCUITING closed-set walk
    // `ConditionKind::ALL.iter().copied().any(|k| self.has_kind(k))`
    // that returns `true` at the FIRST populated kind. The composition
    // laws `has_any_distinct_kind() == (distinct_kind_count() > 0)`,
    // `has_any_distinct_kind() == !distinct_kinds().is_empty()`, and
    // `has_any_distinct_kind() == first_distinct_kind().is_some()` are
    // pinned as the at-least-one halfspace arm of
    // `assert_slice_refinement_composition_laws` on the closed-set-
    // inversion axis. Byte-for-byte peer of
    // `crate::tagged_union::TaggedUnion::has_any_populated_kind` one
    // struct-layer up under the SAME `any(has)` short-circuit shape.

    /// EMPTY-SLICE pin — an empty slice returns `false` on
    /// `has_any_distinct_kind` (no kind is present) — the SOLE arm
    /// where the primitive returns `false`. Dual of the empty-slice
    /// arm on `has_any_missing_kind` (which returns `true`).
    #[test]
    fn condition_slice_has_any_distinct_kind_returns_false_on_empty_slice() {
        let empty: &[Condition] = &[];
        assert!(
            !empty.has_any_distinct_kind(),
            "empty slice must return false on has_any_distinct_kind",
        );
        assert_eq!(
            empty.has_any_distinct_kind(),
            empty.distinct_kind_count() > 0,
            "empty has_any_distinct_kind must equal (distinct_kind_count() > 0)",
        );
        assert_eq!(
            empty.has_any_distinct_kind(),
            !empty.distinct_kinds().is_empty(),
            "empty has_any_distinct_kind must equal !distinct_kinds().is_empty()",
        );
        assert_eq!(
            empty.has_any_distinct_kind(),
            empty.first_distinct_kind().is_some(),
            "empty has_any_distinct_kind must equal first_distinct_kind().is_some()",
        );
    }

    /// SINGLE-KIND pin — a slice populating exactly one variant
    /// returns `true` on `has_any_distinct_kind` for every
    /// [`ConditionKind`] (a single element contributes one distinct
    /// kind, ≥ 1). Also pins the composition law
    /// `has_any_distinct_kind() == (distinct_kind_count() > 0)` at
    /// the single-populated arm.
    #[test]
    fn condition_slice_has_any_distinct_kind_returns_true_on_single_kind_slice() {
        for populated in ConditionKind::ALL {
            let slice = [condition_with(populated)];
            assert!(
                slice.has_any_distinct_kind(),
                "single-populated slice with {populated:?} must return true on has_any_distinct_kind",
            );
            assert_eq!(
                slice.has_any_distinct_kind(),
                slice.distinct_kind_count() > 0,
                "single-populated has_any_distinct_kind must equal (distinct_kind_count() > 0) for {populated:?}",
            );
        }
    }

    /// FULL-COVERAGE pin — a slice that carries every
    /// [`ConditionKind`] variant returns `true` on
    /// `has_any_distinct_kind`. Dual of the FULL-COVERAGE arm on
    /// `has_any_missing_kind` (which returns `false`) — the two
    /// Booleans DISAGREE on the saturated arm.
    #[test]
    fn condition_slice_has_any_distinct_kind_returns_true_on_saturated_slice() {
        let saturated: Vec<Condition> =
            ConditionKind::ALL.into_iter().map(condition_with).collect();
        assert!(
            saturated.as_slice().has_any_distinct_kind(),
            "slice containing every ConditionKind must return true on has_any_distinct_kind",
        );
        assert_eq!(
            saturated.as_slice().has_any_distinct_kind(),
            !saturated.as_slice().distinct_kinds().is_empty(),
            "saturated has_any_distinct_kind must equal !distinct_kinds().is_empty()",
        );
    }

    /// DUPLICATE-COVERAGE pin — a slice that carries the SAME
    /// [`ConditionKind`] multiple times still returns `true`
    /// (multiplicity is irrelevant to the at-least-one halfspace
    /// predicate on the closed-set-inversion axis, byte-for-byte peer
    /// of the closed-set-complement halfspace arm).
    #[test]
    fn condition_slice_has_any_distinct_kind_ignores_multiplicity() {
        for k in ConditionKind::ALL {
            let doubled: Vec<Condition> = vec![condition_with(k), condition_with(k)];
            assert!(
                doubled.as_slice().has_any_distinct_kind(),
                "slice carrying {k:?} twice must return true on has_any_distinct_kind",
            );
        }
    }
    ...
    // ── ConditionSliceExt::has_multiple_distinct_kinds — cardinality-many-arm pins ──
    //
    // Boolean cardinality many-arm peer of `has_unique_distinct_kind`
    // on the closed-set-inversion axis: `has_multiple_distinct_kinds()`
    // returns `true` iff AT LEAST TWO `ConditionKind::ALL` variants
    // appear at least once in the slice, byte-for-byte with a two-step-
    // short-circuit walk over `iter_distinct_kinds`. The composition
    // laws `has_multiple_distinct_kinds() == (distinct_kind_count() >= 2)`
    // and `has_multiple_distinct_kinds() == (distinct_kinds().len() >= 2)`
    ...
    /// EMPTY-SLICE pin — an empty slice returns `false` on
    /// `has_multiple_distinct_kinds` (0 distinct, not ≥ 2). Dual of
    /// the empty-slice arm on `has_multiple_missing_kinds` which
    /// returns `true`. Also pins the composition law
    /// `has_multiple_distinct_kinds() == (distinct_kind_count() >= 2)`
    /// at zero-distinct.
    #[test]
    fn condition_slice_has_multiple_distinct_kinds_returns_false_on_empty_slice() {
        let empty: &[Condition] = &[];
        assert!(
            !empty.has_multiple_distinct_kinds(),
            "empty slice must return false on has_multiple_distinct_kinds (0 distinct, not ≥ 2)",
        );
        assert_eq!(
            empty.has_multiple_distinct_kinds(),
            empty.distinct_kind_count() >= 2,
            "empty has_multiple_distinct_kinds must equal (distinct_kind_count() >= 2)",
        );
    }

    /// SINGLE-KIND pin — a slice populating exactly one variant
    /// returns `false` on `has_multiple_distinct_kinds` (1 distinct,
    /// not ≥ 2). The SOLE-populated arrangement where the many-arm
    /// primitive returns `false` alongside the empty-endpoint — the
    /// definitional boundary between the =1 mid-endpoint and the ≥ 2
    /// many-arm on the distinct axis. Also pins the widened
    /// composition law
    /// `has_multiple_distinct_kinds() == (distinct_kinds().len() >= 2)`.
    #[test]
    fn condition_slice_has_multiple_distinct_kinds_returns_false_on_single_kind_slice() {
        for populated in ConditionKind::ALL {
            let slice = [condition_with(populated)];
            assert!(
                !slice.has_multiple_distinct_kinds(),
                "single-populated slice with {populated:?} must return false on has_multiple_distinct_kinds (1 distinct, not ≥ 2)",
            );
            assert_eq!(
                slice.has_multiple_distinct_kinds(),
                slice.distinct_kind_count() >= 2,
                "single-populated has_multiple_distinct_kinds must equal (distinct_kind_count() >= 2) for {populated:?}",
            );
            assert_eq!(
                slice.has_multiple_distinct_kinds(),
                slice.distinct_kinds().len() >= 2,
                "single-populated has_multiple_distinct_kinds must equal (distinct_kinds().len() >= 2) for {populated:?}",
            );
        }
    }

    /// SATURATED pin — a slice carrying every [`ConditionKind`]
    /// variant returns `true` on `has_multiple_distinct_kinds` on any
    /// `N ≥ 2` closed set (`N` distinct, ≥ 2). Dual of the SATURATED
    /// arm on `has_unique_distinct_kind` which returns `false` on
    /// `N ≥ 2`.
    #[test]
    fn condition_slice_has_multiple_distinct_kinds_returns_true_on_saturated_slice() {
        assert!(
            ConditionKind::ALL.len() >= 2,
            "test assumes ConditionKind::ALL has ≥ 2 variants",
        );
        let saturated: Vec<Condition> =
            ConditionKind::ALL.into_iter().map(condition_with).collect();
        assert!(
            saturated.as_slice().has_multiple_distinct_kinds(),
            "saturated slice must return true on has_multiple_distinct_kinds ({} distinct, ≥ 2)",
            ConditionKind::ALL.len(),
        );
        assert_eq!(
            saturated.as_slice().has_multiple_distinct_kinds(),
            saturated.as_slice().distinct_kind_count() >= 2,
            "saturated has_multiple_distinct_kinds must equal (distinct_kind_count() >= 2)",
        );
    }

    /// TWO-POPULATED pin — a slice carrying exactly two distinct
    /// [`ConditionKind`] variants returns `true` on
    /// `has_multiple_distinct_kinds` (2 distinct, ≥ 2). Pins the
    /// SECOND-slot boundary of the ≥ 2 arm — a regression that
    /// dropped the second-slot short-circuit (conflating with the
    /// at-least-one halfspace, returning `true` on any partial
    /// arrangement) would still pass here; complemented by the
    /// SINGLE-KIND pin which distinguishes the =1 arm from the ≥ 2
    /// arm. Only meaningful on `N ≥ 2` closed sets.
    #[test]
    fn condition_slice_has_multiple_distinct_kinds_returns_true_on_two_populated_slice() {
        assert!(
            ConditionKind::ALL.len() >= 2,
            "test assumes ConditionKind::ALL has ≥ 2 variants",
        );
        for i in 0..ConditionKind::ALL.len() {
            for j in (i + 1)..ConditionKind::ALL.len() {
                ...
                assert!(
                    slice.has_multiple_distinct_kinds(),
                    "two-populated slice (kinds at index {i} and {j}) must return true on has_multiple_distinct_kinds (2 distinct, ≥ 2)",
                );
                assert_eq!(
                    slice.has_multiple_distinct_kinds(),
                    slice.distinct_kind_count() >= 2,
                    "two-populated has_multiple_distinct_kinds must equal (distinct_kind_count() >= 2) for kinds=({i}, {j})",
                );
            }
        }
    }
    ...
    /// MULTIPLICITY pin — a slice carrying the SAME [`ConditionKind`]
    /// multiple times still returns `false` on
    /// `has_multiple_distinct_kinds` (multiplicity is irrelevant to
    /// the cardinality-many-arm projection on the closed-set-inversion
    /// axis — one distinct kind regardless of repetition count).
    #[test]
    fn condition_slice_has_multiple_distinct_kinds_ignores_multiplicity() {
        for k in ConditionKind::ALL {
            let doubled: Vec<Condition> = vec![condition_with(k), condition_with(k)];
            assert!(
                !doubled.as_slice().has_multiple_distinct_kinds(),
                "slice carrying {k:?} twice must return false on has_multiple_distinct_kinds (still 1 distinct)",
            );
            let tripled: Vec<Condition> =
                vec![condition_with(k), condition_with(k), condition_with(k)];
            assert!(
    ...
    // ── ConditionSliceExt::has_at_most_one_distinct_kind — "≤ 1" pins ─
    //
    // Boolean cardinality "≤ 1" negation peer of
    // `has_multiple_distinct_kinds` on the closed-set-inversion axis:
    // `has_at_most_one_distinct_kind()` returns `true` iff AT MOST ONE
    // `ConditionKind::ALL` variant appears at least once in the slice,
    // byte-for-byte with `!has_multiple_distinct_kinds()` via the
    // definitional negation in the trait's default body. The
    // composition laws
    // `has_at_most_one_distinct_kind() == !has_multiple_distinct_kinds()`,
    // `has_at_most_one_distinct_kind() == (distinct_kind_count() <= 1)`,
    // `has_at_most_one_distinct_kind() == (distinct_kinds().len() <= 1)`,
    // and `has_at_most_one_distinct_kind() == !has_any_distinct_kind() ||
    // has_unique_distinct_kind()` are pinned as the distinct-axis
    // "≤ 1" arm of `assert_slice_refinement_composition_laws`. Byte-
    // for-byte peer of
    // `crate::tagged_union::TaggedUnion::has_at_most_one_populated_kind`
    // one struct-layer up under the SAME `!has_multiple_populated_kinds`
    // definitional negation shape. Axis-parity mirror of the closed-
    // set-complement "≤ 1" negation peer
    // `has_at_most_one_missing_kind ↔ !has_multiple_missing_kinds` at
    // the same slice-level trait.

    /// EMPTY-SLICE pin — an empty slice returns `true` on
    /// `has_at_most_one_distinct_kind` (0 distinct, `≤ 1`). Dual of
    /// the empty-slice arm on `has_at_most_one_missing_kind` which
    /// returns `false` on `N ≥ 2` closed sets (empty means every kind
    /// missing, `N ≥ 2` missing, not `≤ 1`). Also pins the composition
    /// law `has_at_most_one_distinct_kind() == (distinct_kind_count() <= 1)`
    /// at zero-distinct.
    #[test]
    fn condition_slice_has_at_most_one_distinct_kind_returns_true_on_empty_slice() {
        let empty: &[Condition] = &[];
        assert!(
            empty.has_at_most_one_distinct_kind(),
            "empty slice must return true on has_at_most_one_distinct_kind (0 distinct, ≤ 1)",
        );
        assert_eq!(
            empty.has_at_most_one_distinct_kind(),
            empty.distinct_kind_count() <= 1,
            "empty has_at_most_one_distinct_kind must equal (distinct_kind_count() <= 1)",
        );
    }

    /// SINGLE-KIND pin — a slice populating exactly one variant
    /// returns `true` on `has_at_most_one_distinct_kind` (1 distinct,
    /// `≤ 1`). The singleton arm on the distinct-axis "≤ 1" primitive
    /// — the definitional boundary between the =1 mid-endpoint and the
    /// ≥ 2 many-arm on the distinct axis, where the "≤ 1" negation
    /// still fires. Also pins the widened composition law
    /// `has_at_most_one_distinct_kind() == (distinct_kinds().len() <= 1)`
    /// and the trichotomy-union arm
    /// `has_at_most_one_distinct_kind() == !has_any_distinct_kind() ||
    /// has_unique_distinct_kind()`.
    #[test]
    fn condition_slice_has_at_most_one_distinct_kind_returns_true_on_single_kind_slice() {
        for populated in ConditionKind::ALL {
            let slice = [condition_with(populated)];
            assert!(
                slice.has_at_most_one_distinct_kind(),
                "single-populated slice with {populated:?} must return true on has_at_most_one_distinct_kind (1 distinct, ≤ 1)",
            );
            assert_eq!(
                slice.has_at_most_one_distinct_kind(),
                !slice.has_multiple_distinct_kinds(),
                "single-populated has_at_most_one_distinct_kind must equal !has_multiple_distinct_kinds() for {populated:?}",
            );
            assert_eq!(
                slice.has_at_most_one_distinct_kind(),
                slice.distinct_kind_count() <= 1,
                "single-populated has_at_most_one_distinct_kind must equal (distinct_kind_count() <= 1) for {populated:?}",
            );
            assert_eq!(
                slice.has_at_most_one_distinct_kind(),
                slice.distinct_kinds().len() <= 1,
                "single-populated has_at_most_one_distinct_kind must equal (distinct_kinds().len() <= 1) for {populated:?}",
            );
            assert_eq!(
                slice.has_at_most_one_distinct_kind(),
                !slice.has_any_distinct_kind() || slice.has_unique_distinct_kind(),
                "single-populated has_at_most_one_distinct_kind must equal (!has_any_distinct_kind() || has_unique_distinct_kind()) for {populated:?}",
            );
        }
    }

    /// SATURATED pin — a slice carrying every [`ConditionKind`]
    /// variant returns `false` on `has_at_most_one_distinct_kind` on
    /// any `N ≥ 2` closed set (`N` distinct, not `≤ 1`). Dual of the
    /// SATURATED arm on `has_at_most_one_missing_kind` which returns
    /// `true` on `N ≥ 2` (0 missing, `≤ 1`).
    #[test]
    fn condition_slice_has_at_most_one_distinct_kind_returns_false_on_saturated_slice() {
        assert!(
            ConditionKind::ALL.len() >= 2,
            "test assumes ConditionKind::ALL has ≥ 2 variants",
        );
        let saturated: Vec<Condition> =
            ConditionKind::ALL.into_iter().map(condition_with).collect();
        assert!(
            !saturated.as_slice().has_at_most_one_distinct_kind(),
            "saturated slice must return false on has_at_most_one_distinct_kind ({} distinct, not ≤ 1)",
            ConditionKind::ALL.len(),
        );
        assert_eq!(
            saturated.as_slice().has_at_most_one_distinct_kind(),
            saturated.as_slice().distinct_kind_count() <= 1,
            "saturated has_at_most_one_distinct_kind must equal (distinct_kind_count() <= 1)",
        );
        assert_eq!(
            saturated.as_slice().has_at_most_one_distinct_kind(),
            !saturated.as_slice().has_any_distinct_kind()
                || saturated.as_slice().has_unique_distinct_kind(),
            "saturated has_at_most_one_distinct_kind must equal (!has_any_distinct_kind() || has_unique_distinct_kind())",
        );
    }

    /// TWO-POPULATED pin — a slice carrying exactly two distinct
    /// [`ConditionKind`] variants returns `false` on
    /// `has_at_most_one_distinct_kind` (2 distinct, not `≤ 1`). Pins
    /// the SECOND-slot boundary of the negation of the ≥ 2 arm — a
    /// regression that dropped the second-slot short-circuit on the
    /// many-arm primitive (returning `false` on any partial-populated
    /// arrangement, which negates to `true` here) would still pass
    /// here; complemented by the SATURATED pin which distinguishes the
    /// N-distinct arm from the 2-distinct arm. Only meaningful on
    /// `N ≥ 2` closed sets.
    #[test]
    fn condition_slice_has_at_most_one_distinct_kind_returns_false_on_two_populated_slice() {
        assert!(
            ConditionKind::ALL.len() >= 2,
            "test assumes ConditionKind::ALL has ≥ 2 variants",
        );
        for i in 0..ConditionKind::ALL.len() {
            for j in (i + 1)..ConditionKind::ALL.len() {
                ...
                assert!(
                    !slice.has_at_most_one_distinct_kind(),
                    "two-populated slice (kinds at index {i} and {j}) must return false on has_at_most_one_distinct_kind (2 distinct, not ≤ 1)",
                );
                assert_eq!(
                    slice.has_at_most_one_distinct_kind(),
                    slice.distinct_kind_count() <= 1,
                    "two-populated has_at_most_one_distinct_kind must equal (distinct_kind_count() <= 1) for kinds=({i}, {j})",
                );
            }
        }
    }
    ...
    /// `has_at_most_one_distinct_kind` (multiplicity is irrelevant to
    /// the "≤ 1" projection on the closed-set-inversion axis — one
    /// distinct kind regardless of repetition count, `≤ 1`).
    #[test]
    fn condition_slice_has_at_most_one_distinct_kind_ignores_multiplicity() {
        for k in ConditionKind::ALL {
            let doubled: Vec<Condition> = vec![condition_with(k), condition_with(k)];
            assert!(
                doubled.as_slice().has_at_most_one_distinct_kind(),
                "slice carrying {k:?} twice must return true on has_at_most_one_distinct_kind (still 1 distinct, ≤ 1)",
            );
            let tripled: Vec<Condition> =
                vec![condition_with(k), condition_with(k), condition_with(k)];
            assert!(
                tripled.as_slice().has_at_most_one_distinct_kind(),
                "slice carrying {k:?} three times must return true on has_at_most_one_distinct_kind (still 1 distinct, ≤ 1)",
            );
        }
    }
    ...
    // ── ConditionSliceExt::lacks_only_kind — kind-scoped strict-
    // refinement on the closed-set-complement (missing) axis ─
    //
    // Byte-for-byte peer of `has_only_kind` under complement: fused
    // short-circuit walk over `ConditionKind::ALL` under `has_kind`
    // that skips populated slots, returns `false` at the earliest
    // missing slot whose kind is NOT `kind`, and returns `true` iff
    // the sweep completes with `kind` seen as the sole missing slot.
    // The composition laws
    // `lacks_only_kind(k) == (missing_kinds() == vec![k])` and
    // `lacks_only_kind(k) == (lacks_kind(k) && missing_kind_count() == 1)`
    // are pinned as the closed-set-complement kind-scoped strict-
    // refinement arms of `assert_slice_refinement_composition_laws`.

    /// EMPTY-SLICE pin — every kind is missing (missing set == ALL),
    /// so no kind is "only" missing on any `N ≥ 2` closed set. Returns
    /// `false` on every arm.
    #[test]
    fn condition_slice_lacks_only_kind_returns_false_on_empty_slice() {
        let empty: &[Condition] = &[];
        for kind in ConditionKind::ALL {
            assert!(
                !empty.lacks_only_kind(kind),
                "empty slice must return false on lacks_only_kind for {kind:?}",
            );
            assert_eq!(
                empty.lacks_only_kind(kind),
                empty.missing_kinds() == vec![kind],
                "empty lacks_only_kind({kind:?}) must equal (missing_kinds() == vec![{kind:?}])",
            );
        }
    }

    /// NEAR-SATURATION pin — a slice covering every kind except one
    /// returns `true` on `lacks_only_kind(omitted)` and `false` on
    /// every other kind. The sole `true` arm on the well-formed
    /// missing diagonal.
    #[test]
    fn condition_slice_lacks_only_kind_returns_true_on_near_saturation_slice() {
        for omitted in ConditionKind::ALL {
            let slice: Vec<Condition> = ConditionKind::ALL
                .into_iter()
                .filter(|k| *k != omitted)
                .map(condition_with)
                .collect();
            for kind in ConditionKind::ALL {
                let expected = kind == omitted;
                assert_eq!(
                    slice.as_slice().lacks_only_kind(kind),
                    expected,
                    "near-saturation slice omitted={omitted:?} must return {expected} on lacks_only_kind for {kind:?}",
                );
                assert_eq!(
                    slice.as_slice().lacks_only_kind(kind),
                    slice.as_slice().missing_kinds() == vec![kind],
                    "near-saturation lacks_only_kind({kind:?}) must equal (missing_kinds() == vec![{kind:?}]) for omitted={omitted:?}",
                );
            }
        }
    }

    /// MULTIPLICITY pin — a slice carrying every kind except one, with
    /// the populated kinds each duplicated, ignores multiplicity on
    /// the populated side (byte-for-byte with `has_kind`'s multiplicity
    /// behavior). Returns `true` on `lacks_only_kind(omitted)`.
    #[test]
    fn condition_slice_lacks_only_kind_ignores_multiplicity_on_the_populated_side() {
        for omitted in ConditionKind::ALL {
            let mut slice: Vec<Condition> = Vec::new();
            for k in ConditionKind::ALL {
                if k != omitted {
                    slice.push(condition_with(k));
                    slice.push(condition_with(k));
                }
            }
            for kind in ConditionKind::ALL {
                ...
                assert_eq!(
                    slice.as_slice().lacks_only_kind(kind),
                    expected,
                    "duplicate-populated near-saturation slice omitted={omitted:?} must return {expected} on lacks_only_kind for {kind:?}",
                );
            }
        }
    }

    /// TWO-MISSING pin — a slice omitting exactly two kinds returns
    /// `false` on every arm; the strict refinement fails at the
    /// earliest walk step that hits the second missing kind. On
    /// `ConditionKind::ALL` of cardinality `N`, `N ≥ 3` is required
    /// for a two-missing arrangement to exist.
    #[test]
    fn condition_slice_lacks_only_kind_returns_false_on_two_missing_slice() {
        assert!(
            ConditionKind::ALL.len() >= 3,
            "two-missing arrangement requires N ≥ 3",
        );
        // Slice carries every kind except the first two of ALL.
        let slice: Vec<Condition> = ConditionKind::ALL
            .into_iter()
            .skip(2)
            .map(condition_with)
        ...
        for kind in ConditionKind::ALL {
            assert!(
                !slice.as_slice().lacks_only_kind(kind),
                "two-missing slice must return false on lacks_only_kind for {kind:?}",
            );
            assert_eq!(
                slice.as_slice().lacks_only_kind(kind),
                slice.as_slice().missing_kinds() == vec![kind],
                "two-missing lacks_only_kind({kind:?}) must equal (missing_kinds() == vec![{kind:?}])",
            );
        }
    }

    /// SATURATED pin — every kind populated, no kind missing, no kind
    /// is "only" missing. Returns `false` on every arm.
    #[test]
    fn condition_slice_lacks_only_kind_returns_false_on_saturated_slice() {
        let saturated: Vec<Condition> =
            ConditionKind::ALL.into_iter().map(condition_with).collect();
        for kind in ConditionKind::ALL {
            assert!(
                !saturated.as_slice().lacks_only_kind(kind),
                "saturated slice must return false on lacks_only_kind for {kind:?}",
            );
            assert_eq!(
                saturated.as_slice().lacks_only_kind(kind),
                saturated.as_slice().lacks_kind(kind)
                    && saturated.as_slice().missing_kind_count() == 1,
                "saturated lacks_only_kind({kind:?}) must equal (lacks_kind && missing_kind_count == 1)",
            );
        }
    }
    ...
    /// SUBSTRATE-DELEGATION pin (Boundary at-least-one halfspace triad
    /// on the closed-set-inversion axis) — the three
    /// `has_any_distinct_*_condition_kind` methods on [`Boundary`]
    /// delegate to the slice-level substrate primitive
    /// [`ConditionSliceExt::has_any_distinct_kind`] over the two
    /// `Vec<Condition>` slots (precondition + postcondition) and
    /// compose the union via a SHORT-CIRCUITING closed-set walk over
    /// [`ConditionKind::ALL`] under [`Boundary::has_condition_kind`].
    /// Sweeps the empty boundary (every arm returns `false` — no kind
    /// present in either slice), a single-populated-per-side
    /// arrangement (every per-slice arm returns `true`, the union
    /// returns `true`), a single-populated-precondition-only
    /// arrangement (precondition arm `true`, postcondition arm
    /// `false`, union `true`), and the saturated boundary (every arm
    /// returns `true`). Also pins the composition law
    /// `has_any_distinct_*_condition_kind() ==
    /// (distinct_*_condition_kind_count() > 0)` at each arm — a
    /// regression that dropped the short-circuit, drifted the
    /// underlying `has_condition_kind` predicate, or negated the wrong
    /// side surfaces HERE.
    #[test]
    fn has_any_distinct_condition_kind_triad_delegates_to_slice_has_any_distinct_kind() {
        // Empty boundary — every arm returns false (no kind present
        // in either slice; distinct_kind_count == 0 in both).
        let b = Boundary::default();
        assert!(
            !b.has_any_distinct_precondition_kind(),
            "empty boundary must return false on has_any_distinct_precondition_kind",
        );
        assert!(
            !b.has_any_distinct_postcondition_kind(),
            "empty boundary must return false on has_any_distinct_postcondition_kind",
        );
        assert!(
            !b.has_any_distinct_condition_kind(),
            "empty boundary must return false on has_any_distinct_condition_kind",
        );

        // Single-populated per side — sweep ALL × ALL. Every per-slice
        // arm returns true; the union returns true.
        for pre_kind in ConditionKind::ALL {
            for post_kind in ConditionKind::ALL {
                let mut b = Boundary::default();
                b.preconditions.push(condition_with(pre_kind));
                ...
                assert_eq!(
                    b.has_any_distinct_precondition_kind(),
                    b.preconditions.has_any_distinct_kind(),
                    "Boundary::has_any_distinct_precondition_kind must delegate verbatim to \
                     preconditions.has_any_distinct_kind() for pre={pre_kind:?} post={post_kind:?}",
                );
                assert_eq!(
                    b.has_any_distinct_postcondition_kind(),
                    b.postconditions.has_any_distinct_kind(),
                    "Boundary::has_any_distinct_postcondition_kind must delegate verbatim to \
        ...
        for pre_kind in ConditionKind::ALL {
            ...
            assert!(
                b.has_any_distinct_precondition_kind(),
                "pre-only boundary must return true on has_any_distinct_precondition_kind for pre={pre_kind:?}",
            );
            assert!(
                !b.has_any_distinct_postcondition_kind(),
                "pre-only boundary must return false on has_any_distinct_postcondition_kind for pre={pre_kind:?}",
            );
            assert!(
                b.has_any_distinct_condition_kind(),
                "pre-only boundary must return true on has_any_distinct_condition_kind for pre={pre_kind:?}",
            );
        }
        ...
        assert!(
            b.has_any_distinct_precondition_kind(),
            "saturated boundary must return true on has_any_distinct_precondition_kind",
        );
        assert!(
            b.has_any_distinct_postcondition_kind(),
            "saturated boundary must return true on has_any_distinct_postcondition_kind",
        );
        assert!(
            b.has_any_distinct_condition_kind(),
    ...
    /// on the closed-set-inversion axis) — the three
    /// `has_multiple_distinct_*_condition_kind` methods on [`Boundary`]
    /// delegate to the slice-level substrate primitive
    /// [`ConditionSliceExt::has_multiple_distinct_kinds`] over the two
    /// `Vec<Condition>` slots (precondition + postcondition) and
    /// compose the union via a two-step-short-circuit walk over
    /// [`ConditionKind::ALL`] under [`Boundary::has_condition_kind`].
    /// Sweeps the empty boundary (every arm returns `false` — 0
    /// distinct, not ≥ 2), a single-populated-per-side arrangement
    /// (every per-slice arm returns `false` — 1 distinct per slice;
    /// the union returns `true` iff the two kinds DIFFER, otherwise
    /// both coincide and the union has exactly 1 distinct), and the
    /// saturated boundary (every arm returns `true` on `N ≥ 2` — N
    /// distinct, ≥ 2). Also pins the composition law
    /// `has_multiple_distinct_*_condition_kind() ==
    /// (distinct_*_condition_kind_count() >= 2)` at each arm — a
    /// regression that dropped the second-slot short-circuit, drifted
    /// the underlying `has_kind` predicate, or conflated with
    /// `has_any_distinct_kind` surfaces HERE.
    #[test]
    fn has_multiple_distinct_condition_kind_triad_delegates_to_slice_has_multiple_distinct_kinds() {
        ...
        let b = Boundary::default();
        assert!(
            !b.has_multiple_distinct_precondition_kind(),
            "empty boundary must return false on has_multiple_distinct_precondition_kind",
        );
        assert!(
            !b.has_multiple_distinct_postcondition_kind(),
            "empty boundary must return false on has_multiple_distinct_postcondition_kind",
        );
        assert!(
            !b.has_multiple_distinct_condition_kind(),
            "empty boundary must return false on has_multiple_distinct_condition_kind",
        );
        assert_eq!(
            b.has_multiple_distinct_condition_kind(),
            b.distinct_condition_kind_count() >= 2,
            "empty has_multiple_distinct_condition_kind must equal (distinct_condition_kind_count() >= 2)",
        );

        // Single-populated per side — sweep ALL × ALL on N ≥ 2 closed
        // sets. Every per-slice arm returns false (1 distinct per
        // slice, not ≥ 2); the union returns true iff the two kinds
        // DIFFER (covering 2 distinct), otherwise both coincide and
        // the union has exactly 1 distinct.
        assert!(
            ConditionKind::ALL.len() >= 2,
            "test assumes ConditionKind::ALL has ≥ 2 variants",
        );
        for pre_kind in ConditionKind::ALL {
            for post_kind in ConditionKind::ALL {
                ...
                assert_eq!(
                    b.has_multiple_distinct_precondition_kind(),
                    b.preconditions.has_multiple_distinct_kinds(),
                    "Boundary::has_multiple_distinct_precondition_kind must delegate verbatim to \
                     preconditions.has_multiple_distinct_kinds() for pre={pre_kind:?} post={post_kind:?}",
                );
                assert_eq!(
                    b.has_multiple_distinct_postcondition_kind(),
                    b.postconditions.has_multiple_distinct_kinds(),
                    "Boundary::has_multiple_distinct_postcondition_kind must delegate verbatim to \
                     postconditions.has_multiple_distinct_kinds() for pre={pre_kind:?} post={post_kind:?}",
                );
                assert!(
                    !b.has_multiple_distinct_precondition_kind(),
                    "single-populated preconditions must return false on has_multiple_distinct_precondition_kind for pre={pre_kind:?}",
                );
                assert!(
                    !b.has_multiple_distinct_postcondition_kind(),
                    "single-populated postconditions must return false on has_multiple_distinct_postcondition_kind for post={post_kind:?}",
                );
                let covered_count = ConditionKind::ALL
                    .into_iter()
                    .filter(|k| *k == pre_kind || *k == post_kind)
                ...
                assert_eq!(
                    b.has_multiple_distinct_condition_kind(),
                    expected_union,
                    "Boundary::has_multiple_distinct_condition_kind must equal \
                ...
                assert_eq!(
                    ...
                    b.distinct_condition_kind_count() >= 2,
                    "Boundary::has_multiple_distinct_condition_kind must equal \
                     (distinct_condition_kind_count() >= 2) for pre={pre_kind:?} post={post_kind:?}",
                );
            }
        }
        ...
        assert!(
            b.has_multiple_distinct_precondition_kind(),
            "saturated boundary must return true on has_multiple_distinct_precondition_kind",
        );
        assert!(
            b.has_multiple_distinct_postcondition_kind(),
            "saturated boundary must return true on has_multiple_distinct_postcondition_kind",
        );
        assert!(
            b.has_multiple_distinct_condition_kind(),
            "saturated boundary must return true on has_multiple_distinct_condition_kind",
        );
    }

    /// SUBSTRATE-DELEGATION pin (Boundary cardinality "≤ 1" triad on
    /// the closed-set-inversion axis) — the three
    /// `has_at_most_one_distinct_*_condition_kind` methods on
    /// [`Boundary`] delegate to the slice-level substrate primitive
    /// [`ConditionSliceExt::has_at_most_one_distinct_kind`] over the
    /// two `Vec<Condition>` slots (precondition + postcondition) and
    /// compose the union via a definitional negation of the many-arm
    /// two-step-short-circuit walk over [`ConditionKind::ALL`] under
    /// [`Boundary::has_condition_kind`]. Sweeps the empty boundary
    /// (every arm returns `true` — 0 distinct, `≤ 1`), a single-
    /// populated-per-side arrangement (every per-slice arm returns
    /// `true` — 1 distinct per slice, `≤ 1`; the union returns `true`
    /// iff the two kinds COINCIDE, otherwise the union has 2 distinct
    /// and drops to `false`), and the saturated boundary (every arm
    /// returns `false` on `N ≥ 2` — N distinct, not `≤ 1`). Also pins
    /// the composition law `has_at_most_one_distinct_*_condition_kind()
    /// == !has_multiple_distinct_*_condition_kind()` and the widened
    /// composition law
    /// `has_at_most_one_distinct_*_condition_kind() ==
    /// (distinct_*_condition_kind_count() <= 1)` at each arm — a
    /// regression that dropped the definitional negation, drifted the
    /// underlying `has_kind` predicate, or swapped the wrong side of
    /// the negation surfaces HERE.
    #[test]
    fn has_at_most_one_distinct_condition_kind_triad_delegates_to_slice_has_at_most_one_distinct_kind(
    ) {
        // Empty boundary — every arm returns true (0 distinct, ≤ 1).
        let b = Boundary::default();
        assert!(
            b.has_at_most_one_distinct_precondition_kind(),
            "empty boundary must return true on has_at_most_one_distinct_precondition_kind",
        );
        assert!(
            b.has_at_most_one_distinct_postcondition_kind(),
            "empty boundary must return true on has_at_most_one_distinct_postcondition_kind",
        );
        assert!(
            b.has_at_most_one_distinct_condition_kind(),
            "empty boundary must return true on has_at_most_one_distinct_condition_kind",
        );
        assert_eq!(
            b.has_at_most_one_distinct_condition_kind(),
            b.distinct_condition_kind_count() <= 1,
        ...
        // Single-populated per side — sweep ALL × ALL on N ≥ 2 closed
        // sets. Every per-slice arm returns true (1 distinct per
        // slice, ≤ 1); the union returns true iff the two kinds
        // COINCIDE (union has 1 distinct kind), otherwise the union
        // has 2 distinct kinds and drops to false.
        assert!(
            ConditionKind::ALL.len() >= 2,
            "test assumes ConditionKind::ALL has ≥ 2 variants",
        );
        for pre_kind in ConditionKind::ALL {
            for post_kind in ConditionKind::ALL {
                ...
                assert_eq!(
                    b.has_at_most_one_distinct_precondition_kind(),
                    b.preconditions.has_at_most_one_distinct_kind(),
                    "Boundary::has_at_most_one_distinct_precondition_kind must delegate verbatim to \
                     preconditions.has_at_most_one_distinct_kind() for pre={pre_kind:?} post={post_kind:?}",
                );
                assert_eq!(
                    b.has_at_most_one_distinct_postcondition_kind(),
                    b.postconditions.has_at_most_one_distinct_kind(),
                    "Boundary::has_at_most_one_distinct_postcondition_kind must delegate verbatim to \
                     postconditions.has_at_most_one_distinct_kind() for pre={pre_kind:?} post={post_kind:?}",
                );
                assert!(
                    b.has_at_most_one_distinct_precondition_kind(),
                    "single-populated preconditions must return true on has_at_most_one_distinct_precondition_kind for pre={pre_kind:?}",
                );
                assert!(
                    b.has_at_most_one_distinct_postcondition_kind(),
                    "single-populated postconditions must return true on has_at_most_one_distinct_postcondition_kind for post={post_kind:?}",
                );
                let covered_count = ConditionKind::ALL
                    .into_iter()
                    .filter(|k| *k == pre_kind || *k == post_kind)
                ...
                assert_eq!(
                    b.has_at_most_one_distinct_condition_kind(),
                    expected_union,
                    "Boundary::has_at_most_one_distinct_condition_kind must equal \
                     (covered-ALL-count <= 1) for pre={pre_kind:?} post={post_kind:?}",
                );
                assert_eq!(
                    b.has_at_most_one_distinct_condition_kind(),
                    !b.has_multiple_distinct_condition_kind(),
                    "Boundary::has_at_most_one_distinct_condition_kind must equal \
                     !has_multiple_distinct_condition_kind() for pre={pre_kind:?} post={post_kind:?}",
                );
                assert_eq!(
                    b.has_at_most_one_distinct_condition_kind(),
                    b.distinct_condition_kind_count() <= 1,
        ...
        assert!(
            !b.has_at_most_one_distinct_precondition_kind(),
            "saturated boundary must return false on has_at_most_one_distinct_precondition_kind",
        );
        assert!(
            !b.has_at_most_one_distinct_postcondition_kind(),
            "saturated boundary must return false on has_at_most_one_distinct_postcondition_kind",
        );
        assert!(
            !b.has_at_most_one_distinct_condition_kind(),
    ...
    /// TRIAD delegation pin — the (precondition, postcondition,
    /// condition-union) kind-scoped strict-refinement-on-missing triad
    /// on [`Boundary`] agrees byte-for-byte with the slice-level
    /// substrate primitive [`ConditionSliceExt::lacks_only_kind`] on
    /// every authored arrangement.
    ///
    /// Sweeps [`ConditionKind::ALL`] × [`ConditionKind::ALL`] over
    /// single-populated-per-side arrangements + near-saturation-per-
    /// side arrangements (the union arm's well-formed missing
    /// diagonal), probing every [`ConditionKind`] at the union arm
    /// against the DERIVED oracle
    /// `boundary.missing_condition_kinds() == vec![probe]` — a
    /// regression at the union arm's fused walk (dropping the
    /// short-circuit, swapping the `saw_kind` arm, mis-composing the
    /// `has_condition_kind` complement) surfaces HERE rather than as
    /// silent drift at every downstream `lacks-only-<kind>` require-
    /// tag classifier or near-saturation-diagonal coherence check
    /// callsite. Also pins the per-slice arms delegate verbatim to
    /// [`ConditionSliceExt::lacks_only_kind`] over the corresponding
    /// half-slice.
    #[test]
    fn lacks_only_condition_kind_triad_delegates_to_slice_lacks_only_kind() {
        // Empty boundary — every kind is missing from the union
        // (2 ≥ N missing on any N ≥ 2), so no kind is "only" missing.
        let b = Boundary::default();
        for kind in ConditionKind::ALL {
            assert_eq!(
                b.lacks_only_precondition_kind(kind),
                b.preconditions.lacks_only_kind(kind),
                "empty boundary lacks_only_precondition_kind must delegate to preconditions.lacks_only_kind for {kind:?}",
            );
            assert_eq!(
                b.lacks_only_postcondition_kind(kind),
                b.postconditions.lacks_only_kind(kind),
                "empty boundary lacks_only_postcondition_kind must delegate to postconditions.lacks_only_kind for {kind:?}",
            );
            assert!(
                !b.lacks_only_condition_kind(kind),
                "empty boundary must return false on lacks_only_condition_kind for {kind:?} (every kind is missing on N ≥ 2)",
            );
        }

        // Single-populated per side — sweep ALL × ALL, then probe
        // every ConditionKind on the (pre, post, union) triad. The
        // per-slice arms return `false` (on N ≥ 3 the slice has ≥ 2
        // missing kinds; on N == 2 the missing set is single-element
        // but only for the OTHER kind). The union arm returns `false`
        // for every kind on N ≥ 3 — the union missing set has size
        // `N - |{pre, post}|` which is ≥ 2 whenever N ≥ 3, or size 1
        // iff pre != post (union covers both), or size N - 1 iff
        // pre == post.
        for pre_kind in ConditionKind::ALL {
            for post_kind in ConditionKind::ALL {
                let mut b = Boundary::default();
                ...
                for probe in ConditionKind::ALL {
                    assert_eq!(
                        b.lacks_only_precondition_kind(probe),
                        b.preconditions.lacks_only_kind(probe),
                        "Boundary::lacks_only_precondition_kind must delegate verbatim to preconditions.lacks_only_kind for pre={pre_kind:?} post={post_kind:?} probe={probe:?}",
                    );
                    assert_eq!(
                        b.lacks_only_postcondition_kind(probe),
                        b.postconditions.lacks_only_kind(probe),
                        "Boundary::lacks_only_postcondition_kind must delegate verbatim to postconditions.lacks_only_kind for pre={pre_kind:?} post={post_kind:?} probe={probe:?}",
                    );
                    assert_eq!(
                        b.lacks_only_condition_kind(probe),
                        b.missing_condition_kinds() == vec![probe],
                        "Boundary::lacks_only_condition_kind({probe:?}) must equal (missing_condition_kinds() == vec![{probe:?}]) for pre={pre_kind:?} post={post_kind:?}",
                    );
                }
            }
        }

        // Near-saturation per side — build a boundary whose preconditions
        // carry every kind except one, sweep every "omitted" kind for the
        // pre side, then probe the per-slice arm. On the well-formed
        // near-saturation diagonal (missing == {omitted}),
        // preconditions.lacks_only_kind(omitted) == true; every other
        // arm is false.
        for omitted in ConditionKind::ALL {
            let mut b = Boundary::default();
            for k in ConditionKind::ALL {
            ...
            for probe in ConditionKind::ALL {
                ...
                assert_eq!(
                    b.lacks_only_precondition_kind(probe),
                    expected,
                    "near-saturation boundary omitted={omitted:?} must return {expected} on lacks_only_precondition_kind({probe:?})",
                );
                assert_eq!(
                    b.lacks_only_postcondition_kind(probe),
                    expected,
                    "near-saturation boundary omitted={omitted:?} must return {expected} on lacks_only_postcondition_kind({probe:?})",
                );
                assert_eq!(
                    b.lacks_only_condition_kind(probe),
                    expected,
                    "near-saturation boundary omitted={omitted:?} must return {expected} on lacks_only_condition_kind({probe:?})",
                );
                assert_eq!(
                    b.lacks_only_condition_kind(probe),
                    b.missing_condition_kinds() == vec![probe],
                    "near-saturation boundary omitted={omitted:?} must agree with missing_condition_kinds() == vec![{probe:?}]",
                );
            }
        }

        // Single-side-only near-saturation — a boundary whose ONE side
        // carries every kind except one, the OTHER side empty: the
        // empty side is missing every kind (per-slice `lacks_only_kind`
        // returns false on N ≥ 2), but the union covers everything the
        // populated side does, so the union missing set is still
        // `{omitted}` and the union arm returns `true` for `omitted`.
        // Pins that the union arm reaches the union primitive, not the
        // (pre AND post) AND-composition (which would fail on the
        // empty side).
        for omitted in ConditionKind::ALL {
            let mut b = Boundary::default();
            for k in ConditionKind::ALL {
            ...
            for probe in ConditionKind::ALL {
                ...
                assert_eq!(
                    b.lacks_only_condition_kind(probe),
                    expected,
                    "pre-only near-saturation boundary omitted={omitted:?} must return {expected} on lacks_only_condition_kind({probe:?})",
                );
                assert_eq!(
                    b.lacks_only_condition_kind(probe),
                    b.missing_condition_kinds() == vec![probe],
                    "pre-only near-saturation boundary omitted={omitted:?} must agree with missing_condition_kinds() == vec![{probe:?}]",
                );
            }
            let mut b = Boundary::default();
            for k in ConditionKind::ALL {
            ...
            for probe in ConditionKind::ALL {
                ...
                assert_eq!(
                    b.lacks_only_condition_kind(probe),
                    expected,
                    "post-only near-saturation boundary omitted={omitted:?} must return {expected} on lacks_only_condition_kind({probe:?})",
                );
            }
        }

        // Saturated boundary — every kind populated in the union, so
        // no kind is missing, so `lacks_only_kind` returns false on
        // every arm.
        let mut b = Boundary::default();
        for k in ConditionKind::ALL {
            b.preconditions.push(condition_with(k));
        ...
        for kind in ConditionKind::ALL {
            assert!(
                !b.lacks_only_precondition_kind(kind),
                "saturated boundary must return false on lacks_only_precondition_kind for {kind:?}",
            );
            assert!(
                !b.lacks_only_postcondition_kind(kind),
                "saturated boundary must return false on lacks_only_postcondition_kind for {kind:?}",
            );
            assert!(
                !b.lacks_only_condition_kind(kind),
                "saturated boundary must return false on lacks_only_condition_kind for {kind:?}",
            );
        }
    }
    ...
    /// SUBSTRATE-DELEGATION pin (Boundary unique-distinct-kind
    /// witnessing triad on the closed-set-inversion axis) — the three
    /// `unique_distinct_*_condition_kind` methods on [`Boundary`]
    /// delegate to the slice-level substrate primitive
    /// [`ConditionSliceExt::unique_distinct_kind`] over the two
    /// `Vec<Condition>` slots (precondition + postcondition) and
    /// compose the union via a two-step-short-circuit walk over
    /// [`ConditionKind::ALL`] under [`Boundary::has_condition_kind`].
    /// Sweep `ConditionKind::ALL × ConditionKind::ALL` so a regression
    /// that (a) dropped the second-hit short-circuit (returning
    /// `Some(first)` on a two-distinct union), (b) inlined a divergent
    /// closed-set walk at either half-slice arm, or (c) diverged the
    /// witness scalar from the Boolean predicate surfaces HERE. Also
    /// pins the composition laws `unique_distinct_*_kind().is_some() ==
    /// has_unique_distinct_*_kind()` and (on the `Some` arm)
    /// `unique_distinct_*_kind() == first_distinct_*_kind() ==
    /// last_distinct_*_kind()` at each arm.
    #[test]
    fn unique_distinct_condition_kind_triad_delegates_to_slice_unique_distinct_kind() {
        // Empty boundary — every arm returns None.
        let b = Boundary::default();
        assert_eq!(
            b.unique_distinct_precondition_kind(),
            None,
            "empty boundary must return None on unique_distinct_precondition_kind",
        );
        assert_eq!(
            b.unique_distinct_postcondition_kind(),
            None,
            "empty boundary must return None on unique_distinct_postcondition_kind",
        );
        assert_eq!(
            b.unique_distinct_condition_kind(),
            None,
            "empty boundary must return None on unique_distinct_condition_kind",
        );
        ...
        for pre_kind in ConditionKind::ALL {
            for post_kind in ConditionKind::ALL {
                ...
                assert_eq!(
                    b.unique_distinct_precondition_kind(),
                    b.preconditions.unique_distinct_kind(),
                    "Boundary::unique_distinct_precondition_kind must delegate verbatim to \
                     preconditions.unique_distinct_kind() for pre={pre_kind:?} post={post_kind:?}",
                );
                assert_eq!(
                    b.unique_distinct_precondition_kind(),
                    Some(pre_kind),
                    "Boundary::unique_distinct_precondition_kind must equal Some(pre_kind) on \
                     single-populated preconditions for pre={pre_kind:?} post={post_kind:?}",
                );
                assert_eq!(
                    b.unique_distinct_postcondition_kind(),
                    b.postconditions.unique_distinct_kind(),
                    "Boundary::unique_distinct_postcondition_kind must delegate verbatim to \
                     postconditions.unique_distinct_kind() for pre={pre_kind:?} post={post_kind:?}",
                );
                assert_eq!(
                    b.unique_distinct_postcondition_kind(),
                    Some(post_kind),
                    "Boundary::unique_distinct_postcondition_kind must equal Some(post_kind) on \
                     single-populated postconditions for pre={pre_kind:?} post={post_kind:?}",
                );

                // Union arm: Some(k) iff the union covers exactly one kind.
                let covered: Vec<ConditionKind> = ConditionKind::ALL
                    .into_iter()
                    .filter(|k| pre_kind == *k || post_kind == *k)
                    .collect();
                let expected_union = if covered.len() == 1 {
                    Some(covered[0])
                } else {
                    None
                };
                assert_eq!(
                    b.unique_distinct_condition_kind(),
                    expected_union,
                    "Boundary::unique_distinct_condition_kind must equal Some(k) iff the \
                     ALL-entries covered by either half-slice sum to exactly one for \
                     pre={pre_kind:?} post={post_kind:?}",
                );

                // Boolean-witness composition laws.
                assert_eq!(
                    b.unique_distinct_condition_kind().is_some(),
                    b.has_unique_distinct_condition_kind(),
                    "unique_distinct_condition_kind().is_some() must equal \
                     has_unique_distinct_condition_kind() for pre={pre_kind:?} post={post_kind:?}",
                );
                assert_eq!(
                    b.unique_distinct_precondition_kind().is_some(),
                    b.has_unique_distinct_precondition_kind(),
                    "unique_distinct_precondition_kind().is_some() must equal \
                     has_unique_distinct_precondition_kind() for pre={pre_kind:?} post={post_kind:?}",
                );
                assert_eq!(
                    b.unique_distinct_postcondition_kind().is_some(),
                    b.has_unique_distinct_postcondition_kind(),
                    "unique_distinct_postcondition_kind().is_some() must equal \
                     has_unique_distinct_postcondition_kind() for pre={pre_kind:?} post={post_kind:?}",
                );

                // Endpoint-agreement — on Some, first == last == unique.
                if let Some(k) = b.unique_distinct_condition_kind() {
                    assert_eq!(
                        b.first_distinct_condition_kind(),
                        Some(k),
                        "unique_distinct_condition_kind() Some arm must equal \
                         first_distinct_condition_kind() for pre={pre_kind:?} post={post_kind:?}",
                    );
                    assert_eq!(
                        b.last_distinct_condition_kind(),
                        Some(k),
                        "unique_distinct_condition_kind() Some arm must equal \
                         last_distinct_condition_kind() for pre={pre_kind:?} post={post_kind:?}",
                    );
                }
            }
        }

        // Saturated boundary — every arm returns None on N ≥ 2.
        if ConditionKind::ALL.len() >= 2 {
            let mut b = Boundary::default();
            for k in ConditionKind::ALL {
                b.preconditions.push(condition_with(k));
            ...
            assert_eq!(
                b.unique_distinct_precondition_kind(),
                None,
                "saturated boundary must return None on unique_distinct_precondition_kind",
            );
            assert_eq!(
                b.unique_distinct_postcondition_kind(),
                None,
                "saturated boundary must return None on unique_distinct_postcondition_kind",
            );
            assert_eq!(
                b.unique_distinct_condition_kind(),
                None,
                "saturated boundary must return None on unique_distinct_condition_kind",
            );
        }
    }

    /// SUBSTRATE-DELEGATION pin (Boundary unique-missing-kind
    /// witnessing triad on the closed-set-complement axis) — the three
    /// `unique_missing_*_condition_kind` methods on [`Boundary`]
    /// delegate to the slice-level substrate primitive
    /// [`ConditionSliceExt::unique_missing_kind`] over the two
    /// `Vec<Condition>` slots (precondition + postcondition) and
    /// compose the union via a two-step-short-circuit walk over
    /// [`ConditionKind::ALL`] under a NEGATED
    /// [`Boundary::has_condition_kind`]. Sweep `ConditionKind::ALL ×
    /// ConditionKind::ALL` so a regression that dropped the negation,
    /// dropped the second-hit short-circuit, or diverged the witness
    /// scalar from the Boolean predicate surfaces HERE. Also pins the
    /// composition laws `unique_missing_*_kind().is_some() ==
    /// has_unique_missing_*_kind()` and (on the `Some` arm)
    /// `unique_missing_*_kind() == first_missing_*_kind() ==
    /// last_missing_*_kind()` at each arm.
    #[test]
    fn unique_missing_condition_kind_triad_delegates_to_slice_unique_missing_kind() {
        // Empty boundary — every arm returns None on N ≥ 2 (every
        // kind missing, not exactly 1).
        let b = Boundary::default();
        if ConditionKind::ALL.len() >= 2 {
            assert_eq!(
                b.unique_missing_precondition_kind(),
                None,
                "empty boundary must return None on unique_missing_precondition_kind \
                 on any N ≥ 2 closed set",
            );
            assert_eq!(
                b.unique_missing_postcondition_kind(),
                None,
                "empty boundary must return None on unique_missing_postcondition_kind \
                 on any N ≥ 2 closed set",
            );
            assert_eq!(
                b.unique_missing_condition_kind(),
                None,
                "empty boundary must return None on unique_missing_condition_kind \
                 on any N ≥ 2 closed set",
            );
        }
        ...
        for pre_kind in ConditionKind::ALL {
            for post_kind in ConditionKind::ALL {
                ...
                assert_eq!(
                    b.unique_missing_precondition_kind(),
                    b.preconditions.unique_missing_kind(),
                    "Boundary::unique_missing_precondition_kind must delegate verbatim to \
                     preconditions.unique_missing_kind() for pre={pre_kind:?} post={post_kind:?}",
                );
                assert_eq!(
                    b.unique_missing_postcondition_kind(),
                    b.postconditions.unique_missing_kind(),
                    "Boundary::unique_missing_postcondition_kind must delegate verbatim to \
                     postconditions.unique_missing_kind() for pre={pre_kind:?} post={post_kind:?}",
                );

                // Union arm: Some(k) iff the union is missing exactly one kind.
                let missing: Vec<ConditionKind> = ConditionKind::ALL
                    .into_iter()
                    .filter(|k| pre_kind != *k && post_kind != *k)
                    .collect();
                let expected_union = if missing.len() == 1 {
                    Some(missing[0])
                } else {
                    None
                };
                assert_eq!(
                    b.unique_missing_condition_kind(),
                    expected_union,
                    "Boundary::unique_missing_condition_kind must equal Some(k) iff the \
                     ALL-entries NOT covered by either half-slice sum to exactly one for \
                     pre={pre_kind:?} post={post_kind:?}",
                );

                // Boolean-witness composition laws.
                assert_eq!(
                    b.unique_missing_condition_kind().is_some(),
                    b.has_unique_missing_condition_kind(),
                    "unique_missing_condition_kind().is_some() must equal \
                     has_unique_missing_condition_kind() for pre={pre_kind:?} post={post_kind:?}",
                );
                assert_eq!(
                    b.unique_missing_precondition_kind().is_some(),
                    b.has_unique_missing_precondition_kind(),
                    "unique_missing_precondition_kind().is_some() must equal \
                     has_unique_missing_precondition_kind() for pre={pre_kind:?} post={post_kind:?}",
                );
                assert_eq!(
                    b.unique_missing_postcondition_kind().is_some(),
                    b.has_unique_missing_postcondition_kind(),
                    "unique_missing_postcondition_kind().is_some() must equal \
                     has_unique_missing_postcondition_kind() for pre={pre_kind:?} post={post_kind:?}",
                );

                // Endpoint-agreement — on Some, first == last == unique.
                if let Some(k) = b.unique_missing_condition_kind() {
                    assert_eq!(
                        b.first_missing_condition_kind(),
                        Some(k),
                        "unique_missing_condition_kind() Some arm must equal \
                         first_missing_condition_kind() for pre={pre_kind:?} post={post_kind:?}",
                    );
                    assert_eq!(
                        b.last_missing_condition_kind(),
                        Some(k),
                        "unique_missing_condition_kind() Some arm must equal \
                         last_missing_condition_kind() for pre={pre_kind:?} post={post_kind:?}",
                    );
                }
            }
        }

        // Saturated boundary — every arm returns None (zero missing).
        let mut b = Boundary::default();
        for k in ConditionKind::ALL {
            b.preconditions.push(condition_with(k));
            b.postconditions.push(condition_with(k));
        ...
        assert_eq!(
            b.unique_missing_precondition_kind(),
            None,
            "saturated boundary must return None on unique_missing_precondition_kind",
        );
        assert_eq!(
            b.unique_missing_postcondition_kind(),
            None,
            "saturated boundary must return None on unique_missing_postcondition_kind",
        );
        assert_eq!(
            b.unique_missing_condition_kind(),
            None,
            "saturated boundary must return None on unique_missing_condition_kind",
        );

        // Near-saturation arm: exactly one ALL entry missing on each
        // side (achieved by populating every kind except `hole`).
        for hole in ConditionKind::ALL {
            let mut b = Boundary::default();
            for k in ConditionKind::ALL {
                if k != hole {
                    b.preconditions.push(condition_with(k));
            ...
            assert_eq!(
                b.unique_missing_precondition_kind(),
                Some(hole),
                "near-saturation boundary must return Some(hole={hole:?}) on unique_missing_precondition_kind",
            );
            assert_eq!(
                b.unique_missing_postcondition_kind(),
                Some(hole),
                "near-saturation boundary must return Some(hole={hole:?}) on unique_missing_postcondition_kind",
            );
            assert_eq!(
                b.unique_missing_condition_kind(),
                Some(hole),
    ...
    /// Empty slice pins — `position_of_kind` returns `None` for every
    /// kind, and both composition-law projections
    /// (`position.is_some() == has_kind`, `position.map(&slice[_]) ==
    /// find_kind`) hold trivially on the None-arm.
    #[test]
    fn condition_slice_position_of_kind_returns_none_on_empty_slice_for_every_kind() {
        let slice: &[Condition] = &[];
        for kind in ConditionKind::ALL {
            let pos = slice.position_of_kind(kind);
            assert!(
                pos.is_none(),
                "empty slice must return None for position_of_kind({kind:?})",
            );
            // Composition-law pins on the trivial arm.
            assert_eq!(
                pos.is_some(),
                slice.has_kind(kind),
                "empty slice position_of_kind({kind:?}).is_some() must equal has_kind",
            );
            assert_eq!(
                pos.map(|i| &slice[i] as *const Condition),
                slice.find_kind(kind).map(|c| c as *const Condition),
                "empty slice position_of_kind({kind:?}).map(&slice[_]) must equal find_kind",
            );
        }
    }

    /// Singleton + mixed + interleaved-duplicate sweeps — pin that
    /// `position_of_kind` returns the EARLIEST slice index, coincides
    /// with `find_kind` under the value-vs-index projection, and
    /// matches `.iter().position(|c| c.kind == k)` byte-for-byte on
    /// every authored arrangement.
    #[test]
    fn condition_slice_position_of_kind_witnesses_earliest_index_and_pins_composition_laws() {
        // Singleton sweep — one condition per kind, at slice index 0.
        for populated in ConditionKind::ALL {
            let slice: [Condition; 1] = [condition_with(populated)];
            for query in ConditionKind::ALL {
                let pos = slice.position_of_kind(query);
                if query == populated {
                    assert_eq!(
                        pos,
                        Some(0),
                        "singleton position_of_kind({query:?}) must equal Some(0) on \
                         populated-{populated:?}",
                    );
                } else {
                    assert!(
                        pos.is_none(),
                        "singleton position_of_kind({query:?}) must be None on \
                         populated-{populated:?} for non-matching query",
                    );
                }
                // Composition-law pins vs sibling primitives.
                assert_eq!(
                    pos.is_some(),
                    slice.has_kind(query),
                    "singleton position_of_kind({query:?}).is_some() drifted from has_kind",
                );
                assert_eq!(
                    pos.map(|i| &slice[i] as *const Condition),
                    slice.find_kind(query).map(|c| c as *const Condition),
                    "singleton position_of_kind({query:?}).map(&slice[_]) drifted from find_kind",
                );
                assert_eq!(
                    pos,
                    slice.iter().position(|c| c.kind == query),
                    "singleton position_of_kind({query:?}) drifted from .iter().position",
                );
            }
        }

        // Interleaved-duplicate sweep — pin EARLIEST index binding.
        for a in ConditionKind::ALL {
            for b in ConditionKind::ALL {
                if a == b {
                    continue;
                ...
                let slice: [Condition; 3] =
                    [condition_with(a), condition_with(b), condition_with(a)];
                assert_eq!(
                    slice.position_of_kind(a),
                    Some(0),
                    "interleaved position_of_kind(a={a:?}) must equal Some(0) — EARLIEST match",
                );
                assert_eq!(
                    slice.position_of_kind(b),
                    Some(1),
                    "interleaved position_of_kind(b={b:?}) must equal Some(1)",
                );
                // Composition-law pin: position witness projects to
                // find_kind's value witness at every arm.
                for query in ConditionKind::ALL {
                    assert_eq!(
                        slice
                            .position_of_kind(query)
                            .map(|i| &slice[i] as *const Condition),
                        slice.find_kind(query).map(|c| c as *const Condition),
                        "interleaved position_of_kind({query:?}).map(&slice[_]) drifted from \
                         find_kind for a={a:?} b={b:?}",
                    );
                }
            }
        }
    ...
    /// arrangement of a single-condition-per-side spec, asserts each
    /// per-slice arm delegates verbatim to
    /// [`ConditionSliceExt::position_of_kind`], and pins the union arm
    /// against the `or_else` composition of the two half-slice arms.
    /// Composition laws pinned:
    /// `position_of_condition_kind(k).is_some() == has_condition_kind(k)`,
    /// `position_of_precondition_kind(k).map(&preconditions[_]) ==
    ///  find_precondition_kind(k)`,
    /// `position_of_condition_kind(k) ==
    ///  position_of_precondition_kind(k).or_else(||
    ///  position_of_postcondition_kind(k))`.
    #[test]
    fn position_of_condition_kind_triad_delegates_to_slice_position_of_kind() {
        ...
        for kind in ConditionKind::ALL {
            assert!(
                b.position_of_precondition_kind(kind).is_none(),
                "empty boundary position_of_precondition_kind({kind:?}) must be None",
            );
            assert!(
                b.position_of_postcondition_kind(kind).is_none(),
                "empty boundary position_of_postcondition_kind({kind:?}) must be None",
            );
            assert!(
                b.position_of_condition_kind(kind).is_none(),
                "empty boundary position_of_condition_kind({kind:?}) must be None",
            );
        }

        // Single-populated-per-side sweep. Per-slice arms match slice
        // primitive on pointer identity. Union arm follows the
        // pre-then-post or_else order.
        for pre_kind in ConditionKind::ALL {
            for post_kind in ConditionKind::ALL {
                let mut b = Boundary::default();
                b.preconditions.push(condition_with(pre_kind));
                ...
                for query in ConditionKind::ALL {
                    // Delegation pins — per-slice arms must equal the
                    // slice primitive.
                    assert_eq!(
                        b.position_of_precondition_kind(query),
                        b.preconditions.position_of_kind(query),
                        "Boundary::position_of_precondition_kind must delegate verbatim to \
                         preconditions.position_of_kind for pre={pre_kind:?} post={post_kind:?} \
                         query={query:?}",
                    );
                    assert_eq!(
                        b.position_of_postcondition_kind(query),
                        b.postconditions.position_of_kind(query),
                        "Boundary::position_of_postcondition_kind must delegate verbatim to \
                    ...
                    // two half-slice arms.
                    let union = b.position_of_condition_kind(query);
                    let via_or_else = b
                        .position_of_precondition_kind(query)
                        .or_else(|| b.position_of_postcondition_kind(query));
                    assert_eq!(
                        union, via_or_else,
                    ...
                    // Per-slice arm value-vs-index composition-law pin.
                    let pre_via_pos = b
                        .position_of_precondition_kind(query)
                        .map(|i| &b.preconditions[i] as *const Condition);
                    let pre_via_find = b
                        .find_precondition_kind(query)
                        .map(|c| c as *const Condition);
                    assert_eq!(
                        pre_via_pos, pre_via_find,
                        "position_of_precondition_kind({query:?}).map(&pre[_]) drifted from \
                         find_precondition_kind for pre={pre_kind:?} post={post_kind:?}",
                    );
                    let post_via_pos = b
                        .position_of_postcondition_kind(query)
                        .map(|i| &b.postconditions[i] as *const Condition);
                    let post_via_find = b
                        .find_postcondition_kind(query)
                        .map(|c| c as *const Condition);
                    assert_eq!(
                        post_via_pos, post_via_find,
                        "position_of_postcondition_kind({query:?}).map(&post[_]) drifted from \
                         find_postcondition_kind for pre={pre_kind:?} post={post_kind:?}",
                    );
                }
            }
        }
        ...
        for doubled in ConditionKind::ALL {
            ...
            b.postconditions.push(condition_with(doubled));
            assert_eq!(
                b.position_of_postcondition_kind(doubled),
                Some(0),
                "doubled-post position_of_postcondition_kind({doubled:?}) must equal Some(0)",
            );
            assert_eq!(
                b.position_of_condition_kind(doubled),
                Some(0),
