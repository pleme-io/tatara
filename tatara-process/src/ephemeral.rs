...
impl EphemeralSpec {
    ...
    /// `preconditions ∪ postconditions`, or `None` when the union
    /// covers 0 or ≥ 2 kinds — the peer of
    /// [`crate::boundary::Boundary::unique_distinct_condition_kind`]
    /// on the [`EphemeralSpec`] sugar surface.
    ///
    /// # Composed body — byte-identical to
    /// [`crate::boundary::Boundary::unique_distinct_condition_kind`]
    ...
    /// else `None`. Byte-identical to the peer method on the point-
    /// domain [`crate::boundary::Boundary`] surface — both compose
    /// against the SAME slice-level substrate primitive
    /// [`crate::boundary::ConditionSliceExt::unique_distinct_kind`]
    /// via the two-slice union so a regression at the per-slice
    /// singleton-coverage witnessing walk fails at that primitive's
    /// tests rather than as silent drift at either struct-level
    /// singleton-coverage caller.
    ///
    /// # Sibling to [`Self::has_unique_distinct_condition_kind`]
    ///
    /// Witnessing scalar peer of the Boolean cardinality-mid-endpoint
    /// predicate at the SAME two-step short-circuit shape — where
    /// `has_unique_distinct_condition_kind` returns `true` iff the
    /// union covers exactly one kind, `unique_distinct_condition_kind`
    /// returns `Some(k)` naming that SOLE covered kind (composition
    /// law `unique_distinct_condition_kind().is_some() ==
    /// has_unique_distinct_condition_kind()` binds the two).
    ///
    /// Theory anchor: THEORY.md §II.1 invariant 5 (composition
    /// preserves proofs — the exactly-one-hit witnessing projection
    /// composes the SAME two-step short-circuit walk on both this
    /// ephemeral surface and the point-domain
    /// [`crate::boundary::Boundary`] surface). THEORY.md §VI.1
    /// (generation over composition — a future [`ConditionKind`]
    /// variant added to `ALL` reaches both surfaces' unique-distinct-
    /// kind triads mechanically through the SAME two-step short-
    /// circuit walk).
    #[must_use]
    ...
    /// triad on [`EphemeralSpec`] on the closed-set-inversion axis.
    /// Thin typed delegate to
    /// [`crate::boundary::ConditionSliceExt::unique_distinct_kind`]
    /// over [`Self::preconditions`].
    ///
    /// Peer of
    /// [`crate::boundary::Boundary::unique_distinct_precondition_kind`]
    /// on the point-domain surface — both peers compose against the
    /// SAME slice-level substrate primitive so a regression at the
    /// per-slice two-step short-circuit witnessing walk fails at that
    /// primitive's tests rather than as silent drift at either
    /// struct-level arm.
    #[must_use]
    ...
    /// The SOLE [`ConditionKind::ALL`] variant covered by
    /// [`Self::postconditions`], or `None` when postconditions cover
    /// 0 or ≥ 2 kinds — the postcondition-side arm of the
    /// (precondition, postcondition, condition-union) exactly-one-hit
    /// witnessing triad on [`EphemeralSpec`] on the closed-set-
    /// inversion axis. Thin typed delegate to
    /// [`crate::boundary::ConditionSliceExt::unique_distinct_kind`]
    /// over [`Self::postconditions`].
    ///
    /// Peer of
    /// [`crate::boundary::Boundary::unique_distinct_postcondition_kind`]
    /// on the point-domain surface. See
    /// [`Self::unique_distinct_precondition_kind`] for the full
    /// rationale — the two methods share ONE lift motivation, ONE
    /// fail-before-pass-after composition-law pin, and ONE two-
    /// surface parity contract with the point-domain
    /// [`crate::boundary::Boundary`] unique-distinct-kind peer methods.
    #[must_use]
    ...
    /// [`ConditionKind::ALL`] variant — the peer of
    /// [`crate::boundary::Boundary::is_condition_kind_empty`] on the
    /// [`EphemeralSpec`] sugar surface. Names the cardinality zero-
    /// endpoint on the closed-set-inversion axis at the union struct
    /// layer.
    ///
    /// # Composed body — byte-identical to
    /// [`crate::boundary::Boundary::is_condition_kind_empty`]
    ///
    /// `!self.has_any_distinct_condition_kind()` — a definitional
    /// negation of the at-least-one halfspace union primitive. Byte-
    /// identical to the peer method on the point-domain
    /// [`crate::boundary::Boundary`] surface — both compose against
    /// the SAME slice-level substrate primitive
    /// [`crate::boundary::ConditionSliceExt::is_kind_empty`] via the
    /// two-slice union so a regression at the per-slice zero-endpoint
    /// short-circuit fails at that primitive's tests rather than as
    /// silent drift at either struct-level empty caller.
    ///
    /// # Sibling to [`Self::is_condition_kind_saturated`]
    ///
    /// Axis-parity mirror of the closed-set-complement saturation-
    /// endpoint peer at the union struct layer — where
    /// `is_condition_kind_saturated` tests "every kind PRESENT across
    /// the union", this primitive tests "no kind PRESENT across the
    /// union". Both name a cardinality-endpoint on their respective
    /// axis under the same union struct layer.
    ///
    /// # Sibling to [`Self::distinct_condition_kinds`] /
    /// [`Self::distinct_condition_kind_count`]
    ///
    /// Cardinality zero-endpoint Boolean projection of the widened +
    /// scalar closed-set-inversion primitives on the ephemeral-union
    /// surface — where those primitives return the FULL distinct SET
    /// and its cardinality, `is_condition_kind_empty` collapses either
    /// the widened primitive to its emptiness Boolean or the scalar to
    /// its `== 0` cardinality-endpoint Boolean.
    ///
    /// Theory anchor: THEORY.md §II.1 invariant 5 (composition
    ...
    /// [`ConditionKind::ALL`] variant — the precondition-side arm of
    /// the (precondition, postcondition, condition-union) cardinality
    /// zero-endpoint triad on [`EphemeralSpec`]. Thin typed delegate to
    /// [`crate::boundary::ConditionSliceExt::is_kind_empty`] over
    /// [`Self::preconditions`].
    ///
    /// Peer of
    /// [`crate::boundary::Boundary::is_precondition_kind_empty`] on the
    /// point-domain surface — both peers compose against the SAME
    ...
    ///
    /// Peer of
    /// [`crate::boundary::Boundary::is_postcondition_kind_empty`] on
    /// the point-domain surface. See
    /// [`Self::is_precondition_kind_empty`] for the full rationale —
    /// the two methods share ONE lift motivation, ONE fail-before-pass-
    /// after composition-law pin, and ONE two-surface parity contract
    /// with the point-domain [`crate::boundary::Boundary`] cardinality
    /// zero-endpoint peer methods.
    #[must_use]
    ...
    /// missing 0 or ≥ 2 kinds — the peer of
    /// [`crate::boundary::Boundary::unique_missing_condition_kind`]
    /// on the [`EphemeralSpec`] sugar surface.
    ///
    /// # Composed body — byte-identical to
    /// [`crate::boundary::Boundary::unique_missing_condition_kind`]
    ...
    /// [`None`], else `None`. Byte-identical to the peer method on the
    /// point-domain [`crate::boundary::Boundary`] surface — both
    /// compose against the SAME slice-level substrate primitive
    /// [`crate::boundary::ConditionSliceExt::unique_missing_kind`] via
    /// the two-slice union so a regression at the per-slice near-
    /// saturation witnessing walk fails at that primitive's tests
    /// rather than as silent drift at either struct-level near-
    /// saturation caller.
    ///
    ...
    ///
    /// Witnessing scalar peer of the Boolean cardinality-mid-endpoint
    /// predicate on the missing axis at the SAME two-step short-
    /// circuit shape — where `has_unique_missing_condition_kind`
    /// returns `true` iff the union is one kind AWAY from covered,
    /// `unique_missing_condition_kind` returns `Some(k)` naming that
    /// SOLE remaining hole (composition law
    /// `unique_missing_condition_kind().is_some() ==
    /// has_unique_missing_condition_kind()` binds the two).
    ///
    /// # Compounding
    ///
    /// An operator-facing "one dependency still unfulfilled: X" gap-
    /// analysis diagnostic on an ephemeral env reads
    /// `spec.unique_missing_condition_kind()` at ONE call site — the
    /// WITNESS + the exactly-one predicate composed at ONE short-
    /// circuit walk, rather than pairing the Boolean
    /// [`Self::has_unique_missing_condition_kind`] with
    /// [`Self::first_missing_condition_kind`] at TWO independent
    /// walks.
    ///
    /// Theory anchor: THEORY.md §II.1 invariant 5 (composition
    /// preserves proofs — the complement-exactly-one-hit witnessing
    /// projection composes the SAME two-step short-circuit walk on
    /// both this ephemeral surface and the point-domain
    /// [`crate::boundary::Boundary`] surface under a negated predicate).
    /// THEORY.md §VI.1 (generation over composition — a future
    /// [`ConditionKind`] variant added to `ALL` reaches both surfaces'
    /// unique-missing-kind triads mechanically through the SAME two-
    /// step short-circuit walk).
    #[must_use]
    ...
    /// missing witnessing triad on [`EphemeralSpec`]. Thin typed
    /// delegate to
    /// [`crate::boundary::ConditionSliceExt::unique_missing_kind`]
    /// over [`Self::preconditions`].
    ///
    /// Peer of
    /// [`crate::boundary::Boundary::unique_missing_precondition_kind`]
    /// on the point-domain surface — both peers compose against the
    /// SAME slice-level substrate primitive so a regression at the
    /// per-slice two-step short-circuit witnessing walk under negation
    ...
    ///
    /// Peer of
    /// [`crate::boundary::Boundary::unique_missing_postcondition_kind`]
    /// on the point-domain surface. See
    /// [`Self::unique_missing_precondition_kind`] for the full
    /// rationale — the two methods share ONE lift motivation, ONE
    /// fail-before-pass-after composition-law pin, and ONE two-surface
    /// parity contract with the point-domain
    /// [`crate::boundary::Boundary`] unique-missing-kind peer methods.
    #[must_use]
    ...
    /// mid-endpoint triad on [`EphemeralSpec`]. Composes a two-step-
    /// short-circuit walk over the chained per-kind iterator
    /// [`Self::iter_condition_kind`], which itself chains
    /// [`crate::boundary::ConditionSliceExt::iter_kind`] over
    /// [`Self::preconditions`] then [`Self::postconditions`].
    ///
    /// Peer of
    /// [`crate::boundary::Boundary::has_unique_of_condition_kind`]
    /// on the point-domain surface — both peers compose against the
    /// SAME slice-level substrate primitive
    /// [`crate::boundary::ConditionSliceExt::has_unique_of_kind`]
    /// via the two-slice chain, so a regression at the per-slice
    /// two-step short-circuit walk fails at that primitive's tests
    /// rather than as silent drift at either struct-level arm.
    /// Middle arm of the {= 0, = 1, ≥ 2} per-kind cardinality
    /// Boolean trichotomy at the union level; alongside
    /// [`Self::lacks_condition_kind`] (= 0) and
    /// [`Self::has_multiple_of_condition_kind`] (≥ 2), the three
    /// Booleans PARTITION every non-negative multiplicity.
    #[must_use]
    ...
    /// — the precondition-side arm of the (precondition,
    /// postcondition, condition-union) per-kind cardinality "= 1"
    /// mid-endpoint triad on [`EphemeralSpec`]. Thin typed delegate
    /// to [`crate::boundary::ConditionSliceExt::has_unique_of_kind`]
    /// over [`Self::preconditions`].
    ///
    /// Peer of
    /// [`crate::boundary::Boundary::has_unique_of_precondition_kind`]
    /// on the point-domain surface — both peers compose against the
    /// SAME slice-level substrate primitive so a regression at the
    /// per-slice two-step short-circuit walk fails at that
    /// primitive's tests rather than as silent drift at either
    ...
    ///
    /// Peer of
    /// [`crate::boundary::Boundary::has_unique_of_postcondition_kind`]
    /// on the point-domain surface. See
    /// [`Self::has_unique_of_precondition_kind`] for the full
    /// rationale — the two methods share ONE lift motivation, ONE
    /// fail-before-pass-after composition-law pin, and ONE two-
    /// surface parity contract with the point-domain
    /// [`crate::boundary::Boundary`] per-kind-mid-endpoint peer
    /// methods.
    #[must_use]
...
mod tests {
    ...
    /// SUBSTRATE-DELEGATION pin (EphemeralSpec cardinality zero-
    /// endpoint triad) — the three `is_*_condition_kind_empty` methods
    /// on [`EphemeralSpec`] delegate to the slice-level substrate
    /// primitive
    /// [`crate::boundary::ConditionSliceExt::is_kind_empty`] over the
    ...
    /// [`EphemeralSpec::has_any_distinct_condition_kind`]. Two-surface
    /// parity pin against
    /// [`crate::boundary::Boundary::is_condition_kind_empty`] on the
    /// point-domain [`ProcessSpec`] surface — the two struct-level
    /// zero-endpoint callers compose against the SAME slice-level
    /// substrate primitive so a regression at the per-slice zero-
    /// endpoint short-circuit fails at that primitive's tests rather
    /// than as silent drift at either sugar-surface arm.
    #[test]
    fn is_condition_kind_empty_triad_delegates_to_slice_is_kind_empty() {
        ...
        let spec = empty_ephemeral();
        assert!(
            spec.is_precondition_kind_empty(),
            "empty ephemeral must return true on is_precondition_kind_empty",
        );
        assert!(
            spec.is_postcondition_kind_empty(),
            "empty ephemeral must return true on is_postcondition_kind_empty",
        );
        assert!(
            spec.is_condition_kind_empty(),
            "empty ephemeral must return true on is_condition_kind_empty",
        );

        // Two-surface parity: EphemeralSpec's three arms are byte-for-
        // byte equal to the lowered ProcessSpec's Boundary arms across
        // every sweep arm.
        let lowered: ProcessSpec = spec.clone().into();
        assert_eq!(
            spec.is_precondition_kind_empty(),
            lowered.boundary.is_precondition_kind_empty(),
            "empty ephemeral is_precondition_kind_empty must equal lowered Boundary is_precondition_kind_empty",
        );
        assert_eq!(
            spec.is_postcondition_kind_empty(),
            lowered.boundary.is_postcondition_kind_empty(),
            "empty ephemeral is_postcondition_kind_empty must equal lowered Boundary is_postcondition_kind_empty",
        );
        assert_eq!(
            spec.is_condition_kind_empty(),
            lowered.boundary.is_condition_kind_empty(),
            "empty ephemeral is_condition_kind_empty must equal lowered Boundary is_condition_kind_empty",
        );

        // Single-populated per side — every per-slice arm returns
        // false; the union always returns false.
        assert!(
        ...
        for pre_kind in ConditionKind::ALL {
            for post_kind in ConditionKind::ALL {
                ...
                assert_eq!(
                    spec.is_precondition_kind_empty(),
                    spec.preconditions.is_kind_empty(),
                    "EphemeralSpec::is_precondition_kind_empty must delegate verbatim to \
                     preconditions.is_kind_empty() for pre={pre_kind:?} post={post_kind:?}",
                );
                assert_eq!(
                    spec.is_postcondition_kind_empty(),
                    spec.postconditions.is_kind_empty(),
                    "EphemeralSpec::is_postcondition_kind_empty must delegate verbatim to \
                ...
                assert_eq!(
                    spec.is_condition_kind_empty(),
                    !spec.has_any_distinct_condition_kind(),
                    "EphemeralSpec::is_condition_kind_empty must equal !has_any_distinct_condition_kind() for pre={pre_kind:?} post={post_kind:?}",
                );
                ...
                assert_eq!(
                    spec.is_precondition_kind_empty(),
                    lowered.boundary.is_precondition_kind_empty(),
                    "EphemeralSpec::is_precondition_kind_empty must equal lowered Boundary::is_precondition_kind_empty for pre={pre_kind:?} post={post_kind:?}",
                );
                assert_eq!(
                    spec.is_postcondition_kind_empty(),
                    lowered.boundary.is_postcondition_kind_empty(),
                    "EphemeralSpec::is_postcondition_kind_empty must equal lowered Boundary::is_postcondition_kind_empty for pre={pre_kind:?} post={post_kind:?}",
                );
                assert_eq!(
                    spec.is_condition_kind_empty(),
                    lowered.boundary.is_condition_kind_empty(),
                    "EphemeralSpec::is_condition_kind_empty must equal lowered Boundary::is_condition_kind_empty for pre={pre_kind:?} post={post_kind:?}",
                );
            }
        }

        // Saturated ephemeral spec — every arm returns false on N ≥ 1
        // (every kind PRESENT across the union, not = 0).
        let mut spec = empty_ephemeral();
        for k in ConditionKind::ALL {
            spec.preconditions.push(cond(k));
            spec.postconditions.push(cond(k));
        ...
        assert!(
            !spec.is_precondition_kind_empty(),
            "saturated ephemeral must return false on is_precondition_kind_empty",
        );
        assert!(
            !spec.is_postcondition_kind_empty(),
            "saturated ephemeral must return false on is_postcondition_kind_empty",
        );
        assert!(
            !spec.is_condition_kind_empty(),
    ...
    /// SUBSTRATE-DELEGATION pin (EphemeralSpec unique-distinct-kind
    /// witnessing triad on the closed-set-inversion axis) — the three
    /// `unique_distinct_*_condition_kind` methods on [`EphemeralSpec`]
    /// delegate to the slice-level substrate primitive
    /// [`crate::boundary::ConditionSliceExt::unique_distinct_kind`]
    /// over the two `Vec<Condition>` slots (precondition +
    /// postcondition) and compose the union via a two-step-short-
    ...
    /// [`crate::boundary::Boundary::unique_distinct_condition_kind`]
    /// on the point-domain [`ProcessSpec`] surface — the two struct-
    /// level singleton-coverage witnesses compose against the SAME
    /// slice-level substrate primitive so a regression at the per-
    ...
    fn unique_distinct_condition_kind_triad_delegates_to_slice_unique_distinct_kind() {
        ...
        let spec = empty_ephemeral();
        assert_eq!(
            spec.unique_distinct_precondition_kind(),
            None,
            "empty ephemeral must return None on unique_distinct_precondition_kind",
        );
        assert_eq!(
            spec.unique_distinct_postcondition_kind(),
            None,
            "empty ephemeral must return None on unique_distinct_postcondition_kind",
        );
        assert_eq!(
            spec.unique_distinct_condition_kind(),
            None,
            "empty ephemeral must return None on unique_distinct_condition_kind",
        );
        ...
        for pre_kind in ConditionKind::ALL {
            for post_kind in ConditionKind::ALL {
                ...
                assert_eq!(
                    spec.unique_distinct_precondition_kind(),
                    spec.preconditions.unique_distinct_kind(),
                    "EphemeralSpec::unique_distinct_precondition_kind must delegate verbatim to \
                     preconditions.unique_distinct_kind() for pre={pre_kind:?} post={post_kind:?}",
                );
                assert_eq!(
                    spec.unique_distinct_precondition_kind(),
                    Some(pre_kind),
                    "EphemeralSpec::unique_distinct_precondition_kind must equal Some(pre_kind) on \
                     single-populated preconditions for pre={pre_kind:?} post={post_kind:?}",
                );
                assert_eq!(
                    spec.unique_distinct_postcondition_kind(),
                    spec.postconditions.unique_distinct_kind(),
                    "EphemeralSpec::unique_distinct_postcondition_kind must delegate verbatim to \
                     postconditions.unique_distinct_kind() for pre={pre_kind:?} post={post_kind:?}",
                );
                assert_eq!(
                    spec.unique_distinct_postcondition_kind(),
                    Some(post_kind),
                    "EphemeralSpec::unique_distinct_postcondition_kind must equal Some(post_kind) on \
                ...
                assert_eq!(
                    spec.unique_distinct_condition_kind(),
                    expected_union,
                    "EphemeralSpec::unique_distinct_condition_kind must equal Some(k) iff the \
                ...
                assert_eq!(
                    spec.unique_distinct_condition_kind().is_some(),
                    spec.has_unique_distinct_condition_kind(),
                ...
                assert_eq!(
                    spec.unique_distinct_precondition_kind(),
                    lowered.boundary.unique_distinct_precondition_kind(),
                    "two-surface unique_distinct_precondition_kind parity drift for \
                     pre={pre_kind:?} post={post_kind:?}",
                );
                assert_eq!(
                    spec.unique_distinct_postcondition_kind(),
                    lowered.boundary.unique_distinct_postcondition_kind(),
                    "two-surface unique_distinct_postcondition_kind parity drift for \
                     pre={pre_kind:?} post={post_kind:?}",
                );
                assert_eq!(
                    spec.unique_distinct_condition_kind(),
                    lowered.boundary.unique_distinct_condition_kind(),
                    "two-surface unique_distinct_condition_kind parity drift for \
                     pre={pre_kind:?} post={post_kind:?}",
                );
            }
        }

        // Saturated ephemeral — every arm returns None on N ≥ 2.
        if ConditionKind::ALL.len() >= 2 {
            let mut spec = empty_ephemeral();
            for k in ConditionKind::ALL {
                spec.preconditions.push(cond(k));
            ...
            assert_eq!(
                spec.unique_distinct_precondition_kind(),
                None,
                "saturated ephemeral must return None on unique_distinct_precondition_kind",
            );
            assert_eq!(
                spec.unique_distinct_postcondition_kind(),
                None,
                "saturated ephemeral must return None on unique_distinct_postcondition_kind",
            );
            assert_eq!(
                spec.unique_distinct_condition_kind(),
                None,
                "saturated ephemeral must return None on unique_distinct_condition_kind",
            );
        }
    }

    /// SUBSTRATE-DELEGATION pin (EphemeralSpec unique-missing-kind
    /// witnessing triad on the closed-set-complement axis) — the three
    /// `unique_missing_*_condition_kind` methods on [`EphemeralSpec`]
    /// delegate to the slice-level substrate primitive
    /// [`crate::boundary::ConditionSliceExt::unique_missing_kind`]
    /// over the two `Vec<Condition>` slots (precondition +
    /// postcondition) and compose the union via a two-step-short-
    /// circuit walk over [`ConditionKind::ALL`] under a NEGATED
    /// [`EphemeralSpec::has_condition_kind`]. Two-surface parity pin
    /// against
    /// [`crate::boundary::Boundary::unique_missing_condition_kind`]
    /// on the point-domain [`ProcessSpec`] surface.
    #[test]
    fn unique_missing_condition_kind_triad_delegates_to_slice_unique_missing_kind() {
        // Empty ephemeral spec — every arm returns None on N ≥ 2
        // (every kind is missing, not exactly one).
        let spec = empty_ephemeral();
        if ConditionKind::ALL.len() >= 2 {
            assert_eq!(
                spec.unique_missing_precondition_kind(),
                None,
                "empty ephemeral must return None on unique_missing_precondition_kind on N ≥ 2",
            );
            assert_eq!(
                spec.unique_missing_postcondition_kind(),
                None,
                "empty ephemeral must return None on unique_missing_postcondition_kind on N ≥ 2",
            );
            assert_eq!(
                spec.unique_missing_condition_kind(),
                None,
                "empty ephemeral must return None on unique_missing_condition_kind on N ≥ 2",
            );
        }
        ...
        for pre_kind in ConditionKind::ALL {
            for post_kind in ConditionKind::ALL {
                ...
                assert_eq!(
                    spec.unique_missing_precondition_kind(),
                    spec.preconditions.unique_missing_kind(),
                    "EphemeralSpec::unique_missing_precondition_kind must delegate verbatim to \
                     preconditions.unique_missing_kind() for pre={pre_kind:?} post={post_kind:?}",
                );
                assert_eq!(
                    spec.unique_missing_postcondition_kind(),
                    spec.postconditions.unique_missing_kind(),
                    "EphemeralSpec::unique_missing_postcondition_kind must delegate verbatim to \
                ...
                assert_eq!(
                    spec.unique_missing_condition_kind(),
                    expected_union,
                    "EphemeralSpec::unique_missing_condition_kind must equal Some(k) iff the \
                ...
                // Boolean-witness composition laws.
                assert_eq!(
                    spec.unique_missing_condition_kind().is_some(),
                    spec.has_unique_missing_condition_kind(),
                ...
                assert_eq!(
                    spec.unique_missing_precondition_kind(),
                    lowered.boundary.unique_missing_precondition_kind(),
                    "two-surface unique_missing_precondition_kind parity drift for \
                     pre={pre_kind:?} post={post_kind:?}",
                );
                assert_eq!(
                    spec.unique_missing_postcondition_kind(),
                    lowered.boundary.unique_missing_postcondition_kind(),
                    "two-surface unique_missing_postcondition_kind parity drift for \
                     pre={pre_kind:?} post={post_kind:?}",
                );
                assert_eq!(
                    spec.unique_missing_condition_kind(),
                    lowered.boundary.unique_missing_condition_kind(),
                    "two-surface unique_missing_condition_kind parity drift for \
                     pre={pre_kind:?} post={post_kind:?}",
                );
            }
        }

        // Saturated ephemeral — every arm returns None (zero missing).
        let mut spec = empty_ephemeral();
        for k in ConditionKind::ALL {
            spec.preconditions.push(cond(k));
            spec.postconditions.push(cond(k));
        ...
        assert_eq!(
            spec.unique_missing_precondition_kind(),
            None,
            "saturated ephemeral must return None on unique_missing_precondition_kind",
        );
        assert_eq!(
            spec.unique_missing_postcondition_kind(),
            None,
            "saturated ephemeral must return None on unique_missing_postcondition_kind",
        );
        assert_eq!(
            spec.unique_missing_condition_kind(),
            None,
            "saturated ephemeral must return None on unique_missing_condition_kind",
        );

        // Near-saturation arm: exactly one ALL entry missing on each
        // side (populate every kind except `hole`).
        for hole in ConditionKind::ALL {
            ...
            assert_eq!(
                spec.unique_missing_precondition_kind(),
                Some(hole),
                "near-saturation ephemeral must return Some(hole={hole:?}) on unique_missing_precondition_kind",
            );
            assert_eq!(
                spec.unique_missing_postcondition_kind(),
                Some(hole),
                "near-saturation ephemeral must return Some(hole={hole:?}) on unique_missing_postcondition_kind",
            );
            assert_eq!(
                spec.unique_missing_condition_kind(),
                Some(hole),
    ...
    /// per-kind cardinality "= 1" mid-endpoint triad on
    /// [`EphemeralSpec`] delegates verbatim to the slice-level
    /// substrate primitive
    /// [`crate::boundary::ConditionSliceExt::has_unique_of_kind`].
    /// Sweeps three arrangements: (a) an empty ephemeral (every arm
    /// returns `false` on every kind); (b) a single-populated-per-
    /// side sweep where per-slice arms fire iff their side's kind
    /// equals `query`, and the union arm fires iff EXACTLY ONE of
    /// `{pre, post}` equals `query` (chain sums to 1 on disjoint,
    /// 2 on shared); (c) the saturated-singleton spec (every kind
    /// appears exactly once on every slice — per-slice arms return
    /// `true` on every kind, union returns `false` on every kind
    /// as `= 2` chain matches). Also verifies TWO-SURFACE PARITY —
    /// the ephemeral-side arm agrees with the lowered
    /// [`crate::boundary::Boundary`] arm through the same slice-
    ...
    fn has_unique_of_condition_kind_triad_delegates_to_slice_has_unique_of_kind() {
        ...
        for kind in ConditionKind::ALL {
            assert!(
                !spec.has_unique_of_precondition_kind(kind),
                "empty ephemeral must return false on has_unique_of_precondition_kind({kind:?})",
            );
            assert!(
                !spec.has_unique_of_postcondition_kind(kind),
                "empty ephemeral must return false on has_unique_of_postcondition_kind({kind:?})",
            );
            assert!(
                !spec.has_unique_of_condition_kind(kind),
                "empty ephemeral must return false on has_unique_of_condition_kind({kind:?})",
            );
        }

        // Single-populated-per-side sweep — per-slice arm fires iff
        // its side's kind equals `query`; union arm fires iff
        // EXACTLY ONE of {pre, post} equals `query`. Also verifies
        // two-surface parity with Boundary.
        for pre_kind in ConditionKind::ALL {
            for post_kind in ConditionKind::ALL {
                let mut spec = empty_ephemeral();
                spec.preconditions.push(cond(pre_kind));
                ...
                for query in ConditionKind::ALL {
                    assert_eq!(
                        spec.has_unique_of_precondition_kind(query),
                        spec.preconditions.has_unique_of_kind(query),
                        "EphemeralSpec::has_unique_of_precondition_kind must delegate \
                         verbatim to preconditions.has_unique_of_kind for \
                         pre={pre_kind:?} post={post_kind:?} query={query:?}",
                    );
                    assert_eq!(
                        spec.has_unique_of_postcondition_kind(query),
                        spec.postconditions.has_unique_of_kind(query),
                        "EphemeralSpec::has_unique_of_postcondition_kind must delegate \
                         verbatim to postconditions.has_unique_of_kind for \
                         pre={pre_kind:?} post={post_kind:?} query={query:?}",
                    );
                    ...
                    assert_eq!(
                        ...
                        expected_union,
                        "EphemeralSpec::has_unique_of_condition_kind({query:?}) must equal \
                         ((pre == query) XOR (post == query)) for \
                         pre={pre_kind:?} post={post_kind:?}",
                    );
                    ...
                    assert_eq!(
                        spec.has_unique_of_condition_kind(query),
                        lowered.boundary.has_unique_of_condition_kind(query),
                        "two-surface has_unique_of_condition_kind({query:?}) parity drift \
                         for pre={pre_kind:?} post={post_kind:?}",
                    );
                    assert_eq!(
                        spec.has_unique_of_precondition_kind(query),
                        lowered.boundary.has_unique_of_precondition_kind(query),
                        "two-surface has_unique_of_precondition_kind({query:?}) parity drift",
                    );
                    assert_eq!(
                        spec.has_unique_of_postcondition_kind(query),
                        lowered.boundary.has_unique_of_postcondition_kind(query),
                        "two-surface has_unique_of_postcondition_kind({query:?}) parity drift",
                    );
                }
            }
        }
        ...
        // union returns false on every kind (= 2 chain matches).
        let mut spec = empty_ephemeral();
        for k in ConditionKind::ALL {
            spec.preconditions.push(cond(k));
            spec.postconditions.push(cond(k));
        ...
        for kind in ConditionKind::ALL {
            assert!(
                spec.has_unique_of_precondition_kind(kind),
                "saturated-singleton ephemeral must return true on has_unique_of_precondition_kind({kind:?})",
            );
            assert!(
                spec.has_unique_of_postcondition_kind(kind),
                "saturated-singleton ephemeral must return true on has_unique_of_postcondition_kind({kind:?})",
            );
            assert!(
                !spec.has_unique_of_condition_kind(kind),
