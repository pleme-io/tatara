...
use tatara_process::classification::{
    Arity, CalmClassification, ConvergencePointType, DataClassification, HorizonKind,
...
/// [`tests::evaluate_point_require_tag_report_payload_shape_and_report_format_coexist_via_projection`],
...
static POINT_FIXED_TAG_ARMS: &[FixedTagArm<tatara_process::crd::ProcessSpec>] = &[
    ...
    // `data-restricted` — FIFTH occupant on the (parent × derived-
    // nullary-bool) corner of the workspace-wide fixed-tag algebra
    // after the two `horizon.*`, one `coordination-required`, and one
    // `data-regulated` arms opened + populated the corner; SECOND
    // occupant threading the classification-data axis after
    // `data-regulated` opened it. Composes the ONE substrate
    // primitive [`Classification::data_is_restricted`] that walks
    // `self.data_classification.is_restricted()` — the six-variant
    // [`DataClassification`] closed set's access-control-mandatory
    // bucket partition as a fixed-tag audit surface. Answers the
    // access-control-baseline-facing question "does this Process
    // require access controls beyond freely-distributable?" — `true`
    // on the `Internal` default (and on `Confidential | Pii | Phi |
    // Pci`), `false` only on `Public`. Byte-for-byte symmetrical with
    // the ephemeral surface's `data-restricted` arm on
    // [`EPHEMERAL_FIXED_TAG_ARMS`] via
    // [`tatara_process::ephemeral::EphemeralSpec::data_is_restricted`]
    // — both surfaces route through the SAME
    // [`Classification::data_is_restricted`] primitive after the
    // ephemeral surface pays ONE resolver hop. Byte-for-byte
    // structural peer of the sibling `data-regulated` arm above:
    // both walk the SAME direct scalar closed-set field
    // (`self.data_classification`) through TWO layers of `Default`
    // short-circuit but the answer flips on the Internal default —
    // `data-regulated` reads `false`, `data-restricted` reads `true`.
    // FIRST corner peer whose gate-compute point-surface baseline
    // projects to `true` (mirror-image of the ephemeral surface's
    // absent-classification default), and the FIRST corner pair whose
    // two arms carry a non-trivial closed-set-internal implication
    // (`data-regulated ⇒ data-restricted`, sealed at the closed-set
    // primitive by `data_classification_regulated_implies_restricted`
    // and at the parent-composed level by
    ...
    // `diffusive-point` — SEVENTH occupant on the (parent × derived-
    // nullary-bool) corner and SECOND arm threading the classification-
    // `point_type` axis. Composes the ONE substrate primitive
    // [`tatara_process::classification::Classification::point_is_diffusive`]
    // that walks `self.point_type.is_diffusive()` — the eight-variant
    // [`ConvergencePointType`] closed set's 1→N fan-out topology-
    // bucket partition as a fixed-tag audit surface. Answers the DAG-
    // composition-facing question "is this Process a diffusive fan-out
    // (1→N shape) on its dataflow graph?" — `true` on `Fork |
    // Broadcast`, `false` on every other variant. Byte-for-byte
    // symmetrical with the ephemeral surface's `diffusive-point` arm
    // on [`EPHEMERAL_FIXED_TAG_ARMS`] via
    // [`tatara_process::ephemeral::EphemeralSpec::point_is_diffusive`]
    // — both surfaces route through the SAME
    // [`Classification::point_is_diffusive`] primitive after the
    // ephemeral surface pays ONE resolver hop. Byte-for-byte
    // structural peer of the sibling `endomorphic-point` arm above:
    // both walk the SAME direct scalar closed-set field
    // (`self.point_type`) whose carrier has NO `impl Default`, so
    // both gate-compute-baseline `false` answers are chosen-field
    // answers through the baseline's `point_type: Gate` field. FIRST
    // fixed-tag corner-peer pair on the `point_type` axis whose two
    // arms carry a non-trivial closed-set-internal MUTEX relationship
    // (`endomorphic-point ⇒ ¬diffusive-point`, sealed at the closed-
    // set primitive by
    ...
    // distinct from the sibling `data`-axis corner-peer pair whose
    // two arms carry a non-trivial IMPLICATION relationship. When
    // the third sibling `convergent-point` lands the mutex closes
    // into the three-way XOR partition composed through this corner
    // as a substrate-wide theorem.
    FixedTagArm {
        tag: "diffusive-point",
    ...
    // `resource-substrate` — NINTH occupant on the (parent × derived-
    // nullary-bool) corner of the workspace-wide fixed-tag algebra
    // after the two `horizon.*`, one `coordination-required`, two
    // `data-*`, and three `point-*` arms opened + populated the
    // corner; FIRST occupant threading the classification-`substrate`
    // axis (the fourth of six classification axes participating on
    // this corner). Composes the ONE substrate primitive
    // [`tatara_process::classification::Classification::substrate_is_resource`]
    // that walks `self.substrate.is_resource()` — the eight-variant
    // [`tatara_process::classification::SubstrateType`] closed set's
    // resource-plane bucket partition (Financial | Compute | Network
    // | Storage) as a fixed-tag audit surface. Answers the
    // budget-allocation-facing question "does this Process live on a
    // substrate you allocate budgets from (rather than a policy or
    // telemetry substrate)?" — `true` on the `Compute` default
    // (workspace baseline) and on `Financial | Network | Storage`,
    // `false` on `Security | Identity | Observability | Regulatory`.
    // Byte-for-byte symmetrical with the ephemeral surface's
    // `resource-substrate` arm on [`EPHEMERAL_FIXED_TAG_ARMS`] via
    // [`tatara_process::ephemeral::EphemeralSpec::substrate_is_resource`]
    // — both surfaces route through the SAME
    // [`Classification::substrate_is_resource`] primitive after the
    // ephemeral surface pays ONE resolver hop. Structural byte-for-
    // byte peer of the three sibling `point-*` arms above: all four
    // walk a DIRECT scalar closed-set field's derived projection
    // through TWO layers of `Default` short-circuit (the parent
    // `Classification` defaults, but the scalar child has no
    // `#[default]` — the `substrate` field on `gate_compute` is
    // chosen, not defaulted, just as the `point_type` field is).
    // FIRST substrate-axis peer opens the substrate axis on the
    // corner; siblings [`SubstrateType::is_policy`] and
    // [`SubstrateType::is_telemetry`] compose byte-identically as
    // future tenth + eleventh corner occupants that will close the
    // substrate axis into the FULL three-way XOR partition
    // `is_resource ⊕ is_policy ⊕ is_telemetry` sealed on the closed
    // set by `substrate_type_buckets_cover_every_variant`, exactly
...
static EPHEMERAL_FIXED_TAG_ARMS: &[FixedTagArm<tatara_process::ephemeral::EphemeralSpec>] = &[
    ...
    // `data-restricted` — byte-for-byte peer of the point surface's
    // `data-restricted` arm on [`POINT_FIXED_TAG_ARMS`] via
    // [`Classification::data_is_restricted`] reached through the
    // ephemeral surface's [`EphemeralSpec::resolved_classification`]
    // resolver. Answers the same access-control-baseline-facing
    // question on the ephemeral surface — `true` on the absent-
    // `:classification` default (routes through
    // [`Classification::gate_compute`] →
    // [`DataClassification::default = Internal`] → `is_restricted() =
    // true`) and on any operator-authored
    // `Internal | Confidential | Pii | Phi | Pci` classification,
    // `false` only on `Public`. The two-surface parity contract holds
    // by construction: both surfaces route through the SAME
    // [`Classification::data_is_restricted`] primitive after the
    // ephemeral surface pays ONE resolver hop. Byte-for-byte
    // structural peer of the sibling `data-regulated` arm on this
    // surface: both walk the SAME direct scalar closed-set field's
    // derived projection with TWO layers of `Default` short-circuit
    // but the answer flips on the Internal default — `data-regulated`
    // reads `false`, `data-restricted` reads `true`. FIRST direct-
    // scalar corner peer whose absent-classification ephemeral
    // baseline projects to `true` (mirror-image of the nested-struct
    // `terminating-horizon` arm which also projects `true` on absent
    // classification through THREE layers of `Default`).
    FixedTagArm {
        tag: "data-restricted",
    ...
    // `diffusive-point` — byte-for-byte peer of the point surface's
    // `diffusive-point` arm on [`POINT_FIXED_TAG_ARMS`] via
    // [`Classification::point_is_diffusive`] reached through the
    // ephemeral surface's [`EphemeralSpec::resolved_classification`]
    // resolver. Answers the same DAG-composition-facing question on
    // the ephemeral surface — `false` on the absent-`:classification`
    // default (routes through [`Classification::gate_compute`] →
    // `point_type = Gate` → `is_diffusive() = false`) and on any
    // operator-authored variant except [`ConvergencePointType::Fork`]
    // / [`ConvergencePointType::Broadcast`]. The two-surface parity
    // contract holds by construction: both surfaces route through the
    // SAME [`Classification::point_is_diffusive`] primitive after the
    // ephemeral surface pays ONE resolver hop. Byte-for-byte
    // structural peer of the sibling `endomorphic-point` arm on this
    // surface: both walk the SAME direct scalar closed-set field
    // (`self.point_type`) whose carrier has NO `impl Default`, so
    // both absent-`:classification` `false` answers are chosen-field
    // answers through [`Classification::gate_compute`]'s deliberate
    // `point_type: Gate` field rather than substrate-`#[default]`
    // short-circuits on the closed-set side. FIRST ephemeral-surface
    // fixed-tag corner-peer pair on the `point_type` axis whose two
    // arms carry a non-trivial closed-set-internal MUTEX relationship.
    FixedTagArm {
        tag: "diffusive-point",
    ...
    // `resource-substrate` — byte-for-byte peer of the point surface's
    // `resource-substrate` arm on [`POINT_FIXED_TAG_ARMS`] via
    // [`Classification::substrate_is_resource`] reached through the
    // resolver primitive
    // [`tatara_process::ephemeral::EphemeralSpec::resolved_classification`].
    // Composes the ONE ephemeral-surface substrate primitive
    // [`tatara_process::ephemeral::EphemeralSpec::substrate_is_resource`]
    // that walks
    // `self.resolved_classification().substrate.is_resource()`. Both
    // surfaces route through the SAME
    // [`Classification::substrate_is_resource`] primitive after the
    // ephemeral surface pays ONE resolver hop. FIRST ephemeral-
    // surface peer on the substrate axis — opens the axis on the
    // ephemeral surface exactly as the point-surface twin opened it
    // on the parent-composed surface. Sibling
    // [`SubstrateType::is_policy`] / [`SubstrateType::is_telemetry`]
    // projections will compose as future tenth + eleventh peers,
    // closing the axis into the FULL three-way XOR partition
    // contract exactly as the `point_type` axis was closed on this
    // surface by
...
///   [`tatara_process::export::ExportSpecSliceExt::has_report_format`]
///   on [`EphemeralSpec::exports`] directly. Peer of the point surface's
///   `report-format-<kind>` family (the ninth closed-set-driven prefix
///   family on the point surface, which composes the SAME slice-level
///   `ExportSpecSliceExt::has_report_format` substrate primitive through
///   the `Lifetime::resolved_ephemeral()` Option-parent gate); this
///   ephemeral peer routes through the SAME primitive with NO Option-
///   parent hop (contrast the point-surface routing
///   `spec.lifetime.resolved_ephemeral().is_some_and(|e| e.exports.has_report_format(kind))`).
///   The [`From<EphemeralSpec>`] lowering copies `e.exports →
///   EphemeralLifetime::exports` byte-for-byte (see
///   [`tatara_process::ephemeral`] `From` impl line 311), so the SAME
///   slice values reach both surfaces' `report-format-<kind>` families
///   through the SAME [`ExportSpecSliceExt::has_report_format`] walk —
///   a future fifth [`ReportFormat`] variant reaches BOTH surfaces
///   through the [`ReportFormat::ALL`] sweep + one `as_str` arm on the
///   closed set with no per-caller edit. FIFTH closed-set-driven prefix
///   family in the ephemeral require-tag vocabulary and THIRD
///   slice-child occupant routed DIRECTLY through the slice-level
///   substrate; distinct from the sibling `export-when-<kind>` and
///   `channel-<kind>` families on ONE dimension — the child projection
///   reads a NESTED-Option scalar
///   (`e.source.test_report.as_ref().is_some_and(|tr| tr.format == kind)`),
///   so a reachable-populated export whose `source.test_report` slot is
///   empty (e.g. a `receipts`-only source) answers `false` for EVERY
///   `report-format-<kind>` query INCLUDING the default
///   [`ReportFormat::Raw`] that a naive `unwrap_or_default()` projection
///   would spuriously match. Contrast the direct closed-set scalar
///   (`ExportSpec::when` on `export-when-<kind>`) and the outer
///   tagged-union carrier
///   ([`ChannelKind::from(&VectorChannel)`] on `channel-<kind>`) —
///   sibling on the same slice, distinct on the child-projection axis
///   for the SECOND time in the ephemeral vocabulary. Locks the
///   nested-Option-collapse contract at ONE substrate primitive: a
///   future normalization at
///   [`ExportSpecSliceExt::has_report_format`] (a widened return
///   carrying the matching `TestReportSource` ref, a debug-build
///   assertion on ambiguous source variants) lands at ONE site and
...
/// [`tests::evaluate_ephemeral_require_tag_channel_matches_point_peer_through_lowered_exports`],
/// [`tests::evaluate_ephemeral_require_tag_returns_true_on_populated_report_format_slot_per_kind`],
/// [`tests::evaluate_ephemeral_require_tag_returns_false_on_empty_exports_for_every_report_format_kind`],
...
/// [`tests::evaluate_ephemeral_require_tag_point_type_matches_point_peer_through_resolved_classification`],
/// [`tests::evaluate_ephemeral_require_tag_returns_true_iff_substrate_matches_authored_classification_per_kind`],
...
/// [`tests::evaluate_ephemeral_require_tag_calm_matches_point_peer_through_resolved_classification`],
/// [`tests::evaluate_ephemeral_require_tag_returns_true_iff_data_classification_matches_authored_classification_per_kind`],
...
/// [`tests::evaluate_ephemeral_require_tag_optimization_direction_matches_point_peer_through_resolved_classification`],
/// [`tests::evaluate_ephemeral_require_tag_returns_true_iff_input_arity_matches_authored_classification_per_kind`],
...
/// Assemble a [`CompiledSource`] from a compile-step's typed vec and a
/// non-capturing evaluator `fn(&S, &str) -> Result<bool, UnknownRequireTag>`.
/// Pre-lift both [`RequireTagDomain::compile`] impls (`PointDomain` +
/// `EphemeralDomain`) hand-authored the identical `(count = defs.len(),
/// classify = Box::new(move |tag| eval(&defs[0].spec, tag)))` shape past
/// the ★★ PRIME-DIRECTIVE ≥ 2 duplication threshold — post-lift ONE
/// substrate primitive owns the (count computation, first-def
/// selection, boxed classifier capture) discipline; both current
/// callers plus every future [`RequireTagDomain`] peer route through it.
///
/// Generic over the spec type `S: 'static` — `S` is bounded `'static`
/// so the owning `defs` vec (and the `move`-captured `&defs[0].spec`
/// borrow through the returned `Fn`) does not carry a lifetime past
/// the trait method's `Box<dyn Fn(...) + 'static>` return. The two
/// current callers (`ProcessSpec`, `EphemeralSpec`) both satisfy it,
/// and every future workspace-typed spec struct will too because a
/// domain's spec is by construction a `#[derive(TataraDomain)]` struct
/// with no lifetime parameters.
///
/// The `evaluate` slot is a `fn(&S, &str) -> Result<bool,
/// UnknownRequireTag>` — a non-capturing function pointer, not a
/// closure — so the boxing at the classify-closure boundary is the
/// ONLY heap allocation on the compile path, and a future peer's
/// evaluator (`evaluate_<newdomain>_require_tag`) plugs in without a
/// trait-object indirection.
///
/// # Compounding
///
/// A future third [`RequireTagDomain`] peer's `compile` impl reads as
/// `Ok(build_compiled_source(compile_<newdomain>_source(src)?,
/// evaluate_<newdomain>_require_tag))` — a two-line body binding to
/// the same substrate owner. A future diagnostic shift on the compile-
/// carrier shape (attaching the domain-name to the classify closure,
/// caching the first-def's name for a downstream `"({name}: … )"`
/// preamble, threading a per-definition index in place of the
/// hard-coded `[0]`) lands at THIS ONE substrate owner and every
/// current + future peer inherits the shift.
///
/// # Theory grounding
///
/// THEORY.md §II.1 invariant 2 — free middle; the (defs, evaluator)
/// pair is pure data at the call boundary, so the composition
/// (`count = defs.len()`, `classify = Box::new(move |tag| evaluator(
/// &defs[0].spec, tag))`) can never drift between peers. THEORY.md
/// §VI.1 — generation over composition; the classify closure is
/// generated once for every current + future domain rather than
/// hand-authored per-arm.
fn build_compiled_source<S: 'static>(
    defs: Vec<tatara_lisp::NamedDefinition<S>>,
    evaluate: fn(&S, &str) -> Result<bool, UnknownRequireTag>,
) -> CompiledSource {
    let count = defs.len();
    CompiledSource {
        count,
        classify: Box::new(move |tag| evaluate(&defs[0].spec, tag)),
    }
}
...
trait RequireTagDomain: Sync {
    ...
    /// The domain-specific operator-facing prose head — everything the
    /// pre-lift `"unknown :requires tag[...]"` `format!` string held
    /// LEFT of the `": {tag}{err}"` suffix. Point-domain reads
    /// `"unknown :requires tag"`; ephemeral-domain reads
    /// `"unknown :requires tag for ephemeral domain"`. The
    /// [`RequireTagDomain::unknown_tag_diagnostic`] default composes
    /// this prefix with the shared `": {tag}{err}"` suffix at ONE
    /// substrate owner, so a future third domain declares its head and
    /// inherits the suffix by construction — no per-domain `format!`
    /// restatement.
    fn diagnostic_prefix(&self) -> &'static str;
    /// Compose the operator-facing `"<diagnostic_prefix>: <tag><err>"`
    /// prose the pre-lift `check_lisp_compiles` executor's two arms
    /// hand-authored verbatim. Default body threads
    /// [`RequireTagDomain::diagnostic_prefix`] through the shared
    /// suffix; the [`UnknownRequireTag`] adornment renders through its
    /// [`std::fmt::Display`] impl (a parenthetical " (unknown <set>:
    /// 'suffix'; did you mean 'prefix-Suffix'?)" for a KNOWN-prefix +
    /// bad-suffix input, `" (did you mean \"<tag>\"?)"` for a fixed-tag
    /// near-miss, empty for a hint-free far-miss). Override only when a
    /// future domain's diagnostic needs a shape that does not decompose
    /// as `prefix + ": " + tag + err_display`.
    fn unknown_tag_diagnostic(&self, tag: &str, err: &UnknownRequireTag) -> String {
        format!("{}: {tag}{err}", self.diagnostic_prefix())
    }
}

/// [`RequireTagDomain`] impl for the point (ProcessSpec) surface —
/// wraps [`tatara_process::compile_source`] and
/// [`evaluate_point_require_tag`] and the pre-lift
/// `"unknown :requires tag: <tag>"` diagnostic prose (now generated at
/// the trait's default [`RequireTagDomain::unknown_tag_diagnostic`]
/// from [`PointDomain::diagnostic_prefix`]).
struct PointDomain;
...
impl RequireTagDomain for PointDomain {
    ...
    fn diagnostic_prefix(&self) -> &'static str {
        "unknown :requires tag"
    }
    fn compile(&self, src: &str) -> tatara_lisp::Result<CompiledSource> {
        Ok(build_compiled_source(
            tatara_process::compile_source(src)?,
            evaluate_point_require_tag,
        ))
    }
}

/// [`RequireTagDomain`] impl for the ephemeral (EphemeralSpec) surface
/// — wraps [`tatara_process::ephemeral::compile_ephemeral_source`] and
/// [`evaluate_ephemeral_require_tag`] and the pre-lift
/// `"unknown :requires tag for ephemeral domain: <tag>"` diagnostic
/// prose (now generated at the trait's default
/// [`RequireTagDomain::unknown_tag_diagnostic`] from
/// [`EphemeralDomain::diagnostic_prefix`]).
...
impl RequireTagDomain for EphemeralDomain {
    ...
    fn diagnostic_prefix(&self) -> &'static str {
        "unknown :requires tag for ephemeral domain"
    }
    fn compile(&self, src: &str) -> tatara_lisp::Result<CompiledSource> {
        Ok(build_compiled_source(
            tatara_process::ephemeral::compile_ephemeral_source(src)?,
            evaluate_ephemeral_require_tag,
        ))
    }
}
...
mod tests {
    use super::{
        build_compiled_source, check_yaml_parses, evaluate_ephemeral_require_tag,
        evaluate_fixed_tag, evaluate_point_require_tag, find_kw, find_kw_string_list,
        head_symbol_or_missing, known_require_tag_domain_names, min_defs_shortfall_msg,
        parse_kwargs, positional_string, read_or_fail, report_result_prefixed,
        require_tag_domain_by_name, required_positional_string, startup_diagnostic,
        strip_and_classify_prefixed_kind, FixedTagArm, FixedTagHint, PrefixHint, Report, TagHint,
        UnknownRequireTag, ALL_REQUIRE_TAG_DOMAINS, EPHEMERAL_FIXED_TAG_ARMS, MISSING_ARG_SLUG,
        POINT_FIXED_TAG_ARMS,
    };
    ...
    // ── evaluate_point_require_tag input-arity-<kind> pins ──────────
    //
    // TWENTY-THIRD closed-set-driven prefix family in the point-domain
    // require-tag vocabulary. Fail-before-pass-after: the
    // `input-arity-<kind>` prefix arm did not exist before this commit
    // — the derived-typed-projection walk through
    // `spec.classification.point_type.input_arity() == K` was not
    ...
    // twenty-two pre-existing prefix families AND with the twelfth
    // family (`point-type-<kind>`) with which it shares the SAME
    // `self.point_type` scalar carrier on [`Classification`]. The
    // distinction between the two on that shared carrier: twelfth
    // compares the raw 8-arm [`ConvergencePointType`] closed set;
    // twenty-third compares the derived 2-arm [`Arity`] reached
    // through the typed projection
    // [`ConvergencePointType::input_arity`] — a many-to-one collapse
    // (`Transform | Fork | Broadcast | Observe → One`, `Join | Gate
    // | Select | Reduce → Many`). The primitive is
    // [`tatara_process::classification::Classification::has_input_arity`],
    // published as the THIRD substrate method on the (required-parent
    // × nested-struct-scalar-child) corner of [`Classification`]
    // alongside `has_horizon_kind` + `has_optimization_direction`, and
    // the FIRST derived-typed-projection occupant on that corner.

    /// PROJECTION-TRUTH-TABLE pin — `input-arity-<kind>` dispatches
    /// through the autoderived [`Arity`] `FromStr` + the substrate
    /// [`tatara_process::classification::Classification::has_input_arity`]
    /// primitive, returning `true` only when the resolved
    /// [`ConvergencePointType`] projects (through
    /// [`ConvergencePointType::input_arity`]) to this arity. Sweep the
    ...
    /// disagrees on the fan-out arms `Fork | Broadcast`), (d)
    /// hard-coded the arm to a single arity (silently returning
    /// `true` for every populated classification regardless of query
    /// kind), or (e) wired the closure to a fixed unrelated field
    /// fails HERE at the classifier before landing at the operator-
    /// facing checks.lisp surface. The projection's many-to-one shape
    /// is pinned SYMMETRICALLY on both sides of the cross: `Transform`,
        ...
        for populated in ConvergencePointType::ALL {
            ...
            for query in Arity::ALL {
                ...
                assert_eq!(
                    evaluate_point_require_tag(&spec, &tag),
                    Ok(expected),
                    "point_type={populated:?} → input_arity={expected_arity:?}: tag {tag:?} classification drifted",
                );
            }
        }
    }

    /// UNKNOWN-suffix pin — `input-arity-<garbage>` classifies as
    /// [`UnknownRequireTag`] via the shared
    /// `strip_and_classify_prefixed_kind` primitive so the caller's
    /// operator-facing `unknown :requires tag: <verbatim>` diagnostic
    ...
    /// identifiers like `Fork` live on the twelfth family and MUST
    /// NOT resolve here through the shared `self.point_type` carrier).
    #[test]
    fn evaluate_point_require_tag_returns_unknown_on_unknown_input_arity_suffix() {
        let spec = ProcessSpec::gate_compute_defaults();
        for garbage in [
            "input-arity-one",
            "input-arity-many",
    ...
    /// BARE-prefix pin — `input-arity-` (the prefix alone, empty
    /// suffix) classifies as [`UnknownRequireTag`] via the shared
    /// `strip_and_classify_prefixed_kind` primitive's empty-suffix
    /// arm (pinned generically by
    /// [`strip_and_classify_prefixed_kind_returns_unknown_on_empty_suffix`]).
    /// This site pins the family's SPECIFIC bare-prefix boundary so a
    /// regression that special-cased `input-arity-` to fall through
    ...
    #[test]
    fn evaluate_point_require_tag_returns_unknown_on_bare_input_arity_prefix() {
        let spec = ProcessSpec::gate_compute_defaults();
        assert_eq!(
    ...
    /// SAME-CARRIER PROJECTION-COEXISTENCE pin — a Process with
    /// `classification.point_type = ConvergencePointType::Fork` MUST
    /// simultaneously satisfy BOTH `point-type-Fork` (the twelfth
    /// family, raw 8-arm [`ConvergencePointType`] discriminator on the
    /// SAME `self.point_type` scalar carrier) AND `input-arity-One`
    /// (the twenty-third family, derived 2-arm [`Arity`] reached
    /// through the typed projection
    /// [`ConvergencePointType::input_arity`]) AND simultaneously fail
    /// each off-diagonal probe (`point-type-Gate`, `input-arity-Many`).
    /// Locks the semantic split between the two probes on the SAME
    /// `self.point_type` scalar carrier — the second two-family
    /// same-carrier projection-coexistence conjunction in the point-
    /// domain require-tag vocabulary (the first, on the nested-Option
    /// `Option<TestReportSource>` carrier, is
    /// `report-format-<kind>` × `report-payload-shape-<kind>`). A
    /// regression that (a) collapsed the twenty-third family to the
    /// twelfth (silently answering `true` for `input-arity-Fork`, an
    /// out-of-vocabulary Arity), (b) collapsed the twelfth family to
    /// the twenty-third (silently answering `true` for
    /// `point-type-One`, an out-of-vocabulary ConvergencePointType),
    /// or (c) swapped the projection (`Fork → Many` instead of `Fork
    /// → One`) fails HERE at ONE narrow site. The audit `every Fork-
    /// topology point is a single-input fan-out` reads as this exact
    /// two-way conjunction at the checks.lisp surface — the fleet-
    /// wide DAG-composition property the (`Fork`, `One`) projection
    /// composition is designed to name.
    #[test]
    fn evaluate_point_require_tag_input_arity_and_point_type_coexist_via_projection() {
        let mut spec = ProcessSpec::gate_compute_defaults();
        spec.classification.point_type = ConvergencePointType::Fork;
        assert_eq!(
            evaluate_point_require_tag(&spec, "point-type-Fork"),
            Ok(true),
            "fine `point-type-Fork` must be true when the classification declares that variant",
        );
        assert_eq!(
            evaluate_point_require_tag(&spec, "input-arity-One"),
            Ok(true),
            "peer `input-arity-One` must be true: Fork projects to input_arity=One",
        );
        assert_eq!(
            evaluate_point_require_tag(&spec, "point-type-Gate"),
            Ok(false),
            "off-diagonal `point-type-Gate` must be false: the point_type is Fork",
        );
        assert_eq!(
    ...
    // ── report-format-<kind> prefix family (ephemeral) ───────────────
    //
    // Fail-before-pass-after granularity: the ephemeral require-tag
    // vocabulary carried FOUR closed-set-driven prefix families
    // (`condition-<kind>`, `teardown-policy-<kind>`,
    // `export-when-<kind>`, `channel-<kind>`) before this commit and
    // had no way to distinguish which REPORT FORMAT an ephemeral
    // export's `source.test_report.format` slot carries — a JUnit run
    // vs a NDJSON stream lift. The lift adds the FIFTH closed-set-
    // driven prefix family symmetrical with the four prior ones,
    // routing through the pre-existing
    // [`tatara_process::export::ExportSpecSliceExt::has_report_format`]
    // substrate primitive via `strip_and_classify_prefixed_kind`. THIRD
    // slice-child occupant routed DIRECTLY through the slice-level
    // substrate on the ephemeral surface — sibling of
    // `export-when-<kind>` (direct closed-set scalar `ExportSpec::when`)
    // and `channel-<kind>` (outer tagged-union carrier
    // `ChannelKind::from(&VectorChannel)`) on the SAME slice, distinct
    // on the child-projection axis: this family reads a NESTED-Option
    // scalar (`e.source.test_report.as_ref().is_some_and(|tr| tr.format
    // == kind)`), so a reachable-populated export whose
    // `source.test_report` slot is empty answers `false` for EVERY
    // format kind INCLUDING the default [`ReportFormat::Raw`] a naive
    // `unwrap_or_default()` projection would spuriously match.

    /// POPULATED-slot pin — `report-format-<kind>` dispatches through
    /// the autoderived [`ReportFormat`] `FromStr` + the substrate
    /// [`tatara_process::export::ExportSpecSliceExt::has_report_format`]
    /// primitive, returning `true` only when the ephemeral's `exports`
    /// vector carries at least one export whose
    /// `source.test_report.format` slot matches this kind. Sweep the
    /// [`ReportFormat::ALL`] × ALL cross so a regression that hard-
    /// coded the arm to a single kind (silently returning `true` for
    /// every populated export vector regardless of query kind), wired
    /// the closure to a fixed unrelated field (a stray
    /// `experiment_id_override.is_some()`, a probe on `when` /
    /// `channel`), or collapsed the outer nested-Option projection
    /// (probing `test_report.is_some()` alone and treating the empty
    /// case as `ReportFormat::default() == Raw`) fails HERE at the
    /// classifier before landing at the operator-facing checks.lisp
    /// surface. Byte-for-byte analog of the peer point-surface pin
    /// [`evaluate_point_require_tag_returns_true_on_populated_report_format_slot_per_kind`]
    /// — reuses the `export_with_report_format` fixture on a
    /// `EphemeralSpec { exports: … }` shape rather than the
    /// point-surface `ephemeral_spec_with_exports` shape.
    #[test]
    fn evaluate_ephemeral_require_tag_returns_true_on_populated_report_format_slot_per_kind() {
        for populated in ReportFormat::ALL {
            let spec = EphemeralSpec {
            ...
            for query in ReportFormat::ALL {
                ...
                assert_eq!(
                    evaluate_ephemeral_require_tag(&spec, &tag),
                    Ok(expected),
                    "ephemeral report format populated={populated:?}: tag {tag:?} classification drifted",
                );
            }
        }
    }
    ...
    /// empty returns `Ok(false)` for every `report-format-<kind>` tag.
    /// Locks the "reachable-empty-child" corner so a regression that
    /// short-circuited on presence of the ephemeral surface alone
    /// (returning `true` for every kind on any ephemeral), that
    /// treated the empty vector's default answer as
    /// `ReportFormat::default() == Raw`, or mis-routed the closure to
    /// a fixed always-true path fails HERE. Distinct from the point
    /// surface's peer test which pins TWO corners (unreachable-parent
    /// via permanent lifetime AND reachable-empty-child); this
    /// ephemeral pin covers only the reachable-empty-child corner
    /// because [`EphemeralSpec`] has no permanent variant to collapse.
    /// The default `ephemeral_fixture` (empty `exports: vec![]`) is
    /// the reachable-empty-child fixture.
    #[test]
    fn evaluate_ephemeral_require_tag_returns_false_on_empty_exports_for_every_report_format_kind()
    {
        let spec = ephemeral_fixture();
        for kind in ReportFormat::ALL {
            let tag = format!("report-format-{}", kind.as_str());
    ...
    /// source) returns `Ok(false)` for EVERY `report-format-<kind>`
    /// tag INCLUDING the default [`ReportFormat::Raw`] that a naive
    /// `unwrap_or_default()` projection would spuriously match. This
    /// is the ephemeral peer of the point surface's own NESTED-OPTION-
    /// COLLAPSE pin — proves the nested-Option short-circuit contract
    /// survives the surface change (no `resolved_ephemeral` gate to
    /// short-circuit here, so the closed-set walk MUST fire on the
    /// populated `exports` slot for the collapse to hold). A
    /// regression that dropped the `.as_ref().is_some_and(…)` gate on
    /// the substrate primitive fails HERE — the reachable-empty-child
    /// pin above would still pass (empty vector short-circuits before
    /// the nested-Option collapse fires). Sweeps [`ReportFormat::ALL`]
    /// so the collapse contract is pinned symmetrically across every
    /// format the closed set names.
    #[test]
    fn evaluate_ephemeral_require_tag_returns_false_on_non_test_report_source_for_every_report_format_kind(
    ) {
        let receipts_only = ExportSpec {
            source: ArtifactSource {
                receipts: Some(ReceiptsSource::default()),
        ...
        for kind in ReportFormat::ALL {
            ...
            assert_eq!(
                evaluate_ephemeral_require_tag(&spec, &tag),
                Ok(false),
                "receipts-only ephemeral export must return false for {tag:?} (including Raw)",
            );
        }
    }
    ...
    /// operator-facing `unknown :requires tag for ephemeral domain:
    /// <verbatim>` diagnostic path fires. The canonical
    /// [`ReportFormat`] labels are PascalCase (`Junit`, `TapV13`,
    /// `NdJson`, `Raw`) — matching the serde `rename_all =
    /// "PascalCase"` output verbatim — so camelCase / all-lowercase /
    /// all-caps spellings are UNKNOWN suffixes, a distinct contract
    /// from the camelCase closed set [`ChannelKind`] the fourth
    /// ephemeral family carries. Pin the case-sensitivity axis so a
    /// regression that ASCIIfolded or camel-cased on parse (a
    /// hypothetical `to_lower_camel` normalization at the tag layer)
    /// would fail HERE.
    #[test]
    fn evaluate_ephemeral_require_tag_returns_unknown_on_unknown_report_format_suffix() {
        let spec = ephemeral_fixture();
        for garbage in [
            "report-format-junit",
            "report-format-tapV13",
    ...
    /// BARE-PREFIX pin — the empty-suffix boundary at `report-format-`
    /// classifies as [`UnknownRequireTag`], mirroring every prior
    /// closed-set prefix family on the ephemeral surface. A regression
    /// that special-cased the bare prefix (treating it as a coarse
    /// "any report-format axis present") would fail HERE — the empty
    /// suffix must hit the substrate primitive's canonical empty-string
    /// arm rather than short-circuiting to `Ok(true)` on any ephemeral
    /// that carries a non-empty `exports` vector. Locks the
    /// empty-suffix ↔ unknown-suffix correspondence at ONE narrow
    /// classifier site.
    #[test]
    fn evaluate_ephemeral_require_tag_returns_unknown_on_bare_report_format_prefix() {
        let spec = EphemeralSpec {
            exports: vec![export_with_report_format(ReportFormat::Junit)],
            ..ephemeral_fixture()
        };
        assert_eq!(
    ...
    /// classifier AND the point surface's `evaluate_point_require_tag`
    /// classifier through the SAME
    /// [`ExportSpecSliceExt::has_report_format`] substrate primitive,
    /// so the two classifiers publish IDENTICAL answers for every
    /// [`ReportFormat`] × every `EphemeralSpec::exports` value. Lowers
    ...
    /// `report-format-<kind>` family — a regression that (a) dropped
    /// the ephemeral routing arm, (b) inverted the routing (probed a
    /// wrong slot), or (c) diverged the two surfaces' closure
    /// semantics fails HERE at ONE narrow site. Byte-for-byte analog
    /// of
    /// [`evaluate_ephemeral_require_tag_channel_matches_point_peer_through_lowered_exports`]
    /// on the sibling fourth family, but on the nested-Option scalar
    /// child projection (`test_report.as_ref().is_some_and(|tr|
    /// tr.format == kind)`) rather than the outer tagged-union carrier
    /// ([`ChannelKind::from(&VectorChannel)`]) — proving the parity
    /// contract survives the SECOND child-projection axis change in
    /// the ephemeral vocabulary AND the extra Option-parent hop on the
    /// point surface.
    #[test]
    fn evaluate_ephemeral_require_tag_report_format_matches_point_peer_through_lowered_exports() {
        for populated in ReportFormat::ALL {
            let eph = EphemeralSpec {
                exports: vec![export_with_report_format(populated)],
                ..ephemeral_fixture()
    ...
    // ── evaluate_ephemeral_require_tag / substrate-<kind> pins ───────
    //
    // Fail-before-pass-after granularity: the ephemeral surface's
    // `substrate-<kind>` prefix family did not route pre-lift — the
    // vocabulary block on `evaluate_ephemeral_require_tag` had a peer
    // to the point surface's twelfth family (`point-type-<kind>`) but
    // NOT the thirteenth (`substrate-<kind>`). Post-lift the routing
    // dispatches through the ONE substrate primitive
    // [`tatara_process::ephemeral::EphemeralSpec::has_substrate`] +
    // the sibling closed-set resolver
    // [`tatara_process::ephemeral::EphemeralSpec::resolved_classification`]
    // — a regression that (a) hard-coded the arm to a single kind, (b)
    // inverted the resolver's `None` fill-through (`Some(_)` filled
    // through the default), (c) probed the wrong slot (`point_type`
    // instead of `substrate`), or (d) drifted the fill-through default
    // from the sibling `From<EphemeralSpec>` lowering fails HERE at
    // the substrate primitives rather than as silent operator-facing
    // drift at the ephemeral `substrate-<kind>` require-tag surface.
    // SECOND classification-axis peer on the ephemeral surface — the
    // co-tenant on the (Option-parent × required-non-Option-scalar-
    // child × operator-resolvable-baseline) corner opened by the FIRST
    // classification-axis peer (`point-type-<kind>`).

    /// AUTHORED-slot VARIANT-MATCH pin — the ephemeral `substrate-
    /// <kind>` prefix family dispatches through the autoderived
    /// [`SubstrateType`] `FromStr` + the substrate
    /// [`tatara_process::ephemeral::EphemeralSpec::has_substrate`] +
    /// the sibling resolver
    /// [`tatara_process::ephemeral::EphemeralSpec::resolved_classification`]
    /// on the operator-authored [`Classification`] slot. Sweeps the
    /// [`SubstrateType::ALL`] × ALL cross so a stray hard-coded arm
    /// fails HERE. Byte-for-byte peer of the point-surface pin
    /// [`evaluate_point_require_tag_returns_true_iff_substrate_matches_variant_per_kind`]
    /// on the SAME closed set, routed through the ephemeral sugar
    /// surface.
    #[test]
        ...
        for populated in SubstrateType::ALL {
            ...
            for query in SubstrateType::ALL {
                ...
                assert_eq!(
                    evaluate_ephemeral_require_tag(&spec, &tag),
                    Ok(expected),
                    "ephemeral classification.substrate={populated:?}: tag {tag:?} drifted",
                );
            }
        }
    }
    ...
    /// Pins the (Option-parent × NON-DEFAULT-scalar-child ×
    /// operator-resolvable-baseline) corner's default-arm short-
    /// circuit on the SECOND classification-axis peer: `substrate-
    /// Compute` reads `true`, every other variant reads `false`.
    /// Distinct from the Option-parent corners on the encapsulates
    /// axes (`encapsulation-mode-<kind>` / `encapsulation-target-
    /// <kind>` / `routing-form-<kind>`) which return `false` on every
    /// variant on their `None`-parent arms: the ephemeral
    /// classification carrier fills through a resolved baseline rather
    /// than reading `false` past the missing parent.
    #[test]
    fn evaluate_ephemeral_require_tag_returns_true_on_absent_classification_for_compute_only() {
        let spec = EphemeralSpec {
            classification: None,
            ..ephemeral_fixture()
        };
        for kind in SubstrateType::ALL {
            let tag = format!("substrate-{}", kind.as_str());
    ...
    /// / `CalmClassification` identifiers must NOT resolve here). A
    /// regression that folded case (`FromStr` gone `case_insensitive`)
    /// or gained a permissive-fallthrough (e.g. unknown suffix routes
    /// to `Compute`) would silently reclassify an unknown tag as
    /// `Ok(_)` and fail HERE.
    #[test]
    fn evaluate_ephemeral_require_tag_returns_unknown_on_unknown_substrate_suffix() {
        let spec = ephemeral_fixture();
        for tag in [
            "substrate-compute",
            "substrate-STORAGE",
            "substrate-Networking",
            "substrate-Cache",
            "substrate-Fork",        // point-type variant leaking
            "substrate-NonMonotone", // calm variant leaking
        ] {
            assert_eq!(
                evaluate_ephemeral_require_tag(&spec, tag),
                Err(UnknownRequireTag::default()),
    ...
    /// BARE-prefix pin — `"substrate-"` (empty suffix) MUST classify
    /// as [`UnknownRequireTag`] rather than as any variant. Locks the
    /// suffix-nonempty invariant at the substrate
    /// `strip_and_classify_prefixed_kind` boundary on the ephemeral
    /// surface's SECOND classification-axis peer.
    #[test]
    fn evaluate_ephemeral_require_tag_returns_unknown_on_bare_substrate_prefix() {
        let spec = ephemeral_fixture();
        assert_eq!(
    ...
    /// lowered `ProcessSpec` via `<eph.clone().into()>`. Sweeps both
    /// the (`None` classification) baseline arm and every
    /// [`SubstrateType::ALL`] populated arm × every ALL query so a
    ...
    /// two-surface parity pin on the FIRST classification-axis peer
    /// (`point-type-<kind>`) — the SECOND classification-axis two-
    /// surface parity contract on the ephemeral surface.
    #[test]
    fn evaluate_ephemeral_require_tag_substrate_matches_point_peer_through_resolved_classification()
    {
        // Absent classification baseline — both surfaces resolve
        // through the SAME `default_ephemeral_class` fill-through and
        // agree on every variant.
    ...
    // ── evaluate_ephemeral_require_tag / data-classification-<kind> pins ─
    //
    // Fail-before-pass-after granularity: the ephemeral surface's
    // `data-classification-<kind>` prefix family did not route pre-lift
    // — the vocabulary block on `evaluate_ephemeral_require_tag` had
    // peers to the point surface's twelfth + thirteenth + fourteenth
    // families (`point-type-<kind>` + `substrate-<kind>` +
    // `calm-<kind>`) but NOT the fifteenth (`data-classification-
    // <kind>`). Post-lift the routing dispatches through the ONE
    // substrate primitive
    // [`tatara_process::ephemeral::EphemeralSpec::has_data_classification`]
    // + the sibling resolver
    // [`tatara_process::ephemeral::EphemeralSpec::resolved_classification`]
    // — a regression that (a) hard-coded the arm to a single kind, (b)
    // inverted the resolver's `None` fill-through (`Some(_)` filled
    // through the default), (c) probed the wrong slot (`calm` instead
    // of `data_classification`), or (d) drifted the fill-through
    ...
    // facing drift at the ephemeral `data-classification-<kind>`
    // require-tag surface. FOURTH classification-axis peer on the
    // ephemeral surface — SECOND occupant on the (Option-parent ×
    // DEFAULTED-scalar-child × operator-resolvable-baseline) corner
    // alongside `calm-<kind>`. The two-defaults composition property
    // (parent fill-through baseline AND child `#[default]` land on
    // the SAME variant [`DataClassification::Internal`]) is exercised
    // at the four pins below.

    /// AUTHORED-slot VARIANT-MATCH pin — the ephemeral
    /// `data-classification-<kind>` prefix family dispatches through
    /// the autoderived [`DataClassification`] `FromStr` + the
    /// substrate
    /// [`tatara_process::ephemeral::EphemeralSpec::has_data_classification`]
    /// + the sibling resolver
    /// [`tatara_process::ephemeral::EphemeralSpec::resolved_classification`]
    /// on the operator-authored [`Classification`] slot. Sweeps the
    /// [`DataClassification::ALL`] × ALL cross so a stray hard-coded
    /// arm fails HERE. Byte-for-byte peer of the point-surface pin
    /// [`evaluate_point_require_tag_returns_true_iff_data_classification_matches_variant_per_kind`]
    /// on the SAME closed set, routed through the ephemeral sugar
    /// surface.
    #[test]
        ...
        for populated in DataClassification::ALL {
            ...
            for query in DataClassification::ALL {
                ...
                assert_eq!(
                    evaluate_ephemeral_require_tag(&spec, &tag),
                    Ok(expected),
                    "ephemeral classification.data_classification={populated:?}: tag {tag:?} drifted",
                );
            }
        }
    }
    ...
    /// Pins the (Option-parent × DEFAULTED-scalar-child ×
    /// operator-resolvable-baseline) corner's default-arm short-
    /// circuit on the FOURTH classification-axis peer:
    /// `data-classification-Internal` reads `true`, every other
    /// variant reads `false`. SECOND occupant on that corner after
    /// [`evaluate_ephemeral_require_tag_returns_true_on_absent_classification_for_monotone_only`]
    /// opened it — both the parent Option's fill-through baseline
    /// (`default_ephemeral_class`) AND the child's own `#[default]`
    /// land on the SAME variant ([`DataClassification::Internal`]),
    /// so the workspace's internal-by-default sensitivity posture
    /// reads as `true` at the ephemeral `data-classification-Internal`
    /// require-tag on every operator-authored spec that omits both
    /// the `:classification` slot AND the `:data-classification`
    /// sub-slot.
    #[test]
    fn evaluate_ephemeral_require_tag_returns_true_on_absent_classification_for_internal_only() {
        let spec = EphemeralSpec {
            classification: None,
            ..ephemeral_fixture()
        };
    ...
    /// UNKNOWN-suffix pin — `data-classification-<garbage>` classifies
    /// as [`UnknownRequireTag`] rather than silently returning
    /// `Ok(false)`. Sweeps case-variants of a known variant name plus
    /// pure garbage plus cross-axis rejects (raw
    /// `ConvergencePointType` / `SubstrateType` / `CalmClassification`
    /// identifiers must NOT resolve here). A regression that folded
    /// case (`FromStr` gone `case_insensitive`) or gained a
    /// permissive-fallthrough (e.g. unknown suffix routes to
    /// `Internal`) would silently reclassify an unknown tag as
    /// `Ok(_)` and fail HERE.
    #[test]
    fn evaluate_ephemeral_require_tag_returns_unknown_on_unknown_data_classification_suffix() {
        let spec = ephemeral_fixture();
        for tag in [
            "data-classification-internal",
            "data-classification-PUBLIC",
            ...
            "data-classification-Gate",     // point-type variant leaking
            "data-classification-Compute",  // substrate variant leaking
            "data-classification-Monotone", // calm variant leaking
        ] {
            assert_eq!(
                evaluate_ephemeral_require_tag(&spec, tag),
                Err(UnknownRequireTag::default()),
    ...
    /// BARE-prefix pin — `"data-classification-"` (empty suffix) MUST
    /// classify as [`UnknownRequireTag`] rather than as any variant.
    /// Locks the suffix-nonempty invariant at the substrate
    /// `strip_and_classify_prefixed_kind` boundary on the ephemeral
    /// surface's FOURTH classification-axis peer.
    #[test]
    fn evaluate_ephemeral_require_tag_returns_unknown_on_bare_data_classification_prefix() {
        let spec = ephemeral_fixture();
        assert_eq!(
    ...
    /// lowered `ProcessSpec` via `<eph.clone().into()>`. Sweeps both
    /// the (`None` classification) baseline arm and every
    /// [`DataClassification::ALL`] populated arm × every ALL query so
    /// a future regression on either surface's resolver (an ephemeral-
    /// side fill-through drift, a point-side scalar-carrier probe
    /// drift) fails HERE. Byte-for-byte peer of the sibling
    /// two-surface parity pins on the FIRST + SECOND + THIRD
    /// classification-axis peers (`point-type-<kind>`,
    /// `substrate-<kind>`, `calm-<kind>`) — the FOURTH classification-
    /// axis two-surface parity contract on the ephemeral surface, and
    /// the SECOND on the (Option-parent × DEFAULTED-scalar-child)
    /// corner.
    #[test]
    fn evaluate_ephemeral_require_tag_data_classification_matches_point_peer_through_resolved_classification(
    ) {
        // Absent classification baseline — both surfaces resolve
        // through the SAME `default_ephemeral_class` fill-through and
        // agree on every variant.
    ...
    // ── evaluate_ephemeral_require_tag / input-arity-<kind> pins ─────
    //
    // Fail-before-pass-after granularity: the ephemeral surface's
    // `input-arity-<kind>` prefix family did not route pre-lift — the
    // vocabulary block on `evaluate_ephemeral_require_tag` had peers
    // to the point surface's twelfth through seventeenth families
    // (`point-type-<kind>` + `substrate-<kind>` + `calm-<kind>` +
    // `data-classification-<kind>` + `horizon-<kind>` +
    // `optimization-direction-<kind>`) but NOT the twenty-third
    // (`input-arity-<kind>`). Post-lift the routing dispatches through
    // the ONE substrate primitive
    // [`tatara_process::ephemeral::EphemeralSpec::has_input_arity`] +
    // the sibling resolver
    // [`tatara_process::ephemeral::EphemeralSpec::resolved_classification`]
    // + the sibling closed-set primitive
    // [`tatara_process::classification::Classification::has_input_arity`]
    // (which composes the many-to-one typed projection
    // [`ConvergencePointType::input_arity`] on `self.point_type`) — a
    // regression that (a) hard-coded the arm to a single kind, (b)
    // inverted the resolver's `None` fill-through, (c) dropped the
    // `.input_arity()` typed projection call, (d) inverted the
    // projection (`One ↔ Many`), (e) crossed wires with the sibling
    // `ConvergencePointType::output_arity` projection, or (f) drifted
    ...
    // `input-arity-<kind>` require-tag surface. SEVENTH classification-
    // axis peer on the ephemeral surface — FIRST occupant on the
    // (Option-parent × NESTED-STRUCT-scalar-child × derived-typed-
    // projection) corner on the ephemeral surface, opening the
    // corner's ephemeral-side traversal shape. [`Arity`] carries no
    // `#[default]`, so no default-arm short-circuit shortcut applies
    // here — the absent-classification baseline routes entirely
    // through the projection's bucket-membership decision on
    // `default_ephemeral_class`'s `point_type: Gate` (projects to
    // `Arity::Many` on the input side).

    /// AUTHORED-slot PROJECTED-VARIANT pin — the ephemeral
    /// `input-arity-<kind>` prefix family dispatches through the
    ...
    /// [`ConvergencePointType::output_arity`], or an inverted projection
    /// fails HERE. Byte-for-byte peer of the point-surface pin
    /// [`evaluate_point_require_tag_returns_true_iff_input_arity_matches_projection_per_point_type`]
    /// on the SAME closed set, routed through the ephemeral sugar
    /// surface.
    #[test]
    ...
    /// [`ConvergencePointType::input_arity`] projects `Gate →
    /// Arity::Many`. Pins the (Option-parent × NESTED-STRUCT-scalar-
    /// child × derived-typed-projection) corner's baseline projection
    /// on the SEVENTH classification-axis peer through a chain of ONE
    /// fill-through composed with ONE many-to-one projection:
    /// `input-arity-Many` reads `true`, `input-arity-One` reads
    /// `false`. [`Arity`] carries no `#[default]`, so this pin exercises
    /// the projection's bucket-membership decision on the baseline
    /// rather than any default-arm short-circuit. A regression that
    /// promoted the baseline's `point_type` off `Gate`, dropped the
    /// projection call, inverted the projection, or crossed wires with
    /// [`ConvergencePointType::output_arity`] (which projects
    /// `Gate → Arity::One`, flipping the baseline answer from `Many`
    /// to `One`) fails HERE.
    #[test]
    fn evaluate_ephemeral_require_tag_returns_true_on_absent_classification_for_input_arity_many_only(
    ) {
        let spec = EphemeralSpec {
            classification: None,
            ..ephemeral_fixture()
        ...
        for kind in Arity::ALL {
            let tag = format!("input-arity-{}", kind.as_str());
            let expected = kind == Arity::Many;
            assert_eq!(
                evaluate_ephemeral_require_tag(&spec, &tag),
                Ok(expected),
                "absent ephemeral classification (defaults to gate_compute, point_type=Gate → input_arity=Many): tag {tag:?} drifted",
            );
        }
    }
    ...
    /// regression that folded case (`FromStr` gone `case_insensitive`)
    /// or gained a permissive-fallthrough (e.g. unknown suffix routes
    /// to `Many`) would silently reclassify an unknown tag as `Ok(_)`
    /// and fail HERE.
    #[test]
    fn evaluate_ephemeral_require_tag_returns_unknown_on_unknown_input_arity_suffix() {
        let spec = ephemeral_fixture();
        for tag in [
            "input-arity-one",
            "input-arity-many",
            ...
            "input-arity-Zero",
            "input-arity-Bogus",
            "input-arity-Gate",     // point-type variant leaking
            "input-arity-Compute",  // substrate variant leaking
            "input-arity-Monotone", // calm variant leaking
            "input-arity-Internal", // data-classification variant leaking
            "input-arity-Bounded",  // horizon-kind variant leaking
            "input-arity-Minimize", // optimization-direction variant leaking
        ] {
            assert_eq!(
                evaluate_ephemeral_require_tag(&spec, tag),
                Err(UnknownRequireTag::default()),
    ...
    /// BARE-prefix pin — `"input-arity-"` (empty suffix) MUST classify
    /// as [`UnknownRequireTag`] rather than as any variant. Locks the
    /// suffix-nonempty invariant at the substrate
    /// `strip_and_classify_prefixed_kind` boundary on the ephemeral
    /// surface's SEVENTH classification-axis peer.
    #[test]
    fn evaluate_ephemeral_require_tag_returns_unknown_on_bare_input_arity_prefix() {
        let spec = ephemeral_fixture();
        assert_eq!(
    ...
    /// drift, a wire-cross with `output_arity` on either side) fails
    /// HERE. Byte-for-byte peer of the sibling two-surface parity pins
    /// on the FIRST through SIXTH classification-axis peers
    /// (`point-type-<kind>`, `substrate-<kind>`, `calm-<kind>`,
    /// `data-classification-<kind>`, `horizon-<kind>`,
    /// `optimization-direction-<kind>`) — the SEVENTH classification-
    /// axis two-surface parity contract on the ephemeral surface, and
    /// the FIRST on the (Option-parent × NESTED-STRUCT-scalar-child ×
    /// derived-typed-projection) corner.
    #[test]
    fn evaluate_ephemeral_require_tag_input_arity_matches_point_peer_through_resolved_classification(
    ) {
        // Absent classification baseline — both surfaces resolve
        // through the SAME `default_ephemeral_class` fill-through and
        // agree on every variant.
    ...
    // ── data-restricted fixed tag substrate pins ─────────────────────
    //
    // Fail-before-pass-after granularity: the `data-restricted` fixed
    // tag did not exist before this commit — the (parent × derived-
    // nullary-bool) corner of the fixed-tag algebra carried four peers
    // (`terminating-horizon`, `metric-axes-required`,
    // `coordination-required`, `data-regulated`) across three axes.
    // This lift opens the FIFTH occupant on the corner (SECOND on the
    // classification-data axis) via ONE substrate primitive
    // [`tatara_process::classification::Classification::data_is_restricted`].
    // Every test below binds through the SAME substrate primitive so
    // a regression that (a) hard-coded either surface's arm to a
    // fixed answer, (b) crossed the wires with the sibling
    // `data-regulated` arm (which disagrees on `Internal | Confidential`),
    // (c) dropped the ephemeral surface's resolver hop, or (d) drifted
    // the two-surface parity contract fails HERE at ONE narrow
    // classifier site per pin before landing at the operator-facing
    // checks.lisp surface. FIRST fixed-tag corner peer whose gate-
    // compute point-surface baseline projects to `true`, and FIRST
    // corner pair whose two arms carry a non-trivial closed-set-
    // internal implication (`data-regulated ⇒ data-restricted`).

    /// POINT SURFACE per-variant pin — for every
    /// [`DataClassification`] variant, a [`ProcessSpec`] whose
    /// `classification.data_classification` field carries that variant
    /// answers the `data-restricted` fixed tag matching the closed
    /// set's own [`DataClassification::is_restricted`] truth table.
    /// Sweep [`DataClassification::ALL`] so a regression that probed
    /// a fixed variant, inverted the projection, or crossed the wires
    /// with the sibling [`DataClassification::is_regulated`]
    /// projection (which disagrees on `Internal | Confidential`)
    /// fails HERE at the classifier before landing at the operator-
    /// facing surface.
    #[test]
    fn evaluate_point_require_tag_returns_data_is_restricted_projection_per_data_kind() {
        for populated in DataClassification::ALL {
            ...
            assert_eq!(
                ...
                "point data_classification={populated:?}: data-restricted drift from DataClassification::is_restricted()",
            );
        }
    }
    ...
    /// through [`ProcessSpec::gate_compute_defaults`] (which carries
    /// `data_classification: DataClassification::default()` =
    /// `Internal` via `#[default]`) answers `Ok(true)` on
    /// `data-restricted` WITHOUT the operator naming the data axis.
    /// Pins the default-arm short-circuit through ONE layer of
    /// `Default` (`DataClassification`'s) at ONE narrow classifier
    /// site. FIRST direct-scalar fixed-tag corner peer whose point-
    /// surface baseline projects to `true` (mirror-image of the
    /// nested-struct `terminating-horizon` baseline that walks TWO
    /// layers of `Default`). A regression that promoted
    /// [`DataClassification::Public`] to `#[default]`, or that wired
    /// [`DataClassification::Internal`] to `is_restricted() = false`,
    /// or that wired the fixed tag's probe to a fixed negative
    /// answer, would fail HERE.
    #[test]
    fn evaluate_point_require_tag_returns_true_on_default_data_restricted() {
        let spec = ProcessSpec::gate_compute_defaults();
        assert_eq!(
            evaluate_point_require_tag(&spec, "data-restricted"),
            Ok(true),
            "default (data_classification=Internal → is_restricted=true) baseline",
        );
    }
    ...
    /// [`DataClassification`] variant, an [`EphemeralSpec`] whose
    /// authored [`Classification`] carries that variant answers the
    /// `data-restricted` fixed tag matching the closed set's own
    /// [`DataClassification::is_restricted`] truth table. Byte-for-
    /// byte peer of the point-surface per-variant pin above via the
    /// SAME [`Classification::data_is_restricted`] primitive reached
    /// through the ephemeral surface's `resolved_classification()`
    /// resolver.
    #[test]
    fn evaluate_ephemeral_require_tag_returns_data_is_restricted_projection_per_data_kind() {
        for populated in DataClassification::ALL {
            ...
            assert_eq!(
                ...
                Ok(populated.is_restricted()),
                "ephemeral data_classification={populated:?}: data-restricted drift",
            );
        }
    }
    ...
    /// [`Classification::gate_compute`] (whose data_classification
    /// defaults to [`DataClassification::Internal`] →
    /// `is_restricted() = true`), so the `data-restricted` fixed tag
    /// answers `Ok(true)` WITHOUT the operator naming the
    /// classification axis on `(defephemeral …)`. Pins the default-
    /// arm short-circuit through TWO layers of `Default` at ONE
    /// narrow classifier site. FIRST direct-scalar ephemeral-surface
    /// fixed-tag corner peer whose absent-classification baseline
    /// projects to `true`, not `false` — every other direct-scalar
    /// arm on this surface (`data-regulated`, `coordination-required`)
    /// projects `false` on the same absent classification, and only
    /// the sibling nested-struct `terminating-horizon` arm projects
    /// `true` (through THREE layers of `Default`).
    #[test]
    fn evaluate_ephemeral_require_tag_returns_true_on_absent_classification_for_data_restricted() {
        let spec = ephemeral_fixture();
        assert!(spec.classification.is_none());
        assert_eq!(
            evaluate_ephemeral_require_tag(&spec, "data-restricted"),
    ...
    /// (`None`, `Some(_)` on every [`DataClassification::ALL`] variant))
    /// classifies IDENTICALLY through the point-surface
    /// `data-restricted` fixed tag AND the ephemeral-surface
    /// `data-restricted` fixed tag when the ephemeral spec is
    /// mechanically lowered to a [`ProcessSpec`] via `From`. Byte-
    /// for-byte peer of
    /// `evaluate_data_regulated_matches_across_surfaces_through_lowered_ephemeral`
    /// on the same classification-data axis, published a second time
    /// through the antisymmetric closed-set projection.
    #[test]
    fn evaluate_data_restricted_matches_across_surfaces_through_lowered_ephemeral() {
        // Absent classification.
        let eph = ephemeral_fixture();
        let lowered: ProcessSpec = eph.clone().into();
        assert_eq!(
            evaluate_ephemeral_require_tag(&eph, "data-restricted"),
            evaluate_point_require_tag(&lowered, "data-restricted"),
            "None-classification parity drift",
        );
        // Authored classification.
        for populated in DataClassification::ALL {
    ...
    /// COMPOSED-IMPLICATION pin — for every [`ProcessSpec`] whose
    /// `classification.data_classification` carries every
    /// [`DataClassification::ALL`] variant, the classifier satisfies
    /// `data-regulated ⇒ data-restricted` at the operator-facing
    /// classifier surface. Lifts the closed-set-internal implication
    /// `data_classification_regulated_implies_restricted` +
    /// its parent-composed peer
    /// `classification_data_is_regulated_implies_data_is_restricted_over_all`
    /// through the fixed-tag boundary to the checks.lisp surface. FIRST
    /// fixed-tag corner pair whose two arms carry a non-trivial closed-
    /// set-internal implication — a future compliance-baseline auto-
    /// selector authoring `:requires (data-regulated data-restricted)`
    /// can rely on `data-regulated ⇒ data-restricted` by construction
    /// rather than restating the implication at the checks.lisp site.
    #[test]
    fn evaluate_point_require_tag_data_regulated_implies_data_restricted_over_all() {
        for populated in DataClassification::ALL {
            ...
            let restricted = evaluate_point_require_tag(&spec, "data-restricted")
                .expect("data-restricted is a registered fixed tag");
            assert!(
    ...
    // ── diffusive-point fixed tag substrate pins ─────────────────────
    //
    // Fail-before-pass-after granularity: the `diffusive-point` fixed
    // tag did not exist before this commit — the (parent × derived-
    // nullary-bool) corner of the fixed-tag algebra carried six peers
    // across FOUR closed-set axes (horizon × 2, calm × 1, data × 2,
    // point-type × 1). This lift opens the SEVENTH occupant on the
    // corner (SECOND on the `point_type` axis) via ONE substrate
    // primitive
    // [`tatara_process::classification::Classification::point_is_diffusive`].
    // FIRST fixed-tag corner-peer pair on the `point_type` axis whose
    // two arms carry a non-trivial closed-set-internal MUTEX
    // relationship, distinct from the sibling `data`-axis pair whose
    // two arms carry a non-trivial IMPLICATION relationship.

    /// POINT SURFACE per-variant pin — for every
    /// [`ConvergencePointType`] variant, a [`ProcessSpec`] whose
    /// `classification.point_type` field carries that variant answers
    /// the `diffusive-point` fixed tag matching the closed set's own
    ...
    fn evaluate_point_require_tag_returns_point_is_diffusive_projection_per_point_kind() {
        for populated in ConvergencePointType::ALL {
            ...
            assert_eq!(
                ...
                "point point_type={populated:?}: diffusive-point drift from ConvergencePointType::is_diffusive()",
            );
        }
    }
    ...
    /// `point_type: ConvergencePointType::Gate` deliberately — NOT
    /// via `#[default]` because [`ConvergencePointType`] has no
    /// `impl Default`) answers `Ok(false)` on `diffusive-point`
    /// WITHOUT the operator naming the point-type axis (Gate is a
    /// convergent barrier, not a diffusive fan-out). Pins the
    /// chosen-field baseline at ONE narrow classifier site. Peer of
    /// `evaluate_point_require_tag_returns_false_on_default_endomorphic_point`
    /// on the sibling projection.
    #[test]
    fn evaluate_point_require_tag_returns_false_on_default_diffusive_point() {
        let spec = ProcessSpec::gate_compute_defaults();
        assert_eq!(
            evaluate_point_require_tag(&spec, "diffusive-point"),
            Ok(false),
            "default (point_type=Gate → is_diffusive=false) baseline",
        );
    }
    ...
    /// [`ConvergencePointType`] variant, an [`EphemeralSpec`] whose
    /// authored [`Classification`] carries that variant answers the
    /// `diffusive-point` fixed tag matching the closed set's own
    /// [`ConvergencePointType::is_diffusive`] truth table. Byte-for-
    /// byte peer of the point-surface per-variant pin above via the
    /// SAME [`Classification::point_is_diffusive`] primitive reached
    /// through the ephemeral surface's `resolved_classification()`
    /// resolver.
    #[test]
    fn evaluate_ephemeral_require_tag_returns_point_is_diffusive_projection_per_point_kind() {
        for populated in ConvergencePointType::ALL {
            ...
            assert_eq!(
                ...
                Ok(populated.is_diffusive()),
                "ephemeral point_type={populated:?}: diffusive-point drift",
            );
        }
    }
    ...
    /// [`Classification::gate_compute`] (whose `point_type` is
    /// deliberately [`ConvergencePointType::Gate`] →
    /// `is_diffusive() = false`), so the `diffusive-point` fixed tag
    /// answers `Ok(false)` WITHOUT the operator naming the
    /// classification axis on `(defephemeral …)`.
    #[test]
    fn evaluate_ephemeral_require_tag_returns_false_on_absent_classification_for_diffusive_point() {
        let spec = ephemeral_fixture();
        assert!(spec.classification.is_none());
        assert_eq!(
            evaluate_ephemeral_require_tag(&spec, "diffusive-point"),
    ...
    /// (`None`, `Some(_)` on every [`ConvergencePointType::ALL`]
    /// variant)) classifies IDENTICALLY through the point-surface
    /// `diffusive-point` fixed tag AND the ephemeral-surface
    /// `diffusive-point` fixed tag when the ephemeral spec is
    /// mechanically lowered to a [`ProcessSpec`] via `From`. Byte-
    /// for-byte peer of
    /// `evaluate_endomorphic_point_matches_across_surfaces_through_lowered_ephemeral`
    /// on the SAME closed-set axis (`point_type`) via a sibling
    /// projection.
    #[test]
    fn evaluate_diffusive_point_matches_across_surfaces_through_lowered_ephemeral() {
        // Absent classification.
        let eph = ephemeral_fixture();
        let lowered: ProcessSpec = eph.clone().into();
        assert_eq!(
            evaluate_ephemeral_require_tag(&eph, "diffusive-point"),
            evaluate_point_require_tag(&lowered, "diffusive-point"),
            "None-classification parity drift",
        );
        // Authored classification.
        for populated in ConvergencePointType::ALL {
    ...
    /// POINT-SURFACE MUTEX pin — the `endomorphic-point` and
    /// `diffusive-point` fixed tags are NEVER simultaneously
    /// `Ok(true)` on ANY [`ConvergencePointType`] variant, sealing
    /// the disjoint-bucket contract at the classifier layer. Sweep
    /// [`ConvergencePointType::ALL`] so a regression that crossed
    /// the wires between the two arms at the fixed-tag table (one
    /// probe silently composing the wrong substrate primitive) fails
    /// HERE before drifting through every operator-facing
    /// `:requires (endomorphic-point diffusive-point)` audit that
    /// trusts the two tags partition the point-type slot into
    /// disjoint buckets. FIRST fixed-tag corner-peer MUTEX pin at
    ...
    fn evaluate_endomorphic_point_and_diffusive_point_are_mutex_over_all_point_variants() {
        for populated in ConvergencePointType::ALL {
            ...
            assert!(
                ...
                "point point_type={populated:?}: endomorphic-point AND diffusive-point both Ok(true) (mutex violated)",
            );
        }
    }

    /// EPHEMERAL-SURFACE MUTEX pin — the peer of the point-surface
    /// mutex pin above via the ephemeral surface's
    /// `resolved_classification()` resolver. Sweeps the absent-
    /// classification case (both arms answer `Ok(false)`) plus every
    /// [`ConvergencePointType::ALL`] variant so a regression on
    /// either arm of the resolver-hop fixed tag fails HERE.
    #[test]
    fn evaluate_endomorphic_point_and_diffusive_point_are_mutex_over_all_ephemeral_variants() {
        // Absent classification.
        let eph = ephemeral_fixture();
        ...
        assert!(
            !(endo == Ok(true) && diff == Ok(true)),
            "None-classification: endomorphic-point AND diffusive-point both Ok(true) (mutex violated)",
        );
        // Authored classification.
        for populated in ConvergencePointType::ALL {
            let eph = ephemeral_fixture().with_classification_axis(populated);
    ...
    /// DIAGNOSTIC-PREFIX pin — each registered domain's
    /// `diagnostic_prefix()` returns the exact pre-lift `format!`-head
    /// literal. Post-lift the default
    /// [`super::RequireTagDomain::unknown_tag_diagnostic`] composes
    /// this head with the shared `": {tag}{err}"` suffix, so a
    /// regression that renamed a prefix would silently reshape every
    /// operator-facing `"unknown :requires tag[...]"` prose downstream
    /// (grep-breaking) — this pin catches it BEFORE the executor
    /// composes.
    #[test]
    fn require_tag_domain_diagnostic_prefix_matches_pre_lift_head_literal() {
        let point = require_tag_domain_by_name("point").expect("point domain registered");
        assert_eq!(point.diagnostic_prefix(), "unknown :requires tag");
        let eph = require_tag_domain_by_name("ephemeral").expect("ephemeral domain registered");
        assert_eq!(
            eph.diagnostic_prefix(),
            "unknown :requires tag for ephemeral domain",
        );
    }

    /// DEFAULT-COMPOSITION pin — the trait-provided default
    /// [`super::RequireTagDomain::unknown_tag_diagnostic`] composes as
    /// `<diagnostic_prefix>: <tag><err_display>` byte-for-byte on a
    /// fixture domain that has no other override. Load-bearing on a
    /// FUTURE third domain: a domain whose only per-domain declaration
    /// is a fresh `diagnostic_prefix` inherits the exact composition
    /// shape both current arms restated pre-lift — this pin locks it.
    #[test]
    fn default_unknown_tag_diagnostic_composes_prefix_and_tag_and_err() {
        use super::RequireTagDomain;
        struct FixtureDomain;
        impl RequireTagDomain for FixtureDomain {
            fn name(&self) -> &'static str {
                "fixture"
            }
            fn diagnostic_prefix(&self) -> &'static str {
                "unknown :requires tag on fixture-only surface"
            }
            fn compile(&self, _src: &str) -> tatara_lisp::Result<super::CompiledSource> {
                unreachable!("compile not exercised in this pin");
            }
        }
        let f = FixtureDomain;
        let bare = UnknownRequireTag::default();
        assert_eq!(
            f.unknown_tag_diagnostic("some-tag", &bare),
            "unknown :requires tag on fixture-only surface: some-tag",
        );
        // Hint-bearing err — the fixed-tag near-miss adornment composes
        // through the same `{err}` slot the default's `format!` threads,
        // so a future domain inherits BOTH the prefix + the adornment
        // composition without a per-domain override.
        let with_hint = UnknownRequireTag::with_fixed_hint("depends-on".to_owned());
        assert_eq!(
            f.unknown_tag_diagnostic("deponds-on", &with_hint),
            "unknown :requires tag on fixture-only surface: deponds-on (did you mean \"depends-on\"?)",
        );
    }

    /// BUILD-COMPILED-SOURCE-COUNT pin — the substrate helper reports
    /// `defs.len()` verbatim on the returned [`super::CompiledSource`].
    /// Pre-lift both [`super::PointDomain::compile`] +
    /// [`super::EphemeralDomain::compile`] hand-authored `let count =
    /// defs.len();` before the closure capture; post-lift ONE substrate
    /// owner threads the count so a regression that swapped `count`
    /// for a hard-coded `1`, `0`, or the closure's own state would
    /// fail HERE before drifting into `min_defs_shortfall_msg`'s
    /// operator-facing shortfall prose.
    #[test]
    fn build_compiled_source_returns_defs_len_verbatim() {
        let defs: Vec<tatara_lisp::NamedDefinition<u32>> = (0..3)
            .map(|i| tatara_lisp::NamedDefinition {
                name: format!("n{i}"),
                spec: i,
            })
            .collect();
        fn eval(_spec: &u32, _tag: &str) -> Result<bool, UnknownRequireTag> {
            Ok(true)
        }
        let compiled = build_compiled_source(defs, eval);
        assert_eq!(compiled.count, 3, "count mirrors defs.len() verbatim");
    }

    /// BUILD-COMPILED-SOURCE-FIRST-DEF pin — the substrate helper's
    /// classify closure captures the first definition's spec and
    /// evaluates the passed-in function against it. Feeds the fixture
    /// evaluator two u8 sentinels (matching the u8-typed fixture spec)
    /// so the assertion pins BOTH `evaluate(&defs[0].spec, tag)` (not
    /// `defs[1]` or a snapshot of the tag) AND the tag forwarding.
    /// A regression that captured the last def, cloned the spec into
    /// the closure prematurely, or dropped the tag argument would fail
    /// HERE before drifting into the executor's per-tag classification
    /// loop.
    #[test]
    fn build_compiled_source_classify_calls_evaluator_over_first_def() {
        let defs = vec![
            tatara_lisp::NamedDefinition {
                name: "first".to_owned(),
                spec: 1u8,
            },
            tatara_lisp::NamedDefinition {
        ...
        // Evaluator returns Ok(true) iff spec is the FIRST def's spec
        // (u8 sentinel 1); tag "first-hit" gates the branch so a
        // regression that dropped the tag forwarding would flip the
        // classification.
        fn eval(spec: &u8, tag: &str) -> Result<bool, UnknownRequireTag> {
            Ok(*spec == 1 && tag == "first-hit")
        }
        let compiled = build_compiled_source(defs, eval);
        assert_eq!(
            (compiled.classify)("first-hit"),
            Ok(true),
            "classifier binds to defs[0].spec (u8=1) AND forwards the tag",
        );
        assert_eq!(
            (compiled.classify)("miss"),
            Ok(false),
            "tag forwarding — a non-matching tag misses",
        );
    }
