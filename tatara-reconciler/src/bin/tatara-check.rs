...
/// three-step chain now dispatches FIFTEEN closed-set prefix families
...
/// (via the `probe` closure) independently, so a regression that
/// drifted one prefix family's strip/parse discipline from the other
